 ...
 */ 
...

 {
		...
		var dotPosition=-1;			
		dotPosition=strData.indexOf(".");
		var length=-1;

		if (dotPosition>1 && strData.charAt(0)=="0")//有小数点
		{
			length=dotPosition-2;
		}
		else if (dotPosition<0 && strData.charAt(0)=="0")//没有小数部分
		{
			length=strData.length-2;
		}
		...
		{				
			...
			firstNotZero=i+1;
			if (strTemp.charAt(i)=="0" && strTemp.charAt(i+1)=="0")
			{}
			else
				break;
		}
		if (firstNotZero>0)
		{
			strData=strTemp.substring(firstNotZero,strTemp.length);
		}
...
function checkNumber(d_input,d_notnull,d_str)

{

		if ( !isFloat(resumeNumber(d_input.value)))
...

    {
    	...
    	if  ( ((d_notnull==1) || (d_input.value!=""))  && resumeNumber(d_input.value) < 0 )

    	{

			alert(d_str+ "的值不能为空或小于0。" );
   ...
   if  ( ((d_input.value!="")) && resumeNumber(d_input.value) > 1000000000000 )
 ...
 * @param d_notzero 如果不能为0，填1，否则为0

 */

function checkRate(d_input,d_notnull,d_str,d_notzero)
{
	if (d_notzero == 1)
	{
		return InputValid(d_input,d_notnull,"float",1,0.000001,200,d_str);
	}
	else
	{
