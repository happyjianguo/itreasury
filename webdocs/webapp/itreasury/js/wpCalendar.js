...
function wpCalendar(){
	...
	this.redraw		= function (){				//定义redraw方法，用于重绘整个日历
		...
		with(this){
			...
			strFrame+='        <td colspan=3 align=right class=calendarhead onclick=parent.wpCalendar.close() style="font-size:12px;cursor: pointer;padding-right:5px;" >';
			...
			strFrame+='             align=center><input type=button value="今 天" onclick="parent.wpCalendar.selectToday()" ';
	...
	this.dateCellClick=function (n){  //点击显示框选取日期，主输入函数*************
		if (this.tgtObject){
			...
			this.close(); 
			this.tgtObject.focus();//modified by 2012-12-31 zk 选取日期后带回焦点
		}
