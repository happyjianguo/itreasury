		...
		suggest : function(p) {
			...
			return this.each(function(i, n) {
				...
				if(p.type=="textarea")
				{
					...
					$(this).parent().after('<td><a class="textareaaction" id="magnifier'+'_'+p.id+'" name="magnifier'+'_'+p.id+'"/></td>');
				...
				}else
				{
					...
					$(this).parent().parent().wrap('<table cellpadding="0" cellspacing="0" class="tab_search"></table>');
					$(this).parent().after('<td class="control"><a class="searchaction" id="magnifier'+'_'+p.id+'"  name="magnifier'+'_'+p.id+'"/></td>');
				}
				...
				if ($(this).attr("disabled") != true
					...
					$(this).change(function(e) {
						...
						$.each(p.elements, function(i, element) {
							$("input[type='hidden'][name='"+ element.elName + "']").val("");
						});
					}).closest("table")
					.find('tr > td:last > a[class="searchaction"]')
					.click(function(e) {// 点击放大镜图标
		...
		createPanel : function(p) {
			return this.each(function(i, n) {
				...
				var _p = {
					...
					clazzs : (function(colModel) {
						var clas = new Array();
						$.each(colModel, function() {
									clas.push(this.clazz);
								});
						return clas;
					})(p.colModel),	
					methods : (function(colModel) {
						var mets = new Array();
						$.each(colModel, function() {
									mets.push(this.method);
								});
						return mets;
					})(p.colModel),	
					paramTypes : (function(colModel) {
						var pts = new Array();
						$.each(colModel, function() {
									pts.push(this.paramType);
								});
						return pts;
					})(p.colModel),	
					paramValues : (function(colModel) {
						var pvs = new Array();
						$.each(colModel, function() {
									pvs.push(this.paramValue);
								});
						return pvs;
					})(p.colModel),	
					sql : _sql,// sql语句
