...
* 33。NumValid 控件允许输入的最大数值校验
* 34.trimFormValue 去除表单中文本框内的前后空格
* 
*/
...
* dtEnd 结束日期栏位名称
* str 提示信息
* 该方法用于比较只有月日的日期类型的大小  马现福添加
*/
function CompareMonthDay(dtBegin,dtEnd,str)
{
	var begin,end;
	begin=new String(dtBegin.value);
	end=new String(dtEnd.value);
	var b = begin.split("-");
	var e = end.split("-"); 
	if(b[0].valueOf()>e[0].valueOf())
	{
		alert(str);
		dtBegin.focus();
		return false;
	}
	else if(b[1].valueOf()>e[1].valueOf()){
		alert(str);
		dtBegin.focus();
		return false;
	}
...
function checkAmount(d_input,d_notnull,d_str)
{
    ...
    if(d_notnull!=0)//不判断最小值
    {
    	if  ( ((d_notnull==1) || (d_input.value!=""))  && reverseFormatAmount1(d_input.value) <= 0 )
    	{
   ...
   if  ( ((d_input.value!="")) && reverseFormatAmount1(d_input.value) > 1000000000000 )
		{
			alert(d_str+ "的值不能大于1,000,000,000,000" );
			d_input.focus();
			return (false);
		}
		return true;
 ...
 * @param d_str  域的名称，比如“客户编号”，供提示信息使用
 */
function checkRisk(d_input,d_notnull,d_str)
{
	return InputValid(d_input,d_notnull,"float",1,0.0,1000000000,d_str);
}
 ...
 * @param d_input  需要校验的form中的域
 * @param d_notnull 如果不能为空，填1，否则为0
 * @param d_low 最小值，为空默认为0
 * @param d_up 最大值，为空默认为100
 * @param d_str  域的名称，比如“客户编号”，供提示信息使用
 */
function checkRate1(d_input,d_notnull,d_low,d_up,d_str)
{
	if((d_low == "") || (d_low < 0))
	{
		if((d_up == "") || (d_up < 0))
		...
		{
			return InputValid(d_input,d_notnull,"float",1,0.0,d_up,d_str);
		}
	}
	else
	{
		return InputValid(d_input,d_notnull,"float",1,d_low,d_up,d_str);
	}
}
 ...
 * @param d_str  域的名称，比如“客户编号”，供提示信息使用
 */
function checkDateNOtWithFocus(d_input,d_notnull,d_str)
{
	if((d_input.value=="") && (d_notnull==0))
	{
		return true;
...
function InputValid_A( d_input,d_notnull, d_type,d_limited, d_low, d_up,d_str )
{
  ...
  if(d_input !=null && d_input != "undefined" ){
     isDisabled = d_input.getAttribute("disabled");
  }else{
  	alert("无效的控件");
  	return false;
  }   
...
// "float"
	if (d_type=="float")
	{
		...
		var tempFvalue = reverseFormatAmount(d_input.value);
	    //非空的时候也不能填0
	    if  ( d_notnull==1 && tempFvalue == 0){
			alert(d_str+ "的值不能等于0" );
	    ...
	    if  ( d_limited==1 && tempFvalue < d_low )
		{
	    ...
	    if  ( d_limited==1 && tempFvalue > d_up )
		{
 ...
 * @param title 标题
 * @param d_input 控件名称
 * @param nums 控件允许输入的最大数值
 * @return true/false
 * @author liuxz
 */
function NumValid( d_input,nums, title)
{
	var tempvar  = d_input.value;
	if(tempvar==""||!isInt(tempvar)||(isInt(tempvar)&&parseInt(tempvar)<0)){
		alert(title + "不能为空且必须为自然数，请录入");
		d_input.focus();
		return (false);
	} else if ( parseInt(tempvar) > nums)
	{
		alert(title + "不能大于"+nums+"，请录入");
		d_input.focus();
		return (false);
	}
	return (true);
   ...
   /**
	*		添加by jinchen 2004-11-13 字符串去空格方法.
	*/
...
function trimFormValue()
{
	...
	{
	if  ( (document.all[i].tagName == "INPUT" && document.all[i].type == "text" ) || ( document.all[i].tagName == "TEXTAREA" ) )
		{
