...
function all_refresh(){
  //alert(1)
	if (bw.ns4) {oCMenu.fromleft=((document.body.clientWidth-770)>=0)?((document.body.clientWidth-770)/2+79):79}
	else if (bw.ns6) {oCMenu.fromleft=((document.body.clientWidth-770)>=0)?((document.body.clientWidth-770)/2+79):79}
	else {oCMenu.fromleft=((document.body.clientWidth-770)>=0)?((document.body.clientWidth-770)/2+81):81}
	//alert(1)
}

function showMenu(strPriv,strUrl)
{
  //alert("strPriv is :" + strPriv);
  //strPriv = "帐户管理;;帐户余额;;帐户历史明细;;系统管理;;票据状态查询;;文件下载;;交易类型设置;;签认金额设置::6-1;;6-1-100;;6-1-101;;6-4;;6-4-100;;6-4-101;;6-4-102;;6-4-103::1;;2;;2;;1;;2;;2;;2;;2::;;accountinfo/main.jsp;;accountinfo/LiShi.jsp;;;;system/S810-Ipt.jsp;;system/S850-Ipt.jsp;;system/S840-Ipt.jsp;;system/S844-Ctr.jsp";
...
oCMenu.resizecode="Javascript:all_refresh();"


oCMenu.onlineUrl="/homeresource.nsf/AssetsByDescription/IntranetNavigation/$file/DropDownData.js/";
if (bw.ns4) {oCMenu.fromleft=(document.body.clientWidth-770)/2+79}
else if (bw.ns6) {oCMenu.fromleft=(document.body.clientWidth-770)/2+79}
else {oCMenu.fromleft=(document.body.clientWidth-770)/2+81}
if (bw.ns4) {oCMenu.fromtop=62}
else if (bw.ns6) {oCMenu.fromtop=67}
else {oCMenu.fromtop=65}
oCMenu.level[0]=new Array() 
oCMenu.level[0].width=120
oCMenu.level[0].height=20
oCMenu.level[0].bgcoloroff='#E39135'
oCMenu.level[0].bgcoloron='#CD3301'
oCMenu.level[0].textcolor='#FFFFFF'
oCMenu.level[0].hovercolor='#FFFFFF'
oCMenu.level[0].style='padding:4px;font-family:Arial,Helvetica;font-size:12px; text-align:center'
...
oCMenu.level[1].bgcoloron='#F1F1F1'
...
oCMenu.level[2].bgcoloron='#F1F1F1'
oCMenu.level[2].textcolor='#666666'
oCMenu.level[2].hovercolor='#666666'
oCMenu.level[2].style='padding:4px;font-family:Arial,Helvetica;font-size:11px; '
...
if (arrDepart.length==4){
  ...
  for (j=0;j<iLength;j++)
  {
       ...
       if((sTmp.indexOf("-",3) == -1)&&(sTmp.indexOf("-") > -1))
       {
         ...
         i=0;
         sParent[i] = sTmp;
         oCMenu.makeMenu(sTmp,'', CenterNS(arrName[j],'110'),sTmpUrl,'','90','21','','','','','#FFFFFF','','','status="";','status="";');

         //oCMenu.makeMenu(sTmp,'', CenterNS(arrName[j],'110'),sTmpUrl,'','110','20','','','','','#666666','','','status="";','status="";');
         continue;
...
*/         
         ...
         if(sTmp.indexOf(sParent[i])>-1){
           ...
           {
             oCMenu.makeMenu(sTmp,sParent[i-1],'<img src="/webob/graphics/arrow_a.gif"  align="right" name="tt">'+arrName[j],"",'','200','22','','','','','','','','status="";','status="";');
            }
           }
...
*/
             ...
             {
               oCMenu.makeMenu(sTmp,sParent[i-1],'<img src="/webob/graphics/arrow_a.gif" name="tt" align="right">'+arrName[j],"",'','200','22','','','','','','','','status="";','status="";');
             }
