...
var gSubmitFunction=""; 
var gGetNextFieldFunction="";
var gFormName = "";
////xrzhang添加,第一个获得焦点的控件名称，作为在调用executeSubmitFunction时转移焦点，防止重复回车多次保存记录
var gFirstFocusCtrlName="";

function setGetNextFieldFunction(strFunction)
{
	gGetNextFieldFunction=strFunction;
}  
	...
	// determines whether Netscape or Internet Explorer
		...
		{ // enter key pressed
			if(gnIsSelectCtrl==0)
			{
			    ...
			    else if (nextfield == 'getNextField')
			    {
			      	 executeGetNextFieldFunction();
			      	 return false;
			    }
				else
...
function executeGetNextFieldFunction()
{
	eval(gGetNextFieldFunction);
}
...
function createAmountCtrl_standard(strFormName,strCtrlName,isCanInputNegative,strData,strNextCtrlName,strChineseCtrl,nCurrencyID,sProperty,strSelfOnChange){
 		...
 		document.writeln(" <input class=box type='text' name='"+ strCtrlName + "' value='" + formatAmount( strData ,nCurrencyID ) +"' size='20' maxlength='" + maxlength + "' onKeyPress = 'event.returnValue=IsDigit(" + isCanInputNegative + ");' onchange = 'onAmountChangeEven(this," + isCanInputNegative + "," + intDigit + "," + decimalFractionDigit + ");" + strSelfOnChange + ";'  onblur='adjustAmount(\"" + strFormName +"\",\"" + strCtrlName +"\",1,\"" + strChineseCtrl +"\"," + nCurrencyID+")'  onfocus='adjustAmount(\"" + strFormName +"\",\"" + strCtrlName +"\",2,\"" + strChineseCtrl +"\"," + nCurrencyID + ")' " + sProperty + "><input type='hidden' name='" + strCtrlName + "NextCtrlName' value='" + strNextCtrlName + "'>");
}
...
function onAmountChangeEven(amountObj,isCanInputNegative,intDigit,decimalFractionDigit){
	...
	if(amountObj.value.length == 0) return;
	...
	if (window.event.keyCode!=13)//
	{
	 	...
	 	if (isInnerFloat(strOldData))//是否是浮点数
	 	{
			...
			else
			{
    	 			if (strOldData=="0.00" )
    	 			{
	    	 		  	//modified by mzh_fu 2007/03/23 解决了将金额小于0.01时显为""的问题
	   	 				//strNewData = "";
	   	 		  		strNewData = "0.00";
    	 			}
    	 			else
 ...
 */
	...
	function showChinese(sAmount1,nCurrencyID)
	{
		...
		{
			//modify by wjliu --2007/3/6在金额转换为中文之前先反格式化
			//var sAmount=fntrimzero(sAmount1);
			var sAmount=fntrimzero( reverseFormatAmount( sAmount1 ) );
			//modify --end
			...
			if (nPoint != -1) 
			{
				...
				smid=sAmount.substring(nPoint-1,nPoint);
 ...
 * 创建办事处账户控件控件
 * @param strFormName 控件所在form的名称
 * @param strCtrlName 控件名称，这个名称在整个form里是不能重复的,其value是记录办事处的账号，不记录ID！
 ...
 */
 ...
 function showRecordBeFalsifiedPrompt(){
 	document.writeln("<div id='blnBeFalsified_sending' style='filter:alpha(opacity=20);position:absolute; top:320; left:20; z-index:10;visibility:visible '>");
 	document.writeln("<TABLE WIDTH=100% BORDER=0  CELLSPACING=0 CELLPADDING=0>");
	document.writeln("<TR><td width=30%></td><TD>");
	document.writeln("<TABLE WIDTH=100% height=70 BORDER=1 bordercolor=red CELLSPACING=0 CELLPADDING=0>");
	document.writeln("<TR>");
	document.writeln("<td height='100%'  align=center><font color=red size=4><B>警告：<br>该笔业务已被人非法修改过！</B></font></td>");
	document.writeln("</tr>");
 ...
 function mouseover()
 {
 	try
 	{
 		if(!$("#_Popup_help")[0])
 		{
	 		...
	 		showHelper("#"+id,"警告","    该笔业务已被非法修改!",50,e.clientX,e.clientY);
 		}
 	}
  	catch(e)
 	{
