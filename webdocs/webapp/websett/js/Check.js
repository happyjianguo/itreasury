...
* 34.isMoneyString 校验金额
* 35.limitLength 检验并限制输入的字符数
* 36.CompareDateString_nhcw开始时间和结束时间的匹配校验
*/
...
function checkNegotiateRate(d_input,d_notnull,d_str)
...
/**
 * 在字符串后有零到多个回车的情况下 判断它是否为金额
 * @param strData 需要做判断的数据
 * @return 是，返回true; 否，返回false.
 */ 
function isMoneyString(strData){
	var patrn=/^((([1-9]\d*)|0)\.\d{1,6}?|\r*)$/;
	if (!patrn.exec(strData)){ return false;}
	return true;
}
...
//检验并限制输入的字符数（一个字母为一个字节，一个汉字为两个字节）
 ...
 /**
  * 开始时间和结束时间的匹配校验
  * @parm strBeginDate 开始时间
  * @parm strEndDate  结束时间
  * return true 如果 strBeginDate 晚于 strEndDate 否则false
  * @author Kloud
  */
  function CompareDateString_nhcw(strBeginDate,strEndDate)
 {
 	var temp,s;
 	temp=new String(strBeginDate);
 	if(chkdate(temp)==0)
