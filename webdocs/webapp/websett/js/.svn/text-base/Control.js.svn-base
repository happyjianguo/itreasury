...
* 以下为功能块调用控件(放大镜)
* 1. createClientCtrl,setClientCtrlClientID,setClientCtrlChiefRepCode,setClientCtrlClientName,
     setClientCtrlClientNo,setClientCtrlCleanUp,showClientCtrl,isSelectClient,outputClientCtrl 
     为客户控件用到的函数，具体说明见各个方法前面
* 2. getAccountCtrlFieldStatus,setAccountCtrlFieldStatus,fieldFocus,createAccountCtrl,setAccountCtrlAccountNo,
     setAccountCtrlClientNo,setAccountCtrlClientName,setAccountCtrlClientID,setAccountCtrlAccountID,
     setAccountCtrlCleanUp,isSelectAccount,showAccountCtrl,outputAccountCtrl
     为账户控件用到的函数，具体说明见各个方法前面
* 3. createContractCtrl,createContractCtrlScope,showContractCtrl,isSelectContract,outputContractCtrl
     为合同控件(结算使用)用到的函数，具体说明见各个方法前面
* 4. createBranchCtrl,createTypeBranchCtrl,showBranchCtrl,isSelectBranch,setBranchCtrlBranchAccountCode,
     outputBranchCtrl  为开户行控件用到的函数，具体说明见各个方法前面
* 5. createGeneralLedgerCtrl,showGeneralLedgerCtrl,isSelectGeneralLedger,outputGeneralLedgerCtrl
     为总帐控件用到的函数，具体说明见各个方法前面
* 6. createAbstractCtrl,showAbstractCtrl,isSelectAbstract,outputAbstractCtrl 
     为摘要控件用到的函数，具体说明见各个方法前面
* 7. createSubjectNoCtrl,showSubjectNoCtrl,isSelectSubjectNo,outputSubjectNoCtrl 
     为科目号控件用到的函数，具体说明见各个方法前面
* 8. createAdjustReasonCtrl,showAdjustReasonCtrl,isSelectAdjustReason,outputAdjustReasonCtrl 
     为调整原因控件用到的函数，具体说明见各个方法前面
* 9. createFixedDepositCtrl,showFixedDepositCtrl,isSelectFixedDeposit,outputFixedDepositCtrl,
     setFixedDepositDateEnd  为定期存款单据号控件用到的函数，具体说明见各个方法前面
* 10.createLetoutRequisitionCtrl,showLetoutRequisitionCtrl,isSelectLetoutRequisition,outputLetoutRequisitionCtrl 
     为放款通知单控件用到的函数，具体说明见各个方法前面
* 11.createDiscountCredenceCtrl,showDiscountCredenceCtrl,isSelectDiscountCredence,outputDiscountCredenceCtrl 
     为贴现凭证编号控件用到的函数，具体说明见各个方法前面
* 12.createLetoutRequisitionCtrl1,setLetoutRequisitionBalance,showLetoutRequisitionCtrl1,isSelectLetoutRequisition1, 
     outputLetoutRequisitionCtrl1  为放款通知单-信贷控件用到的函数，具体说明见各个方法前面
* 13.createExtendApplyNewCtrl,showExtendApplyNewCtrl,isSelectExtendApplyNew,outputExtendApplyNewCtrl 
     为展期申请控件用到的函数，具体说明见各个方法前面
* 14.createTableNameCtrl,showTableNameCtrl,isSelectTableName,outputTableNameCtrl 创建数据库表控件
* 15.createLoanDueBillCtrl,showLoanDueBillCtrl,isSelectLoanDueBill,setLoanDueBillCtrlDateStart,
     setLoanDueBillCtrlDateEnd,outputLoanDueBillCtrl 
     为贷款借据号控件用到的函数，具体说明见各个方法前面
* 16.createNotifyAccountCtrl,createNotifyAccountCtrlNoAmount,showNotifyAccountCtrlNoAmount,isSelectNotifyAccountNoAmount,
     showNotifyAccountCtrl,isSelectNotifyAccount,setNotifyAccountCtrlBalance,setNotifyAccountCtrlInterestDate,
     outputNotifyAccountCtrl 为通知存款单据号控件用到的函数，具体说明见各个方法前面
* 17.createOutOfCPFBankCtrl,showOutOfCPFBankCtrl,isSelectOutOfCPFBank,outputOutOfCPFBankCtrl 
     为非中油客户开户行控件用到的函数，具体说明见各个方法前面
* 18.createMultiCodeCtrl,showMultiCodeCtrl,isSelectMultiCode,outputMultiCodeCtrl 
     为多维码控件用到的函数，具体说明见各个方法前面
* 19.createOutOfCPFAccountCtrl,showOutOfCPFAccountCtrl,isSelectOutOfCPFAccount,setOutOfCPFAccountClientName,
     setOutOfCPFAccountProvince,setOutOfCPFAccountCity,setOutOfCPFAccountBank,outputOutOfCPFAccountCtrl
     为非中油客户帐户控件用到的函数，具体说明见各个方法前面

  20-26 为金额控件用到的函数
* 20.createAmountCtrl,createAmountCtrl2,createAmountCtrl3,createAmountCtrl1,setAmount 
     金额控件，根据需要参数列表不同，具体说明见各个方法前面
* 21.formatAmount,formatAmount1 格式化金额 
* 22.floatRound 格式化金额(四舍五入)
* 23.formatAmountUp 调用 floatRound 格式化金额
* 24.reverseFormatAmount 反向格式化金额
* 25.isInnerFloat 是否是float型
* 26.adjustAmount,adjustAmount1 调整金额,金额控件在获得或失去焦点时调用

* 27.createLoanInfoCtrl,showLoanInfoCtrl,isSelectLoanInfo,outputLoanInfoCtrl 
     为申请书控件用到的函数，具体说明见各个方法前面
* 28.createLoanInfoCtrl1,showLoanInfoCtrl1,isSelectLoanInfo1,outputLoanInfoCtrl1 
     为申请书控件(报表和查询专用)用到的函数，具体说明见各个方法前面
* 29.createContractInfoCtrl,showContractInfoCtrl,isSelectContractInfo,outputContractInfoCtrl 
     为合同控件(信贷)用到的函数，具体说明见各个方法前面
* 30.createContractInfoCtrl1,showContractInfoCtrl1,isSelectContractInfo1,outputContractInfoCtrl1 
     为合同控件(信贷 带状态)用到的函数，具体说明见各个方法前面
* 31.createContractInfoCtrl2,showContractInfoCtrl2,isSelectContractInfo2,outputContractInfoCtrl2 
     为合同控件(信贷 报表及查询用)用到的函数，具体说明见各个方法前面
* 32.createContractInfoCtrl3,showContractInfoCtrl3,isSelectContractInfo3,outputContractInfoCtrl3 
     为合同控件（展期用，可根据贷款类型动态显示客户号）用到的函数，具体说明见各个方法前面
* 33.createConsignContractCtrl,showConsignContractCtrl,isSelectConsignContract,outputConsignContractCtrl 
     为委托合同控件用到的函数，具体说明见各个方法前面
* 34.createConsignContractCtrl1,showConsignContractCtrl1,isSelectConsignContract1,outputConsignContractCtrl1 
     为委托合同控件（带状态）用到的函数，具体说明见各个方法前面
* 35.createConsignContractCtrl2,showConsignContractCtrl2,isSelectConsignContract2,outputConsignContractCtrl2 
     为委托合同控件（带状态 包括流动和固定）用到的函数，具体说明见各个方法前面
* 36.show_calendar,show_yearly_calendar 时间控件
* 37.createUserNameCtrl,showUserNameCtrl,isSelectUserName,outputUserNameCtrl 
     为用户名称控件（设置页面使用）用到的函数，具体说明见各个方法前面
* 38.createUserNameCtrl1,showUserNameCtrl1,isSelectUserName1,outputUserNameCtrl1 
     为用户名称控件（普通页面使用）用到的函数，具体说明见各个方法前面
* 39.createUserNameCtrl2,showUserNameCtrl2,isSelectUserName2,outputUserNameCtrl2
     为用户名称控件（普通页面使用,根据模块显示用户）用到的函数，具体说明见各个方法前面
* 40.createUserNameCtrl3,showUserNameCtrl3,isSelectUserName3,outputUserNameCtrl3 
     为用户名称控件（报表信贷-21使用，调整过贷款利率的经办员）用到的函数，具体说明见各个方法前面
* 41.createLoginCtrl,showLoginCtrl,isSelectLogin,outputLoginCtrl 
     为登录名称控件用到的函数，具体说明见各个方法前面
* 42.createGroupNameCtrl,showGroupNameCtrl,isSelectGroupName,outputGroupNameCtrl 
     为用户组控件用到的函数，具体说明见各个方法前面
* 43.createRatePlanCtrl,showRatePlanCtrl,isSelectRatePlan,outputRatePlanCtrl 
     为利率计划控件用到的函数，具体说明见各个方法前面
* 44.createRateCtrl,showRateCtrl,isSelectRate,outputRateCtrl 
     为利率控件用到的函数，具体说明见各个方法前面
* 45.createClientTypeNoCtrl,showClientTypeNoCtrl,isSelectClientTypeNo,outputClientTypeNoCtrl,
     setClientTypeName   为客户分类编号控件用到的函数，具体说明见各个方法前面
* 46.createClientTypeNameCtrl,showClientTypeNameCtrl,isSelectClientTypeName,outputClientTypeNameCtrl 
     为客户分类名称控件用到的函数，具体说明见各个方法前面
* 47.createLiborRateCtrl,showLiborRateCtrl,isSelectLiborRate,outputLiborRateCtrl,setLiborCurrencyID 
     为Libor利率控件用到的函数，具体说明见各个方法前面
* 48.createRateCodeCtrl,setRateCodeRateName,showRateCodeCtrl,isSelectRateCode,outputRateCodeCtrl 
     为利率编号控件用到的函数，具体说明见各个方法前面
* 49.createRateCodeCtrl2,showRateCodeCtrl2,isSelectRateCode2,outputRateCodeCtrl2 
     为利率编号控件(显示利率值)用到的函数，具体说明见各个方法前面
* 50.createRateCodeCtrl1,setRateCodeRateName1,showRateCodeCtrl1,isSelectRateCode1,outputRateCodeCtrl1 
     为利率编号控件(现在及以后)用到的函数，具体说明见各个方法前面
* 51.createRateCodeCtrl3,setRateCodeRateName3,setRateCodeRateValidate,setRateCodeRate,showRateCodeCtrl3
     isSelectRateCode3,outputRateCodeCtrl3
     为利率编号控件(输出利率生效日)用到的函数，具体说明见各个方法前面
* 52.createClientNameCtrl,showClientNameCtrl,isSelectClientName,outputClientNameCtrl 
     为客户名称控件用到的函数，具体说明见各个方法前面
* 53.createClientNameCtrl1,(showRateNameCtrl,isSelectRateName,outputRateNameCtrl 借用createClientNameCtrl )
     为客户名称控件(长input)用到的函数，具体说明见各个方法前面
* 54.createExtendApplyCtrl,showExtendApplyCtrl,isSelectExtendApply,outputExtendApplyCtrl 
     为展期申请控件用到的函数，具体说明见各个方法前面
* 55.createClientAccountCtrl,showClientAccountCtrl,isSelectClientAccount,outputClientAccountCtrl 
     为客户表中的账户号控件用到的函数，具体说明见各个方法前面
* 56.createDiscountCtrl,showDiscountCtrl,isSelectDiscount,outputDiscountCtrl,setDiscountClientName,
     createDiscountCtrl1,showDiscountCtrl1,isSelectDiscount1,outputDiscountCtrl1,setDiscountClientName
     为贴现控件用到的函数，具体说明见各个方法前面
* 57.createCreditNameCtrl,showCreditNameCtrl,isSelectCreditName,outputCreditNameCtrl 
     为债券品种控件用到的函数，具体说明见各个方法前面
* 58.createCreditCodeCtrl,showCreditCodeCtrl,isSelectCreditCode,outputCreditCodeCtrl,setCreditTypeName, 
     setCreditTypeNo     为债券代码控件用到的函数，具体说明见各个方法前面
* 59.formatIntereRate,formatIntereRate6 利率显示控件
* 60.createInvestmentCodeCtrl,showInvestmentCodeCtrl,isSelectInvestmentCode,outputInvestmentCodeCtrl 
     为交易号控件用到的函数，具体说明见各个方法前面
* 61.showDisableAccountCtrl     显示不可用的账户控件
* 62.showChinese     显示中文金额
* 63.isLeftHaveSingle     左值个位是否有值
* 64.getLeftOfPoint     得到金额的左值
* 65.getThousandChineseData 得到千以下数据的中文
* 66.getRightOfPoint 得到小数点右边的部分
* 67.createLiborRateCodeCtrl,showLiborRateCodeCtrl,isSelectLiborRateCode,outputLiborRateCodeCtrl,
     setLiborCurrencyID,setLiborName 
     为Libor利率编号控件用到的函数，具体说明见各个方法前面
* 68.createOfficeAccountCtrl1,showOfficeAccountCtrl1,isSelectOfficeAccount1
     与客户放大镜关联的客户账号控件
* 69.createOfficeAccountCtrl,showOfficeAccountCtrl,isSelectOfficeAccount,outputOfficeAccountCtrl
     为办事处账户控件用到的函数，具体说明见各个方法前面
* 70.createStatusLoanInfoCtrl,showStatusLoanInfoCtrl,isSelectStatusLoanInfo,outputStatusLoanInfoCtrl 
     为申请书控件(带状态)用到的函数，具体说明见各个方法前面
* 71.createCashFlowCtrl,showCashFlowCtrl,isSelectCashFlow,outputCashFlowCtrl 
     为现金流向控件用到的函数，具体说明见各个方法前面(注意！：这个控件命名不规范，标识是控件名称+"ID"，文本框是控件名称)
* 72.createFixedRateCtrl,showFixedRateCtrl,isSelectFixedRate,outputFixedRateCtrl 
     为利率控件用到的函数，具体说明见各个方法前面
* 73.createAheadRepayRequisitionCtrl,showAheadRepayRequisitionCtrl,isSelectAheadRepayRequisition,
     setAheadRepayRequisitionCtrlAmount,setAheadRepayRequisitionCtrlInterestDate,
     outputAheadRepayRequisitionCtrl      为提前还款通知单用到的函数，具体说明见各个方法前面
* 74.createDiscountBillCtrl,showDiscountBillCtrl,isSelectDiscountBill,setDiscountBillCtrlAmount,
     setDiscountBillCtrlStartDate,setDiscountBillCtrlEndDate,outputDiscountBillCtrl
     为贴现票据控件用到的函数，具体说明见各个方法前面
* 75.appendZero,adjustNumber,resumeNumber,formatNumber 为数字操作控制函数，具体说明见各个方法前面
* 76.createNumberCtrl,CreateMutiAmountCtrl,createMutiAmountBaseFun 创建各种接口功能函数，具体见各个方法前面
* 77.doWindowClose关闭窗口
* 
*/
...
function createClientCtrl(strFormName,strCtrlName,lClientID,strClientNo,lOfficeID,strTitle,strNextCtrlName,strFirstTD,strSecondTD)
{
		...
		document.writeln("<td " + strFirstTD + "> " + strTitle + "：<a href='javascript:showClientCtrl(\"" + strFormName + "\",\"" + strCtrlName + "\",\"" + lOfficeID + "\",\"" + strNextCtrlName + "\")'><img border= '0' src='/websett/image/icon.gif' width='15' height='15'></a> "
				+ "  "
 ...
 * @param strSecondTD 第二个TD里面的参数
 */
function createAccountNOCtrl(strFormName,strCtrlName,lAccountID,strAccountNo,lOfficeID,strTitle,strNextCtrlName,strFirstTD,strSecondTD)
{
		if (strFirstTD=="" || strFirstTD==null)
		{
				strFirstTD="valign='middle' height='20' width='33%'";
 ...
 * @param lAccountGroupTypeID 帐户组类型标识
 * @param lGetOrPay 付款或者收款
 * @param strFirstTD 第一个TD里面的参数
 * @param strSecondTD 第二个TD里面的参数
 */
function createAccountCtrl(strFormName,strCtrlName,lAccountID,strAccountNo,lOfficeID,lCurrencyID,strTitle,objClientID,lAccountGroupTypeID,strNextCtrlName,lGetOrPay,strFirstTD,strSecondTD)
{
		if (strFirstTD=="" || strFirstTD==null)
		{
				strFirstTD="valign='middle' height='20' width='33%'";
...
//				eval("strAccountNo" + nNum + "=" + strAccountNo.substring(nStart,i-1));
	...
	document.writeln("<td " + strFirstTD + ">" + strTitle + "：<a href='javascript:if(" + strFormName + "." + strCtrlName +"Ctrl.disabled == false) {showAccountCtrl(\"" + strFormName + "\",\"" + strCtrlName+ "\",\"" + lOfficeID+ "\",\"" + lCurrencyID+ "\",\"" + lAccountGroupTypeID + "\",\"" + lGetOrPay+ "\",\"" + strNextCtrlName + "\");}'><img border= '0' src='/websett/image/icon.gif' width='15' height='15'></a> "
			+ "  "
	...
	if(bflag == true)
	{
		document.writeln("  <input type='text' name='" + strCtrlName + "zhh3' size='4' maxlength='4' class='box'  onkeydown='javascript:fieldFocus(\"" + strFormName + "\",\"" + strCtrlName+ "\",\"" + strCtrlName +"zhh4\",\""  + strCtrlName +"zhh3\",\"" + strCtrlName +"zhh2\",1)'  value='" + strAccountNo3 + "'>"
			+ "  - "
			+ "  <input type='text' name='" + strCtrlName + "zhh4' size='1' maxlength='1' class='box'  value='" + strAccountNo4 + "' onkeydown='javascript:isSelectAccount(\"" + strFormName + "\",\"" + strCtrlName+ "\",\"" + lOfficeID+ "\",\"" + lCurrencyID+ "\",\"" + lAccountGroupTypeID+ "\",\"" + lGetOrPay + "\",\"" + strNextCtrlName + "\",\""  + strCtrlName +"zhh4\",\"" + strCtrlName +"zhh3\")'>");
	}
	else
	{
		document.writeln("  <input type='text' name='" + strCtrlName + "zhh3' size='4' maxlength='4' class='box' value='" + strAccountNo3 + "' onkeydown='javascript:isSelectAccount(\"" + strFormName + "\",\"" + strCtrlName+ "\",\"" + lOfficeID+ "\",\"" + lCurrencyID+ "\",\"" + lAccountGroupTypeID+ "\",\"" + lGetOrPay + "\",\"" + strNextCtrlName + "\",\""  + strCtrlName +"zhh3\",\"" + strCtrlName +"zhh2\")'>");
	}
	document.writeln(" "
 ...
 * @param strNextCtrlName 下一个控件的名称，当关闭了选择窗口的时候，焦点到这个控件
 */
function isSelectAccount(strFormName,strCtrlName,lOfficeID,lCurrencyID,lAccountGroupTypeID,lGetOrPay,strNextCtrlName,currentfield,previousfield)
{
	if(gnIsShowSending==0)
	{
		if (window.event.keyCode==13)//回车说明选择
		{
			...
			showAccountCtrl(strFormName,strCtrlName,lOfficeID,lCurrencyID,lAccountGroupTypeID,lGetOrPay,strNextCtrlName );
		}
		else
		{
 ...
 * @param strNextCtrlName 下一个控件的名称，当关闭了选择窗口的时候，焦点到这个控件
 */
function showAccountCtrl(strFormName,strCtrlName,lOfficeID,lCurrencyID,lAccountGroupTypeID,lGetOrPay,strNextCtrlName)
{
				//第三位前面补0
	...
	var paraState="lClientID=" + lClientID + "&lCurrencyID=" + lCurrencyID + "&strFormName=" + strFormName +"&strCtrlName="+strCtrlName+ "&lAccountGroupTypeID=" + lAccountGroupTypeID + "&lOfficeID=" + lOfficeID + "&strNextCtrlName=" + strNextCtrlName + "&strAccountNo=" + strAccountNo + "&lGetOrPay=" +lGetOrPay;
...
function createContractCtrl(strFormName,strCtrlName,lContractID,strContractNo,lOfficeID,lCurrencyID,strTitle,objClientID,lTransactionTypeID,strNextCtrlName,strFirstTD,strSecondTD)
{
	...
	document.writeln("<td " + strFirstTD + ">" + strTitle + "：<a href='javascript:showContractCtrl(\"" + strFormName + "\",\"" + strCtrlName + "\",\"" + lOfficeID+ "\",\"" + lCurrencyID + "\",\"" + lTransactionTypeID + "\",\"" + strNextCtrlName + "\")'><img border= '0' src='/websett/image/icon.gif' width='15' height='15'></a> "
			+ "  "
...
function createTypeBranchCtrl(strFormName,strCtrlName,lBranchID,strBranch,lOfficeID,strTitle,objAccountID,strNextCtrlName,strType,strFirstTD,strSecondTD,lIsSingleBank)
{
		...
		//add by ming(if(" + strFormName + "." + strCtrlName +"Ctrl.disabled == false) {})
		document.writeln("<td " + strFirstTD + ">" + strTitle + "：<a href='javascript:if(" + strFormName + "." + strCtrlName +"Ctrl.disabled == false) {showBranchCtrl(\"" + strFormName + "\",\"" + strCtrlName + "\",\"" + strType + "\",\"" + lOfficeID +"\",\"" + strNextCtrlName + "\");}'><img border= '0' src='/websett/image/icon.gif' width='15' height='15'></a> "
				+ "  "
...
function createGeneralLedgerCtrl(strFormName,strCtrlName,lGeneralLedgerID,strGeneralLedger,strGeneralLedgerCode,lOfficeID,strTitle,strNextCtrlName,strFirstTD,strSecondTD)
{
		...
		document.writeln("<td " + strFirstTD + ">" + strTitle + "：<a href='javascript:showGeneralLedgerCtrl(\"" + strFormName + "\",\"" + strCtrlName + "\",\"" + lOfficeID +"\",\"" + strNextCtrlName + "\")'><img border= '0' src='/websett/image/icon.gif' width='15' height='15'></a> "
				+ "  "
...
function createAbstractCtrl(strFormName,strCtrlName,strAbstract,strCode,lOfficeID,strTitle,strNextCtrlName,strFirstTD,strSecondTD)
{
		...
		document.writeln("<td " + strFirstTD + ">" + strTitle + "：<a href='javascript:showAbstractCtrl(\"" + strFormName + "\",\"" + strCtrlName + "\",\"" + lOfficeID +"\",\"" + strNextCtrlName + "\")'><img border= '0' src='/websett/image/icon.gif' width='15' height='15'></a> "
				+ "  "
...
function createSubjectNoCtrl(strFormName,strCtrlName,strSubjectNo,strCode,lOfficeID,lCurrencyID,strTitle,strNextCtrlName,strFirstTD,strSecondTD)
{
		...
		document.writeln("<td " + strFirstTD + ">" + strTitle + "：<a href='javascript:showSubjectNoCtrl(\"" + strFormName + "\",\"" + strCtrlName + "\",\"" + lOfficeID + "\",\"" + lCurrencyID +"\",\"" + strNextCtrlName + "\")'><img border= '0' src='/websett/image/icon.gif' width='15' height='15'></a> "
				+ "  "
				+ " <INPUT TYPE='HIDDEN' VALUE='"+strSubjectNo+"' ID='"+strCtrlName+ "' NAME='"+strCtrlName+"'>"
				+ "</td>"
				+ "<td " + strSecondTD + " ><input class=box size='20' type='text' name='" + strCtrlName + "Ctrl' value='" + strSubjectNo + "' onkeydown='javascript:isSelectSubjectNo(\"" + strFormName + "\",\"" + strCtrlName + "\",\"" + lOfficeID + "\",\"" + lCurrencyID + "\",\"" + strNextCtrlName + "\")'>"
				+ " </td>");
}
...
function showSubjectNoCtrl(strFormName,strCtrlName,lOfficeID,lCurrencyID,strNextCtrlName)//收款方客户编号
{
	...
	g_win=window.open( "/NASApp/iTreasury-settlement/jsp/SelectSubjectNo.jsp?"+paraState  , "SelectSubjectNo" , sFeatures , false );
//	alert("3");
	g_win.focus();
}
...
function createAdjustReasonCtrl(strFormName,strCtrlName,strAdjustReason,lOfficeID,strTitle,strNextCtrlName,strFirstTD,strSecondTD)
{
		...
		document.writeln("<td " + strFirstTD + ">" + strTitle + "：<a href='javascript:showAdjustReasonCtrl(\"" + strFormName + "\",\"" + strCtrlName + "\",\"" + lOfficeID +"\",\"" + strNextCtrlName + "\")'><img border= '0' src='/websett/image/icon.gif' width='15' height='15'></a> "
				+ "  "
				+ "</td>"
...
function createFixedDepositCtrl(strFormName,strCtrlName,lFixedDepositID,strFixedDepositNo,lOfficeID,lCurrencyID,strTitle,objClientID,strNextCtrlName,strFirstTD,strSecondTD)
{
		...
		document.writeln("<td " + strFirstTD + ">" + strTitle + "：<a href='javascript:showFixedDepositCtrl(\"" + strFormName + "\",\"" + strCtrlName + "\",\"" + lOfficeID + "\",\"" + lCurrencyID +"\",\"" + strNextCtrlName + "\")'><img border= '0' src='/websett/image/icon.gif' width='15' height='15'></a> "
				+ "  "
...
function createLetoutRequisitionCtrl(strFormName,strCtrlName,lLetoutRequisitionID,strLetoutRequisitionCode,lOfficeID,lCurrencyID,strTitle,objContractID,strNextCtrlName,strFirstTD,strSecondTD)
{
		...
		document.writeln("<td " + strFirstTD + ">" + strTitle + "：<a href='javascript:showLetoutRequisitionCtrl(\"" + strFormName + "\",\"" + strCtrlName + "\",\"" + lOfficeID + "\",\"" + lCurrencyID +"\",\"" + strNextCtrlName + "\")'><img border= '0' src='/websett/image/icon.gif' width='15' height='15'></a> "
				+ "  "
...
function createDiscountCredenceCtrl(strFormName,strCtrlName,lDiscountCredenceID,strDiscountCredenceCode,lOfficeID,lCurrencyID,lIsGrantOrRecede,strTitle,objContractID,strNextCtrlName,strFirstTD,strSecondTD)
{
		...
		document.writeln("<td " + strFirstTD + ">" + strTitle + "：<a href='javascript:showDiscountCredenceCtrl(\"" + strFormName + "\",\"" + strCtrlName + "\",\"" + lOfficeID + "\",\"" + lCurrencyID +"\",\"" + strNextCtrlName + "\",\"" + lIsGrantOrRecede +"\")'><img border= '0' src='/websett/image/icon.gif' width='15' height='15'></a> "
				+ "  "
...
function createLetoutRequisitionCtrl1(strFormName,strCtrlName,lLetoutRequisitionID,strLetoutRequisitionCode,lOfficeID,lCurrencyID,strTitle,objContractID,strNextCtrlName,strFirstTD,strSecondTD)
{
		...
		document.writeln("<td " + strFirstTD + ">" + strTitle + "：<a href='javascript:showLetoutRequisitionCtrl1(\"" + strFormName + "\",\"" + strCtrlName + "\",\"" + lOfficeID + "\",\"" + lCurrencyID +"\",\"" + strNextCtrlName + "\")'><img border= '0' src='/websett/image/icon.gif' width='15' height='15'></a> "
				+ "  "
				+ " <INPUT TYPE='HIDDEN' VALUE='"+lLetoutRequisitionID+"' ID='"+strCtrlName+ "' NAME='"+strCtrlName+"'>"
				+ " <INPUT TYPE='HIDDEN' VALUE='"+strLetoutRequisitionCode+"' ID='"+strCtrlName+ "Code' NAME='"+strCtrlName+"Code'>"
				+ " <input type='hidden' name='" + strCtrlName + "objContractID' value='" + objContractID + "'>"
...
function createExtendApplyNewCtrl(strFormName,strCtrlName,lExtendApplyNewID,strExtendApplyNewCode,lOfficeID,lCurrencyID,strTitle,objContractID,strNextCtrlName,strFirstTD,strSecondTD)
{
		...
		document.writeln("<td " + strFirstTD + ">" + strTitle + "：<a href='javascript:showExtendApplyNewCtrl(\"" + strFormName + "\",\"" + strCtrlName + "\",\"" + lOfficeID + "\",\"" + lCurrencyID +"\",\"" + strNextCtrlName + "\")'><img border= '0' src='/websett/image/icon.gif' width='15' height='15'></a> "
				+ "  "
...
function createTableNameCtrl(strFormName,strCtrlName,strTableName,strTitle,strNextCtrlName,strFirstTD,strSecondTD)
{
		...
		document.writeln("<td " + strFirstTD + ">" + strTitle + "：<a href='javascript:showTableNameCtrl(\"" + strFormName + "\",\"" + strCtrlName + "\",\"" + strNextCtrlName + "\")'><img border= '0' src='/websett/image/icon.gif' width='15' height='15'></a> "
				+ "  "
				+ "</td>"
...
function createLoanDueBillCtrl(strFormName,strCtrlName,lLoanDueBillID,strLoanDueBillCode,lOfficeID,lCurrencyID,strTitle,objContractID,lTransactionTypeID,strNextCtrlName,strFirstTD,strSecondTD,objLoanTypeID,strNextCtrlName1)
{
		...
		document.writeln("<td " + strFirstTD + ">" + strTitle + "：<a href='javascript:showLoanDueBillCtrl(\"" + strFormName + "\",\"" + strCtrlName + "\",\"" + lOfficeID + "\",\"" + lCurrencyID +"\",\"" + lTransactionTypeID +"\",\"" + strNextCtrlName + "\",\"" + strNextCtrlName1 + "\")'><img border= '0' src='/websett/image/icon.gif' width='15' height='15'></a> "
				+ "  "
...
function createNotifyAccountCtrl(strFormName,strCtrlName,lNotifyAccountID,strNotifyAccountCode,lOfficeID,lCurrencyID,strTitle,objAccountID,strNextCtrlName,strFirstTD,strSecondTD)
{
		...
		document.writeln("<td " + strFirstTD + ">" + strTitle + "：<a href='javascript:showNotifyAccountCtrl(\"" + strFormName + "\",\"" + strCtrlName + "\",\"" + lOfficeID + "\",\"" + lCurrencyID +"\",\"" + strNextCtrlName + "\")'><img border= '0' src='/websett/image/icon.gif' width='15' height='15'></a> "
				+ "  "
...
function createNotifyAccountCtrlNoAmount(strFormName,strCtrlName,lNotifyAccountID,strNotifyAccountCode,lOfficeID,lCurrencyID,strTitle,objAccountID,strNextCtrlName,strFirstTD,strSecondTD)
{
		...
		document.writeln("<td " + strFirstTD + ">" + strTitle + "：<a href='javascript:showNotifyAccountCtrlNoAmount(\"" + strFormName + "\",\"" + strCtrlName + "\",\"" + lOfficeID + "\",\"" + lCurrencyID +"\",\"" + strNextCtrlName + "\")'><img border= '0' src='/websett/image/icon.gif' width='15' height='15'></a> "
				+ "  "
				+ " <INPUT TYPE='HIDDEN' VALUE='"+lNotifyAccountID+"' ID='"+strCtrlName+ "' NAME='"+strCtrlName+"'>"
				+ " <INPUT TYPE='HIDDEN' VALUE='"+strNotifyAccountCode+"' ID='"+strCtrlName+ "Code' NAME='"+strCtrlName+"Code'>"
				+ " <input type='hidden' name='" + strCtrlName + "objAccountID' value='" + objAccountID + "'>"
...
function createOutOfCPFBankCtrl(strFormName,strCtrlName,strOutOfCPFBank,lOfficeID,strTitle,strNextCtrlName,strFirstTD,strSecondTD)
{
		...
		document.writeln("<td " + strFirstTD + ">" + strTitle + "：<a href='javascript:if ("+strFormName+"."+strCtrlName+".disabled == false ) {showOutOfCPFBankCtrl(\"" + strFormName + "\",\"" + strCtrlName + "\",\"" + lOfficeID +"\",\"" + strNextCtrlName + "\");}'><img border= '0' src='/websett/image/icon.gif' width='15' height='15'></a> "
				+ "  "
				+ "</td>"
...
function createMultiCodeCtrl(strFormName,strCtrlName,strMultiCode,lOfficeID,strTitle,strNextCtrlName,strFirstTD,strSecondTD)
{
		...
		document.writeln("<td " + strFirstTD + ">" + strTitle + "：<a href='javascript:if ("+strFormName+"."+strCtrlName+".disabled == false ) {showMultiCodeCtrl(\"" + strFormName + "\",\"" + strCtrlName + "\",\"" + lOfficeID +"\",\"" + strNextCtrlName + "\");}'><img border= '0' src='/websett/image/icon.gif' width='15' height='15'></a> "
...
function createOutOfCPFAccountCtrl(strFormName,strCtrlName,strOutOfCPFAccount,lOfficeID,strTitle,strNextCtrlName,strFirstTD,strSecondTD)
{
		...
		document.writeln("<td " + strFirstTD + ">" + strTitle + "：<a href='javascript:if ("+strFormName+"."+strCtrlName+".disabled == false) {showOutOfCPFAccountCtrl(\"" + strFormName + "\",\"" + strCtrlName + "\",\"" + lOfficeID +"\",\"" + strNextCtrlName + "\");}'><img border= '0' src='/websett/image/icon.gif' width='15' height='15'></a> "
				+ "  "
				+ "</td>"
 ...
 */
 ...
 {
 	//modified by mzh_fu(付明正) 解决金额控件的系列问题
	createAmountCtrl_standard(strFormName,strCtrlName,false,strData,strNextCtrlName,strChineseCtrl,nCurrencyID,"","");

	//createAmountCtrl(strFormName,strCtrlName,strData,strNextCtrlName,strChineseCtrl,nCurrencyID,"");
 }
 
 /**
 * 创建金额控件--显示金额小数点后为六位的利率
 * @param strFormName
 * @param strCtrlName 控件名称
 * @param strData 数据
 * @param strNextCtrlName 下一个控件
 ...
 * @param nCurrencyID 币种
 */
 function createAmountCtrlSixRate(strFormName,strCtrlName,strData,strNextCtrlName,strChineseCtrl,nCurrencyID)
 {
 	//modified by mzh_fu(付明正) 解决金额控件的系列问题
	createAmountCtrl_standardSixRate(strFormName,strCtrlName,false,strData,strNextCtrlName,strChineseCtrl,nCurrencyID,"","");
 ...
 */
 ...
 {
	//modified by mzh_fu(付明正) 解决金额控件的系列问题
	createAmountCtrl_standard(strFormName,strCtrlName,false,strData,strNextCtrlName,strChineseCtrl,nCurrencyID,sProperty,"");
 ...
 * @param strFormName
 * @param strCtrlName 控件名称
 * @param isCanInputNegative 是否可以为负数
 * @param strData 数据
 * @param strNextCtrlName 下一个控件
 * @param strChineseCtrl 中文控件
 * @param nCurrencyID 币种
 * @param sProperty 属性
 * @param strSelfOnChange 自己定义onchange事件
 * @Author: mzh_fu(付明正)
 * Date: 2007/06/28
 */
function createAmountCtrl_standard(strFormName,strCtrlName,isCanInputNegative,strData,strNextCtrlName,strChineseCtrl,nCurrencyID,sProperty,strSelfOnChange){
	 	if(isNaN(nCurrencyID)) 
	 	{
	 		nCurrencyID=1;//缺省人民币
	 	} 	
 		...
 		var maxlength = 15;  
 		if(isCanInputNegative==true){
 			maxlength = maxlength + 1;
 		}	
 		var intDigit = 12;
 		var decimalFractionDigit = 2;
 		document.writeln(" <input class='tar_new' type='text' name='"+ strCtrlName + "' value='" + formatAmount( strData ,nCurrencyID ) +"' size='18' maxlength='" + maxlength + "' onKeyPress = 'event.returnValue=IsDigit(" + isCanInputNegative + ");' onchange = 'onAmountChangeEven(this," + isCanInputNegative + "," + intDigit + "," + decimalFractionDigit + ");" + strSelfOnChange + ";'  onblur='adjustAmount(\"" + strFormName +"\",\"" + strCtrlName +"\",1,\"" + strChineseCtrl +"\"," + nCurrencyID+")'  onfocus='adjustAmount(\"" + strFormName +"\",\"" + strCtrlName +"\",2,\"" + strChineseCtrl +"\"," + nCurrencyID + ")' " + sProperty + "><input type='hidden' name='" + strCtrlName + "NextCtrlName' value='" + strNextCtrlName + "'>");

}

function createAmountCtrl_standard_2(strFormName,strCtrlName,isCanInputNegative,strData,strNextCtrlName,strChineseCtrl,nCurrencyID,sProperty,strSelfOnChange,size){
	 	if(isNaN(nCurrencyID)) 
	 	{
	 		nCurrencyID=1;//缺省人民币
	 	} 	
 		...
 		var intDigit = 12;
 		var decimalFractionDigit = 2;
 		document.writeln(" <input class=tar type='text' name='"+ strCtrlName + "' value='" + formatAmount( strData ,nCurrencyID ) +"' size='" + sizelength + " ' maxlength='" + maxlength + "' onKeyPress = 'event.returnValue=IsDigit(" + isCanInputNegative + ");' onchange = 'onAmountChangeEven(this," + isCanInputNegative + "," + intDigit + "," + decimalFractionDigit + ");" + strSelfOnChange + ";'  onblur='adjustAmount(\"" + strFormName +"\",\"" + strCtrlName +"\",1,\"" + strChineseCtrl +"\"," + nCurrencyID+")'  onfocus='adjustAmount(\"" + strFormName +"\",\"" + strCtrlName +"\",2,\"" + strChineseCtrl +"\"," + nCurrencyID + ")' " + sProperty + "><input type='hidden' name='" + strCtrlName + "NextCtrlName' value='" + strNextCtrlName + "'>");
 ...
 * Date: 2007/06/28
 */
function createAmountCtrl_standardSixRate(strFormName,strCtrlName,isCanInputNegative,strData,strNextCtrlName,strChineseCtrl,nCurrencyID,sProperty,strSelfOnChange){
	 	if(isNaN(nCurrencyID)) 
	 	{
	 		nCurrencyID=1;//缺省人民币
	 	} 	
 		...
 		var intDigit = 12;
 		var decimalFractionDigit = 2;
 		document.writeln(" <input class=tar type='text' name='"+ strCtrlName + "' value='" + formatAllAmount( strData ,6 ) +"' size='18' maxlength='" + maxlength + "' onKeyPress = 'event.returnValue=IsDigit(" + isCanInputNegative + ");' onchange = 'onAmountChangeEven(this," + isCanInputNegative + "," + intDigit + "," + decimalFractionDigit + ");" + strSelfOnChange + ";'  onblur='adjustAmount(\"" + strFormName +"\",\"" + strCtrlName +"\",1,\"" + strChineseCtrl +"\"," + nCurrencyID+")'  onfocus='adjustAmount(\"" + strFormName +"\",\"" + strCtrlName +"\",2,\"" + strChineseCtrl +"\"," + nCurrencyID + ")' " + sProperty + "><input type='hidden' name='" + strCtrlName + "NextCtrlName' value='" + strNextCtrlName + "'>");
}
 ...
 * Date: 2007/12/18
 */
function createAllAmountCtrl_standard(strFormName,strCtrlName,isCanInputNegative,strData,strNextCtrlName,strChineseCtrl,nCurrencyID,sProperty,strSelfOnChange,digit){
	 	if(isNaN(nCurrencyID)) 
	 	{
	 		nCurrencyID=1;//缺省人民币
	 	} 	
 		...
 		if(digit!="" || digit!=null)
 		{
 			decimalFractionDigit=digit;
 		}
 		document.writeln(" <input class=tar type='text' name='"+ strCtrlName + "' value='" + formatAllAmount( strData ,decimalFractionDigit ) +"' size='18' maxlength='" + maxlength + "' onKeyPress = 'event.returnValue=IsDigit(" + isCanInputNegative + ");' onchange = 'onAmountChangeEven(this," + isCanInputNegative + "," + intDigit + "," + decimalFractionDigit + ");" + strSelfOnChange + ";'" + sProperty + "><input type='hidden' name='" + strCtrlName + "NextCtrlName' value='" + strNextCtrlName + "'>");
}
 ...
 * Date: 2007/06/28
 */
function createAmountCtrl_standard_1(strFormName,strCtrlName,isCanInputNegative,strData,strNextCtrlName,strChineseCtrl,nCurrencyID,sProperty,strSelfOnChange){
	 	if(isNaN(nCurrencyID)) 
	 	{
	 		nCurrencyID=1;//缺省人民币
	 	} 	
 		...
 		var intDigit = 12;
 		var decimalFractionDigit = 6;
 		document.writeln(" <input class=tar type='text' name='"+ strCtrlName + "' value='" + formatAmount( strData ,nCurrencyID ) +"' size='18' maxlength='" + maxlength + "' onKeyPress = 'event.returnValue=IsDigit(" + isCanInputNegative + ");' onchange = 'onAmountChangeEven(this," + isCanInputNegative + "," + intDigit + "," + decimalFractionDigit + ");" + strSelfOnChange + ";'  onblur='adjustAmount(\"" + strFormName +"\",\"" + strCtrlName +"\",1,\"" + strChineseCtrl +"\"," + nCurrencyID+")'  onfocus='adjustAmount(\"" + strFormName +"\",\"" + strCtrlName +"\",2,\"" + strChineseCtrl +"\"," + nCurrencyID + ")' " + sProperty + "><input type='hidden' name='" + strCtrlName + "NextCtrlName' value='" + strNextCtrlName + "'>");
}
 ...
 * Date: 2007/06/28
 */
function IsDigit(isCanInputNegative) 
{ 
	if(event.keyCode == 46) return true;
	if(event.keyCode == 45 && isCanInputNegative)	return true;
 ...
 * Date: 2007/06/28
 */
function onAmountChangeEven(amountObj,isCanInputNegative,intDigit,decimalFractionDigit){
	amountObj.value = amountObj.value.replace(/\s/gim,"");
	amountObj.value = amountObj.value.replace(/,/g,'');
	
	if(!verifyAmount(amountObj.value,isCanInputNegative,intDigit,decimalFractionDigit)){	
		alert("无效的金额：金额最多只能输入" + intDigit + "位整数，" + decimalFractionDigit + "小数");
		var decimalFractionDigitStr = "";
		for(var i=0;i<decimalFractionDigit;i++) {
			decimalFractionDigitStr += "0";
		}
		amountObj.value = "0." + decimalFractionDigitStr;
		amountObj.select();
	}
}
 ...
 * Date: 2007/06/28
 */
function verifyAmount(strData,isCanInputNegative,intDigit,decimalFractionDigit){

	var tempRegExp = "^(-?\\d{1," + intDigit + "})(\\.\\d{1," + decimalFractionDigit + "})?$";
	var tempRegExp2 = "^(\\d{1," + intDigit + "})(\\.\\d{1," + decimalFractionDigit + "})?$";
	
	var regExp ;
	if(isCanInputNegative){
		//reg = /^(-?\d{1,12})(\.\d{1,2})?$/;
		regExp = new RegExp(tempRegExp);
	}else{
		//reg = /^(\d{1,12})(\.\d{1,2})?$/;
		regExp = new RegExp(tempRegExp2);			
	}
 ...
 * @return 返回格式化的金额
 */
 function formatAmount(strData1)
 {	
 	 //add by wjliu 2007-7-26 把strData类型转为字符串 	 
	 //strData1 = strData1 + "";
	 if(strData1==null)
	 	strData1 = 0;
	 strData1 = strData1 + "";	
	 
 	//added by mzh_fu 2007/07/13 解决金额控件的问题
 	if(strData1 !=null && strData1 !=""){
 		strData1 = strData1.replace(/\s/gim,"");
		strData1 = strData1.replace(/,/g,'');
 	}
	var strData=fntrimzero(strData1);
 ...
 * 格式化金额
 * @param strData 需要格式化的数据
 * @param decimalFractionDigit 小数点保留几位
 * @return 返回格式化的金额
 */
 function formatAllAmount(strData1,decimalFractionDigit)
 {
 	 //add by wjliu 2007-7-26 把strData类型转为字符串
	 strData = strData + "";
 	//added by mzh_fu 2007/07/13 解决金额控件的问题
	...
	var strData=fntrimzero(strData1);
 	if(isInnerFloat(strData))
 	{
 		if(strData!="")
 		{
			...
			//补或者截小数点后面的值，保持两位
	 		if(strEnd.length>decimalFractionDigit)
	 		{
	 			strEnd=strEnd.substring(0,decimalFractionDigit-1);
	 		}
	 		else
	 		{ 
	 			if(strEnd.length<decimalFractionDigit)
	 			{
		 			...
		 			for(m=strEnd.length;m<decimalFractionDigit;m++)
		 			{
		 				strEnd=strEnd+ "0";
		 			}
...
/**
*　检查金额是否大于999999999999.99
*
* @Author: 付明正
* @Date: 2007-03-16
*/
function isRightAmount(amount){
	
	if((parseFloat(amount)).toFixed(2) > 999999999999.99){		
		return false;
	}
	return true;
}
 ...
 */
 function adjustAmount1(strFormName,strCtrlName,nType,strChineseCtrl,nCurrencyID)
 {
	if (window.event.keyCode!=13)//
	{
	 	...
	 	if (isInnerFloat(strOldData))//是否是浮点数
	 	{
	 		//added by mzh_fu 2007-03-16
			if(!isRightAmount(strOldData)){
				alert("输入的金额过大，不能大于 999,999,999,999.99 ！");
				eval(strFormName + "." +strCtrlName + ".value='0.00'");
				eval(strFormName + "." +strCtrlName + ".select()");				
				return ;
			}
...
function createLoanInfoCtrl(strFormName,strCtrlName,lLoanInfoID,strLoanInfoNo,lLoanTypeID,lOfficeID,lCurrencyID,strTitle,strNextCtrlName,strFirstTD,strSecondTD)
{
		...
		document.writeln("<td " + strFirstTD + "> " + strTitle + "：<a href='javascript:showLoanInfoCtrl(\"" + strFormName + "\",\"" + strCtrlName + "\",\"" + lOfficeID + "\",\"" + lCurrencyID  + "\",\"" + lLoanTypeID + "\",\""+ strNextCtrlName + "\")'><img border= '0' src='/websett/image/icon.gif' width='15' height='15'></a> "
				+ "  "
...
function createLoanInfoCtrl1(strFormName,strCtrlName,lLoanInfoID,strLoanInfoNo,objTypeID,lOfficeID,lCurrencyID,strTitle,strNextCtrlName,strFirstTD,strSecondTD)
{
		...
		document.writeln("<td " + strFirstTD + "> " + strTitle + "：<a href='javascript:showLoanInfoCtrl1(\"" + strFormName + "\",\"" + strCtrlName + "\",\"" + lOfficeID + "\",\"" + lCurrencyID  + "\",\""+ strNextCtrlName + "\")'><img border= '0' src='/websett/image/icon.gif' width='15' height='15'></a> "
				+ "  "
				+ " <INPUT TYPE='HIDDEN' VALUE='"+lLoanInfoID+"' ID='"+strCtrlName+ "' NAME='"+strCtrlName+"'>"
				+ " <INPUT TYPE='HIDDEN' VALUE='"+strLoanInfoNo+"' ID='"+strCtrlName+ "Code' NAME='"+strCtrlName+"Code'>"
...
function createContractInfoCtrl(strFormName,strCtrlName,lContractInfoID,strContractInfoNo,lLoanTypeID,lOfficeID,lCurrencyID,strTitle,objClientID,strNextCtrlName,strFirstTD,strSecondTD)
{
		...
		document.writeln("<td " + strFirstTD + "> " + strTitle + "：<a href='javascript:showContractInfoCtrl(\"" + strFormName + "\",\"" + strCtrlName + "\",\"" + lOfficeID + "\",\"" + lCurrencyID  + "\",\"" + lLoanTypeID + "\",\""+ strNextCtrlName + "\")'><img border= '0' src='/websett/image/icon.gif' width='15' height='15'></a> "
				+ "  "
...
function createContractInfoCtrl1(strFormName,strCtrlName,lContractInfoID,strContractInfoNo,lLoanTypeID,lOfficeID,lCurrencyID,lStatusID,strTitle,objClientID,strNextCtrlName,strFirstTD,strSecondTD)
{
		...
		document.writeln("<td " + strFirstTD + "> " + strTitle + "：<a href='javascript:showContractInfoCtrl1(\"" + strFormName + "\",\"" + strCtrlName + "\",\"" + lOfficeID + "\",\"" + lCurrencyID  + "\",\"" + lLoanTypeID + "\",\""+ lStatusID + "\",\""+ strNextCtrlName + "\")'><img border= '0' src='/websett/image/icon.gif' width='15' height='15'></a> "
				+ "  "
				+ " <INPUT TYPE='HIDDEN' VALUE='"+lContractInfoID+"' ID='"+strCtrlName+ "' NAME='"+strCtrlName+"'>"
				+ " <INPUT TYPE='HIDDEN' VALUE='"+strContractInfoNo+"' ID='"+strCtrlName+ "Code' NAME='"+strCtrlName+"Code'>"
				+ " <input type='hidden' name='" + strCtrlName + "objClientID' value='" + objClientID + "'>"
...
function createContractInfoCtrl2(strFormName,strCtrlName,lContractInfoID,strContractInfoNo,lOfficeID,lCurrencyID,strTitle,objClientID,objTypeID,strNextCtrlName,strFirstTD,strSecondTD)
{
		...
		document.writeln("<td " + strFirstTD + "> " + strTitle + "：<a href='javascript:showContractInfoCtrl2(\"" + strFormName + "\",\"" + strCtrlName + "\",\"" + lOfficeID + "\",\"" + lCurrencyID + "\",\""+ strNextCtrlName + "\")'><img border= '0' src='/websett/image/icon.gif' width='15' height='15'></a> "
				+ "  "
				+ " <INPUT TYPE='HIDDEN' VALUE='"+lContractInfoID+"' ID='"+strCtrlName+ "' NAME='"+strCtrlName+"'>"
				+ " <INPUT TYPE='HIDDEN' VALUE='"+strContractInfoNo+"' ID='"+strCtrlName+ "Code' NAME='"+strCtrlName+"Code'>"
				+ " <input type='hidden' name='" + strCtrlName + "objClientID' value='" + objClientID + "'>"
...
function createContractInfoCtrl3(strFormName,strCtrlName,lContractInfoID,strContractInfoNo,lOfficeID,lCurrencyID,lStatusID,strTitle,objClientID,objTypeID,strNextCtrlName,strFirstTD,strSecondTD)
{
		...
		document.writeln("<td " + strFirstTD + "> " + strTitle + "：<a href='javascript:showContractInfoCtrl3(\"" + strFormName + "\",\"" + strCtrlName + "\",\"" + lOfficeID + "\",\"" + lCurrencyID + "\",\""+ lStatusID + "\",\""+ strNextCtrlName + "\")'><img border= '0' src='/websett/image/icon.gif' width='15' height='15'></a> "
				+ "  "
				+ " <INPUT TYPE='HIDDEN' VALUE='"+lContractInfoID+"' ID='"+strCtrlName+ "' NAME='"+strCtrlName+"'>"
				+ " <INPUT TYPE='HIDDEN' VALUE='"+strContractInfoNo+"' ID='"+strCtrlName+ "Code' NAME='"+strCtrlName+"Code'>"
				+ " <input type='hidden' name='" + strCtrlName + "objClientID' value='" + objClientID + "'>"
...
function createConsignContractCtrl(strFormName,strCtrlName,lConsignContractID,strConsignContractNo,lLoanTypeID,lOfficeID,lCurrencyID,objClientID,strTitle,strNextCtrlName,strFirstTD,strSecondTD)
{
		...
		document.writeln("<td " + strFirstTD + "> " + strTitle + "：<a href='javascript:showConsignContractCtrl(\"" + strFormName + "\",\"" + strCtrlName + "\",\"" + lOfficeID + "\",\"" + lCurrencyID  + "\",\"" + lLoanTypeID + "\",\""+ strNextCtrlName + "\")'><img border= '0' src='/websett/image/icon.gif' width='15' height='15'></a> "
				+ "  "
...
function createConsignContractCtrl1(strFormName,strCtrlName,lConsignContractID,strConsignContractNo,lLoanTypeID,lOfficeID,lCurrencyID,lStatusID,objClientID,strTitle,strNextCtrlName,strFirstTD,strSecondTD)
{
		...
		document.writeln("<td " + strFirstTD + "> " + strTitle + "：<a href='javascript:showConsignContractCtrl1(\"" + strFormName + "\",\"" + strCtrlName + "\",\"" + lOfficeID + "\",\"" + lCurrencyID  + "\",\"" + lLoanTypeID  + "\",\"" + lStatusID + "\",\""+ strNextCtrlName + "\")'><img border= '0' src='/websett/image/icon.gif' width='15' height='15'></a> "
				+ "  "
				+ " <INPUT TYPE='HIDDEN' VALUE='"+lConsignContractID+"' ID='"+strCtrlName+ "' NAME='"+strCtrlName+"'>"
				+ " <INPUT TYPE='HIDDEN' VALUE='"+strConsignContractNo+"' ID='"+strCtrlName+ "Code' NAME='"+strCtrlName+"Code'>"
			  + "<input type='hidden' name='" + strCtrlName + "objClientID' value='" + objClientID + "'>"
...
function createConsignContractCtrl2(strFormName,strCtrlName,lConsignContractID,strConsignContractNo,lLoanTypeID,lOfficeID,lCurrencyID,lStatusID,objClientID,strTitle,strNextCtrlName,strFirstTD,strSecondTD)
{
		...
		document.writeln("<td " + strFirstTD + "> " + strTitle + "：<a href='javascript:showConsignContractCtrl2(\"" + strFormName + "\",\"" + strCtrlName + "\",\"" + lOfficeID + "\",\"" + lCurrencyID  + "\",\"" + lLoanTypeID  + "\",\"" + lStatusID + "\",\""+ strNextCtrlName + "\")'><img border= '0' src='/websett/image/icon.gif' width='15' height='15'></a> "
				+ "  "
				+ " <INPUT TYPE='HIDDEN' VALUE='"+lConsignContractID+"' ID='"+strCtrlName+ "' NAME='"+strCtrlName+"'>"
				+ " <INPUT TYPE='HIDDEN' VALUE='"+strConsignContractNo+"' ID='"+strCtrlName+ "Code' NAME='"+strCtrlName+"Code'>"
			  + "<input type='hidden' name='" + strCtrlName + "objClientID' value='" + objClientID + "'>"
...
function createUserNameCtrl(strFormName,strCtrlName,strUserName,strCode,lOfficeID,strTitle,strNextCtrlName,strFirstTD,strSecondTD)
{
		...
		document.writeln("<td " + strFirstTD + ">" + strTitle + "：<a href='javascript:showUserNameCtrl(\"" + strFormName + "\",\"" + strCtrlName + "\",\""+ strUserName + "\",\""  + lOfficeID +"\",\"" + strNextCtrlName + "\")'><img border= '0' src='/websett/image/icon.gif' width='15' height='15'></a> "
				+ "  "
				+ " <INPUT TYPE='HIDDEN' VALUE='"+strCode+"' ID='"+strCtrlName+ "Code' NAME='"+strCtrlName+"Code'>"
				+ "</td>"
...
function createUserNameCtrl1(strFormName,strCtrlName,strUserName,strCode,lOfficeID,strTitle,strNextCtrlName,strFirstTD,strSecondTD)
{
  	if (strFirstTD=="" || strFirstTD==null)
		...
		document.writeln("<td " + strFirstTD + ">" + strTitle + "：<a href='javascript:showUserNameCtrl1(\"" + strFormName + "\",\"" + strCtrlName + "\",\"" + lOfficeID +"\",\"" + strNextCtrlName + "\")'><img border= '0' src='/websett/image/icon.gif' width='15' height='15'></a> "
				+ "  "
				+ " <INPUT TYPE='HIDDEN' VALUE='"+strCode+"' ID='"+strCtrlName+ "Code' NAME='"+strCtrlName+"Code'>"
				+ "</td>"
...
function createUserNameCtrl2(strFormName,strCtrlName,strUserName,strCode,lModuleID,lOfficeID,strTitle,strNextCtrlName,strFirstTD,strSecondTD)
{
  	if (strFirstTD=="" || strFirstTD==null)
		...
		document.writeln("<td " + strFirstTD + ">" + strTitle + "：<a href='javascript:showUserNameCtrl2(\"" + strFormName + "\",\"" + strCtrlName + "\",\"" + lModuleID + "\",\"" + lOfficeID +"\",\"" + strNextCtrlName + "\")'><img border= '0' src='/websett/image/icon.gif' width='15' height='15'></a> "
				+ "  "
				+ " <INPUT TYPE='HIDDEN' VALUE='"+strCode+"' ID='"+strCtrlName+ "Code' NAME='"+strCtrlName+"Code'>"
				+ "</td>"
...
function createUserNameCtrl3(strFormName,strCtrlName,strUserName,strCode,lOfficeID,strTitle,strNextCtrlName,strFirstTD,strSecondTD)
{
  	if (strFirstTD=="" || strFirstTD==null)
		...
		document.writeln("<td " + strFirstTD + ">" + strTitle + "：<a href='javascript:showUserNameCtrl3(\"" + strFormName + "\",\"" + strCtrlName + "\",\"" + lOfficeID +"\",\"" + strNextCtrlName + "\")'><img border= '0' src='/websett/image/icon.gif' width='15' height='15'></a> "
				+ "  "
				+ " <INPUT TYPE='HIDDEN' VALUE='"+strCode+"' ID='"+strCtrlName+ "Code' NAME='"+strCtrlName+"Code'>"
				+ "</td>"
...
function createLoginCtrl(strFormName,strCtrlName,strLogin,strCode,lOfficeID,strTitle,strNextCtrlName,strFirstTD,strSecondTD)
{
		...
		document.writeln("<td " + strFirstTD + ">" + strTitle + "：<a href='javascript:showLoginCtrl(\"" + strFormName + "\",\"" + strCtrlName + "\",\"" + strLogin + "\",\"" + lOfficeID +"\",\"" + strNextCtrlName + "\")'><img border= '0' src='/websett/image/icon.gif' width='15' height='15'></a> "
				+ "  "
				+ " <INPUT TYPE='HIDDEN' VALUE='"+strCode+"' ID='"+strCtrlName+ "Code' NAME='"+strCtrlName+"Code'>"
				+ "</td>"
...
function createGroupNameCtrl(strFormName,strCtrlName,strGroupName,strCode,lOfficeID,strTitle,strNextCtrlName,strFirstTD,strSecondTD,strProperty)
{
		...
		document.writeln("<td " + strFirstTD + ">" + strTitle + "：<a href='javascript:showGroupNameCtrl(\"" + strFormName + "\",\"" + strCtrlName + "\",\"" + strGroupName + "\",\"" + lOfficeID +"\",\"" + strNextCtrlName + "\")'><img border= '0' src='/websett/image/icon.gif' width='15' height='15'></a> "
				+ "  "
				+ " <INPUT TYPE='HIDDEN' VALUE='"+strCode+"' ID='"+strCtrlName+ "Code' NAME='"+strCtrlName+"Code'>"
				+ "</td>"
...
function createRatePlanCtrl(strFormName,strCtrlName,strRatePlan,lRateID,lOfficeID,strTitle,strNextCtrlName,strFirstTD,strSecondTD,nTypeID)
{
		...
		document.writeln("<td " + strFirstTD + ">" + strTitle + "：<a href='javascript:if(" + strFormName + "." + strCtrlName +"Ctrl.disabled == false) {showRatePlanCtrl(\"" + strFormName + "\",\"" + strCtrlName + "\",\"" + lOfficeID +"\",\"" + strNextCtrlName + "\");}'><img border= '0' src='/websett/image/icon.gif' width='15' height='15'></a> "
				+ "  "
 ...
 * @param strSecondTD 第二个TD里面的参数
 */
function createRateCtrl(strFormName,strCtrlName,strRateName,lRateID,lOfficeID,lCurrencyID,strTitle,strNextCtrlName,strFirstTD,strSecondTD)
{
		...
		document.writeln("<td " + strFirstTD + ">" + strTitle + "：<a href='javascript:showRateCtrl(\"" + strFormName + "\",\"" + strCtrlName + "\",\"" + lOfficeID +"\",\"" + lCurrencyID +"\",\"" + strNextCtrlName + "\")'><img border= '0' src='/websett/image/icon.gif' width='15' height='15'></a> "
				+ "  "
				
				+ " <INPUT TYPE='HIDDEN' VALUE='' ID='"+strCtrlName+ "Value' NAME='"+strCtrlName+"Value'>"
				+ "</td>"
				+ "<td " + strSecondTD + " ><input class='box'  type='text' name='" + strCtrlName + "Ctrl' value='" + strRateName + "' onkeydown='javascript:isSelectRate(\"" + strFormName + "\",\"" + strCtrlName + "\",\"" + lOfficeID + "\",\"" + lCurrencyID + "\",\"" + strNextCtrlName + "\")'>"
				+ " </td>");
}
 ...
 * @param strNextCtrlName 下一个控件的名称，当关闭了选择窗口的时候，焦点到这个控件
 */
function showRateCtrl(strFormName,strCtrlName,lOfficeID,lCurrencyID,strNextCtrlName)//收款方客户编号
{
	...
	var paraState="lOfficeID=" + lOfficeID + "&lCurrencyID=" + lCurrencyID + "&strFormName=" + strFormName +"&strCtrlName="+strCtrlName +"&strNextCtrlName=" + strNextCtrlName + "&strName="+ strName;
 ...
 * @param strNextCtrlName 下一个控件的名称，当关闭了选择窗口的时候，焦点到这个控件
 */
function isSelectRate(strFormName,strCtrlName,lOfficeID,lCurrencyID,strNextCtrlName)
{
	if(gnIsShowSending==0)
	{
		if (window.event.keyCode==13)//回车说明选择
		{
			gnIsSelectCtrl=1;
			showRateCtrl(strFormName,strCtrlName,lOfficeID,lCurrencyID,strNextCtrlName);
		}
		else
		{
			eval(strFormName+ "." + strCtrlName + ".value=''");
...
function createClientTypeNoCtrl(strFormName,strCtrlName,strClientTypeNoName,lClientTypeID,lOfficeID,strTitle,strNextCtrlName,strFirstTD,strSecondTD)
{
		...
		document.writeln("<td " + strFirstTD + ">" + strTitle + "：<a href='javascript:showClientTypeNoCtrl(\"" + strFormName + "\",\"" + strCtrlName + "\",\"" + lOfficeID +"\",\"" + strNextCtrlName + "\")'><img border= '0' src='/websett/image/icon.gif' width='15' height='15'></a> "
				+ "  "
...
function createClientTypeNameCtrl(strFormName,strCtrlName,strClientTypeNameName,lClientTypeID,lOfficeID,strTitle,strNextCtrlName,strFirstTD,strSecondTD)
{
		...
		document.writeln("<td " + strFirstTD + ">" + strTitle + "：<a href='javascript:showClientTypeNameCtrl(\"" + strFormName + "\",\"" + strCtrlName + "\",\"" + lOfficeID +"\",\"" + strNextCtrlName + "\")'><img border= '0' src='/websett/image/icon.gif' width='15' height='15'></a> "
				+ "  "
				+ " <INPUT TYPE='HIDDEN' VALUE='"+lClientTypeID+"' ID='"+strCtrlName+ "' NAME='"+strCtrlName+"'>"
				+ "</td>"
...
function createLiborRateCtrl(strFormName,strCtrlName,strLiborRateName,lLiborRateID,lOfficeID,strTitle,strNextCtrlName,strFirstTD,strSecondTD)
{
		...
		document.writeln("<td " + strFirstTD + ">" + strTitle + "：<a href='javascript:showLiborRateCtrl(\"" + strFormName + "\",\"" + strCtrlName + "\",\"" + lOfficeID +"\",\"" + strNextCtrlName + "\")'><img border= '0' src='/websett/image/icon.gif' width='15' height='15'></a> "
				+ "  "
...
function createRateCodeCtrl(strFormName,strCtrlName,strRateCode,lRateID,lCurrencyID,strTitle,strNextCtrlName,strFirstTD,strSecondTD)
{
		...
		document.writeln("<td " + strFirstTD + ">" + strTitle + "：<a href='javascript:showRateCodeCtrl(\"" + strFormName + "\",\"" + strCtrlName + "\",\"" + lCurrencyID +"\",\"" + strNextCtrlName + "\")'><img border= '0' src='/websett/image/icon.gif' width='15' height='15'></a> "
				+ "  "
				+ " <INPUT TYPE='HIDDEN' VALUE='"+lRateID+"' ID='"+strCtrlName+ "' NAME='"+strCtrlName+"'>"
				+ "</td>"
...
function createRateCodeCtrl2(strFormName,strCtrlName,strRateValue,lRateID,lCurrencyID,strTitle,strNextCtrlName,strFirstTD,strSecondTD)
{
		...
		document.writeln("<td " + strFirstTD + ">" + strTitle + "：<a href='javascript:showRateCodeCtrl2(\"" + strFormName + "\",\"" + strCtrlName + "\",\"" + lCurrencyID +"\",\"" + strNextCtrlName + "\")'><img border= '0' src='/websett/image/icon.gif' width='15' height='15'></a> "
				+ "  "
				+ " <INPUT TYPE='HIDDEN' VALUE='"+lRateID+"' ID='"+strCtrlName+ "' NAME='"+strCtrlName+"'>"
				+ "</td>"
...
function createRateCodeCtrl1(strFormName,strCtrlName,strRateCode,lRateID,lCurrencyID,strTitle,strNextCtrlName,strFirstTD,strSecondTD)
{
		...
		document.writeln("<td " + strFirstTD + ">" + strTitle + "：<a href='javascript:showRateCodeCtrl1(\"" + strFormName + "\",\"" + strCtrlName + "\",\"" + lCurrencyID +"\",\"" + strNextCtrlName + "\")'><img border= '0' src='/websett/image/icon.gif' width='15' height='15'></a> "
				+ "  "
				+ " <INPUT TYPE='HIDDEN' VALUE='"+lRateID+"' ID='"+strCtrlName+ "' NAME='"+strCtrlName+"'>"
				+ "</td>"
...
function createRateCodeCtrl3(strFormName,strCtrlName,strRateCode,lRateID,lCurrencyID,strTitle,strNextCtrlName,strFirstTD,strSecondTD)
{
		...
		document.writeln("<td " + strFirstTD + ">" + strTitle + "：<a href='javascript:showRateCodeCtrl3(\"" + strFormName + "\",\"" + strCtrlName + "\",\"" + lCurrencyID +"\",\"" + strNextCtrlName + "\")'><img border= '0' src='/websett/image/icon.gif' width='15' height='15'></a> "
				+ "  "
				+ " <INPUT TYPE='HIDDEN' VALUE='"+lRateID+"' ID='"+strCtrlName+ "' NAME='"+strCtrlName+"'>"
				+ "</td>"
...
function createRateNameCtrl(strFormName,strCtrlName,strRateName,lRateID,lCurrencyID,strTitle,strNextCtrlName,strFirstTD,strSecondTD)
{
		...
		document.writeln("<td " + strFirstTD + ">" + strTitle + "：<a href='javascript:showRateNameCtrl(\"" + strFormName + "\",\"" + strCtrlName + "\",\"" + lCurrencyID +"\",\"" + strNextCtrlName + "\")'><img border= '0' src='/websett/image/icon.gif' width='15' height='15'></a> "
				+ "  "
				+ " <INPUT TYPE='HIDDEN' VALUE='"+lRateID+"' ID='"+strCtrlName+ "' NAME='"+strCtrlName+"'>"
				+ "</td>"
...
function createClientNameCtrl(strFormName,strCtrlName,strClientName,lClientID,lOfficeID,strTitle,strNextCtrlName,strFirstTD,strSecondTD)
{
		...
		document.writeln("<td " + strFirstTD + ">" + strTitle + "：<a href='javascript:showClientNameCtrl(\"" + strFormName + "\",\"" + strCtrlName + "\",\"" + lOfficeID +"\",\"" + strNextCtrlName + "\")'><img border= '0' src='/websett/image/icon.gif' width='15' height='15'></a> "
				+ "  "
				+ " <INPUT TYPE='HIDDEN' VALUE='"+lClientID+"' ID='"+strCtrlName+ "' NAME='"+strCtrlName+"'>"
				+ "</td>"
...
function createExtendApplyCtrl(strFormName,strCtrlName,lExtendApplyID,strExtendApplyNo,lLoanTypeID,lOfficeID,lCurrencyID,strTitle,strNextCtrlName,strFirstTD,strSecondTD)
{
		...
		document.writeln("<td " + strFirstTD + "> " + strTitle + "：<a href='javascript:showExtendApplyCtrl(\"" + strFormName + "\",\"" + strCtrlName + "\",\"" + lOfficeID + "\",\"" + lCurrencyID  + "\",\"" + lLoanTypeID + "\",\""+ strNextCtrlName + "\")'><img border= '0' src='/websett/image/icon.gif' width='15' height='15'></a> "
				+ "  "
...
function createClientAccountCtrl(strFormName,strCtrlName,strClientAccount,lOfficeID,strTitle,strNextCtrlName,strFirstTD,strSecondTD)
{
		...
		document.writeln("<td " + strFirstTD + ">" + strTitle + "：<a href='javascript:showClientAccountCtrl(\"" + strFormName + "\",\"" + strCtrlName + "\",\"" + lOfficeID +"\",\"" + strNextCtrlName + "\")'><img border= '0' src='/websett/image/icon.gif' width='15' height='15'></a> "
				+ "  "
				+ "</td>"
...
function createDiscountCtrl(strFormName,strCtrlName,lDiscountID,strDiscountNo,lOfficeID,lCurrencyID,strTitle,objClientID,strNextCtrlName,strFirstTD,strSecondTD)
{
		...
		document.writeln("<td " + strFirstTD + "> " + strTitle + "：<a href='javascript:showDiscountCtrl(\"" + strFormName + "\",\"" + strCtrlName + "\",\"" + lOfficeID + "\",\"" + lCurrencyID  + "\",\"" +  strNextCtrlName + "\")'><img border= '0' src='/websett/image/icon.gif' width='15' height='15'></a> "
				+ "  "
...
function createDiscountCtrl1(strFormName,strCtrlName,lDiscountID,strDiscountNo,lOfficeID,lCurrencyID,strTitle,objClientID,strNextCtrlName,strFirstTD,strSecondTD)
{
		...
		document.writeln("<td " + strFirstTD + "> " + strTitle + "：<a href='javascript:showDiscountCtrl1(\"" + strFormName + "\",\"" + strCtrlName + "\",\"" + lOfficeID + "\",\"" + lCurrencyID  + "\",\"" +  strNextCtrlName + "\")'><img border= '0' src='/websett/image/icon.gif' width='15' height='15'></a> "
				+ "  "
				+ " <INPUT TYPE='HIDDEN' VALUE='"+lDiscountID+"' ID='"+strCtrlName+ "' NAME='"+strCtrlName+"'>"
				+ " <INPUT TYPE='HIDDEN' VALUE='"+strDiscountNo+"' ID='"+strCtrlName+ "Code' NAME='"+strCtrlName+"Code'>"
				+ "</td>"
...
function createCreditNameCtrl(strFormName,strCtrlName,strCreditName,lCreditID,strTitle,strNextCtrlName,strFirstTD,strSecondTD)
{
		...
		document.writeln("<td " + strFirstTD + ">" + strTitle + "：<a href='javascript:showCreditNameCtrl(\"" + strFormName + "\",\"" + strCtrlName +"\",\"" + strNextCtrlName + "\")'><img border= '0' src='/websett/image/icon.gif' width='15' height='15'></a> "
				+ "  "
...
function createCreditCodeCtrl(strFormName,strCtrlName,strCreditCode,lCreditID,strTitle,strNextCtrlName,strFirstTD,strSecondTD)
{
		...
		document.writeln("<td " + strFirstTD + ">" + strTitle + "：<a href='javascript:showCreditCodeCtrl(\"" + strFormName + "\",\"" + strCtrlName +"\",\"" + strNextCtrlName + "\")'><img border= '0' src='/websett/image/icon.gif' width='15' height='15'></a> "
				+ "  "
				+ " <INPUT TYPE='HIDDEN' VALUE='"+lCreditID+"' ID='"+strCtrlName+ "' NAME='"+strCtrlName+"'>"
				+ "</td>"
...
function createInvestmentCodeCtrl(strFormName,strCtrlName,strInvestmentCode,lInvestmentID,lCurrencyID,lOfficeID,strTitle,strNextCtrlName,strFirstTD,strSecondTD)
{
		...
		document.writeln("<td " + strFirstTD + ">" + strTitle + "：<a href='javascript:showInvestmentCodeCtrl(\"" + strFormName + "\",\"" + strCtrlName + "\",\"" + lCurrencyID + "\",\"" + lOfficeID +"\",\"" + strNextCtrlName + "\")'><img border= '0' src='/websett/image/icon.gif' width='15' height='15'></a> "
				+ "  "
...
//				eval("strAccountNo" + nNum + "=" + strAccountNo.substring(nStart,i-1));
	...
	document.writeln("<td " + strFirstTD + ">" + strTitle + "： "
			...
			+ "  - "
			+ "  <input type='text' disabled name='" + strCtrlName + "zhh3' size='6' maxlength='6' class='box'  value='" + strAccountNo3 + "'>");
	if(strAccountNo4!="")
	{
		document.writeln( "  - "
			+ "  <input type='text' disabled name='" + strCtrlName + "zhh4' size='2' maxlength='2' class='box'  value='" + strAccountNo4 + "' >"
			+ " "
			+ "</td>");
	}
 ...
 */
	 ...
	 * 显示中文金额
	 */
	function showChinese(sAmount1,nCurrencyID)
	{
		var sResult = "";
		var sign="";//added by mzh_fu 2007/06/27解决负数问题
		
		//modified by mzh_fu 2007-03-16 下面一行增加(parseFloat(reverseFormatAmount(sAmount1))).toFixed(2)==0
		sAmount1 = reverseFormatAmount(sAmount1);				
		if (sAmount1 == null || sAmount1=="" || (parseFloat(reverseFormatAmount(sAmount1))).toFixed(2)==0.00) 
		{
				sResult = "零元";
		} 
		else 
		{
			var sAmount=fntrimzero(sAmount1);
			
			//added by mzh_fu 2007/06/27解决负数问题			
			if(sAmount < 0){
				sign = "负";
			  sAmount = -1*sAmount;
			  sAmount = sAmount +'';			 
		  ...
		  if(sTempRight<10)
			...
			switch(nCurrencyID)
			{
				...
				case 3:
					...
					break;
				case 4:
					sb=sb+getThousandChineseData(sRight)+ "生丁";
					break;
				case 6:
					sb=sb+getThousandChineseData(sRight)+ "分";
					break;
  				case 11:
  					sb=sb+getThousandChineseData(sRight)+ "芬尼";
  					break;
  			    case 17:
  				...
  				case 18:
  					...
  					break;
  				case 25:
  					sb=sb+getThousandChineseData(sRight)+ "円";
  					break;
  				case 26:
...
function createLiborRateCodeCtrl(strFormName,strCtrlName,strLiborRateCode,lLiborRateID, lCurrencyID,lOfficeID,strTitle,strNextCtrlName,strFirstTD,strSecondTD)
{
		...
		document.writeln("<td " + strFirstTD + ">" + strTitle + "：<a href='javascript:showLiborRateCodeCtrl(\"" + strFormName + "\",\"" + strCtrlName + "\",\"" + strLiborRateCode + "\",\"" + lOfficeID + "\",\"" + lCurrencyID +"\",\"" + strNextCtrlName + "\")'><img border= '0' src='/websett/image/icon.gif' width='15' height='15'></a> "
				+ "  "
				+ " <INPUT TYPE='HIDDEN' VALUE='"+lLiborRateID+"' ID='"+strCtrlName+ "' NAME='"+strCtrlName+"'>"
				+ "</td>"
...
function createOfficeAccountCtrl1(strFormName,strCtrlName,strOfficeAccount,objClientID, lCurrencyID,strTitle,strNextCtrlName,strFirstTD,strSecondTD)
{
		...
		document.writeln("<td " + strFirstTD + ">" + strTitle + "：<a href='javascript:showOfficeAccountCtrl1(\"" + strFormName + "\",\"" + strCtrlName + "\",\"" + strOfficeAccount + "\",\"" + lCurrencyID +"\",\"" + strNextCtrlName + "\")'><img border= '0' src='/websett/image/icon.gif' width='15' height='15'></a> "
				+ "  "
...
function createOfficeAccountCtrl(strFormName,strCtrlName,strOfficeAccount,lCLIENTID, lCurrencyID,strTitle,strNextCtrlName,strFirstTD,strSecondTD)
{
		...
		document.writeln("<td " + strFirstTD + ">" + strTitle + "：<a href='javascript:showOfficeAccountCtrl(\"" + strFormName + "\",\"" + strCtrlName + "\",\"" + strOfficeAccount + "\",\"" + lCLIENTID + "\",\"" + lCurrencyID +"\",\"" + strNextCtrlName + "\")'><img border= '0' src='/websett/image/icon.gif' width='15' height='15'></a> "
				+ "  "
				+ " <INPUT TYPE='HIDDEN' VALUE='"+strOfficeAccount+"' ID='"+strCtrlName+ "' NAME='"+strCtrlName+"'>"
				+ "</td>"
...
function createStatusLoanInfoCtrl(strFormName,strCtrlName,lLoanInfoID,strLoanInfoNo,lLoanTypeID,lOfficeID,lCurrencyID,strTitle,lStatusID,strNextCtrlName,strFirstTD,strSecondTD)
{
		...
		document.writeln("<td " + strFirstTD + "> " + strTitle + "：<a href='javascript:showStatusLoanInfoCtrl(\"" + strFormName + "\",\"" + strCtrlName + "\",\"" + lOfficeID + "\",\"" + lCurrencyID  + "\",\"" + lLoanTypeID+ "\",\"" + lStatusID + "\",\""+ strNextCtrlName + "\")'><img border= '0' src='/websett/image/icon.gif' width='15' height='15'></a> "
				+ "  "
				+ " <INPUT TYPE='HIDDEN' VALUE='"+lLoanInfoID+"' ID='"+strCtrlName+ "' NAME='"+strCtrlName+"'>"
				+ " <INPUT TYPE='HIDDEN' VALUE='"+strLoanInfoNo+"' ID='"+strCtrlName+ "Code' NAME='"+strCtrlName+"Code'>"
				+ "</td>"
...
{ 
		...
		document.writeln("<td " + strFirstTD + ">" + strTitle + "：<a href='javascript:showCashFlowCtrl(\"" + strFormName + "\",\"" + strCtrlName + "\",\"" + strNextCtrlName + "\")'><img border= '0' src='/websett/image/icon.gif' width='15' height='15'></a> "
				+ "  "
...
function createFixedRateCtrl(strFormName,strCtrlName,fFixedRate,strCode,lOfficeID,strTitle,strNextCtrlName,strFirstTD,strSecondTD)
{
		...
		document.writeln("<td " + strFirstTD + ">" + strTitle + "：<a href='javascript:showFixedRateCtrl(\"" + strFormName + "\",\"" + strCtrlName + "\",\"" + lOfficeID +"\",\"" + strNextCtrlName + "\")'><img border= '0' src='/websett/image/icon.gif' width='15' height='15'></a> "
				+ "  "
				+ " <INPUT TYPE='HIDDEN' VALUE='"+strCode+"' ID='"+strCtrlName+ "Code' NAME='"+strCtrlName+"Code'>"
				+ "</td>"
...
function createAheadRepayRequisitionCtrl(strFormName,strCtrlName,lOfficeID,lCurrencyID,lContractID,lLetoutRequisitionID,lAheadRepayRequisitionID,strAheadRepayRequisitionCode,dAmount,tsInterestStart,strTitle,strAmount,dtInterestDate,strNextCtrlName,strFirstTD,strSecondTD)
{
		...
		document.writeln("<td " + strFirstTD + ">" + strTitle + "：<a href='javascript:showAheadRepayRequisitionCtrl(\"" + strFormName + "\",\"" + strCtrlName + "\",\"" + lOfficeID + "\",\"" + lCurrencyID +"\",\"" + strNextCtrlName + "\")'><img border= '0' src='/websett/image/icon.gif' width='15' height='15'></a> "
				+ "  "
...
function createDiscountBillCtrl(strFormName,strCtrlName,lOfficeID,lCurrencyID,lContractID,lDiscountCredenceID,strDiscountBillCode,dAmount,tsStartDate,tsEndDate,strTitle,strAmount,strStartDate,strEndDate,strNextCtrlName,strFirstTD,strSecondTD)
{
		...
		document.writeln("<td " + strFirstTD + ">" + strTitle + "：<a href='javascript:showDiscountBillCtrl(\"" + strFormName + "\",\"" + strCtrlName + "\",\"" + lOfficeID + "\",\"" + lCurrencyID +"\",\"" + strNextCtrlName + "\")'><img border= '0' src='/websett/image/icon.gif' width='15' height='15'></a> "
				+ "  "
...
//------------------------------------------------------------------------
	...
	function fntrimzero(str)//去除首位0函数
{
	nospace=1;
	//modify by wjliu --当很str被识别为数字时,就没有chatAt()的方法,把str1转换为字符串 2007-3-31
	//var str1=str;
	var str1=str + "";
 ...
 /**
  * added by mzh_fu 2007/06/23 用于IFRAME
  * Modify by leiyang date 2007/07/20
  */
function iframeAutoFit(arg0)
{
	var oWindow = null;
	try{
		if(window.frameElement != null){
			oWindow = window.frameElement;
		}
		else{
			alert("脚本无法跨域操作！");
			return;
	...
	//网页正文全文高
	var iframeBodyHeigth = window.document.body.scrollHeight;

	var iFrameColl = oWindow.document.getElementsByTagName("IFRAME");
	for(var i=0; i<iFrameColl.length; i++){
		var oIframe = iFrameColl[i];
		if(oIframe.contentWindow == window){
			if(iframeBodyHeigth < arg0){
				oIframe.height = iframeBodyHeigth;
			}
			else{
				oIframe.height = arg0;
			}
...
/**
 * added by gmqiu 2012/06/01 批量操作
 * @param frm  当前页面<from>标签name属性的值
 * @param groupCheckBoxName  复选框组name属性的值
 * @param msg  提示消息
 * @param strAction    页面隐藏的Action控制标识
 * @param strActionCode  标识值
 * @return
 */
function betchToDo(frm,groupCheckBoxName,msg,strAction,strActionCode){
    if(frm == ""){
       confirm("from不能为空！"); 
       return;
    }
    if(groupCheckBoxName == ""){
       confirm("groupCheckBoxName不能为空！"); 
       return;
    }
    var ids = document.getElementsByName(groupCheckBoxName);
    var j = 0;
    if(ids.length > 0){
       ...
       if(j > 0){
	      if(msg != "" && confirm(msg)){
	         if(strAction != "" && strActionCode != ""){
	             document.getElementById(strAction).value=strActionCode;
		         showSending();
				 frm.submit();
	         }else{
	             confirm("strAction、strActionCode不能为空！");
		         return;
	         }
	      }
       }else{
...
/**
 * added by gmqiu 2012/06/01 全选操作
 * @param checkBoxId checkBox的id属性的值
 * @param groupCheckBoxName 复选框组name属性的值
 * @return
 */
function isSelectAll(checkBoxId, groupCheckBoxName) {
	var checkBoxes = document.getElementById(checkBoxId).checked;
	var checkIds = document.getElementsByName(groupCheckBoxName);
	if (checkBoxes == true) {
...
/**
 * add by gmqiu 2012/06/21 验证日期格式
 * @param elementId  元素Id属性
 * @param isNotNull  是否不能为空，不允许为空输入true,允许为空输入false
 * @param msg  提示消息  如：开户日期
 * @return 
 */
function validateDate(elementId,isNotNull,msg){
    //验证日期格式
    var reg = /^(([1-9][0-9][0-9][0-9])-(([0]|[1])[0-9])-(([0]|[1]|[2])[0-9])|([3]([0]|[1])))$/;
    var checkDate = document.getElementById(elementId);
    if(isNotNull == true){
       if(!reg.test(checkDate.value)){
        confirm(msg+"格式不正确,请重新输入！");
        document.getElementById(elementId).value = "";
        document.getElementById(elementId).focus();
        return;
       }
    }else{
        confirm(msg+"不能为空,请输入！");
        document.getElementById(elementId).focus();
        return;
    }
}
...
/**
 * add by gmqiu 2012/06/21 比较日期，与validateDate同时使用
 * @param dtStart  开始日期
 * @param dtEnd    结束日期
 * @param msg  提示消息  如：开始日期不能早于结束日期，请重新输入！
 * @return
 */
function compareValidateDate(dtStart,dtEnd,msg){
    if(dtStart > dtEnd){
        confirm(msg);
        document.getElementById(dtEnd).value = "";
        document.getElementById(dtEnd).focus();
        return;
    }
}
