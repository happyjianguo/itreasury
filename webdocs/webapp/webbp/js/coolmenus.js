...
makeCM.prototype.mout = function(){ //Changed v4.06
	var c = this;
	clearTimeout(c.tim); c.isover = 0; var f="if(!"+c.name+".isover)"+c.name+".hidesub()"
...
makeCM.prototype.construct=function(nowrite){ //Changed v4.06
  var c=this; if(!c.l[0]||c.l[0].m.length==0) return cm_message('No menus defined');
	 if(!nowrite){for(var i=1;i<c.l.length;i++){c.l[i].str+="</div><iframe class='backgroundIframe' frameborder=0 marginheight=0 marginwidth=0 hspace=0 vspace=0 scrolling=no></iframe>"}} //Added 4.06
...
function showMenu(strPriv,strUrl){
	if (strPriv.length > 0){
		...
		oM.level[1]=new cm_makeLevel(160,23,"clS","clSover",0,0,"clBS",0,"right",0,0,"/itreasury/graphics/m_arrow.gif",9,9);
		...
		var tmpArrNo = new Array();	//????????????
		var newArrNo = new Array();	//????????????????
		...
		if (arrDepart.length==4){
		  ...
		  for(i=0;i<iLength;i++){
			...
			//????????????
			if (arrUrl[i].length>0 && arrUrl[i] != "null"){
			...
			//????????
			if (arrLevel[i]!=999){
				oM.makeMenu(newArrNo[i],sParent[i], arrName[i],arrUrl[i],'DataArea','','','','','','','','','','status="";','status="";','status="";')
			}
		  }
		}
		oM.menuPlacement=0;
