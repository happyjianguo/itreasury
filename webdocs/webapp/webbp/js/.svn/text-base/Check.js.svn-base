...
function validateFields(form) 
{
	...
	for (x in oAllFields) 
	{
        ...
        */
		//判断是否允许为空
		//放大镜类型放在前面判断，因为focusField比较特殊（带'Ctrl'后缀）
		if(bIsFormControl == true && oAllFields[x][3] == 1 && oAllFields[x][2] == "magnifier" && form[oAllFields[x][0]+'Ctrl'].value == '')
		{
			if (i == 0) 
			{
		...
		{
			if (form[oAllFields[x][0]].value <= 0 || form[oAllFields[x][0]].value == '')
			{
				if (i == 0) 
				{
					focusField = form[oAllFields[x][0]+'Ctrl'];
		...
		if (bIsFormControl == true && oAllFields[x][2] == 'money' && form[oAllFields[x][0]].value != '') 
		{
			...
			else
			{
				if(oAllFields[x][3] == 1)
				{
					if(parseFloat(reverseFormatAmount(form[oAllFields[x][0]].value)) <= 0)
					{
						if (i == 0) 
						{
							focusField = form[oAllFields[x][0]];
		...
		//如果是textarea类型，则判断字符的长度是否小于等于指定字节数
		if (bIsFormControl == true && oAllFields[x][2] == 'textMaxlength' && form[oAllFields[x][0]].value != '') {
			var strValue = form[oAllFields[x][0]].value;
			var maxLength = 100;
			if(oAllFields[x][4]){
				maxLength = (oAllFields[x][4] != '' ? oAllFields[x][4]:100);
			}
			//加特殊字符限制
			/*
			var reg = /^[a-zA-Z0-9\-\_\s\u0391-\uFFE5]*$/;
			if(!reg.test(strValue))
			{
				if (i == 0) {
					focusField = form[oAllFields[x][0]];
				}
				fields[i++] = oAllFields[x][1]+"含有非法的特殊字符，请重新录入！";
			    bValid = false;			
			}
			//alert(maxLength);
			...
			else
			{
				*/
			    ...
			    if (strValue.length>maxLength) {
			    	...
			    	fields[i++] = oAllFields[x][1]+"不能多于"+(maxLength/2)+"个中文或"+maxLength+"个英文，请重新录入！";
			    	bValid = false;			
			    }
