...
{ 
	...
	// determines whether Netscape or Internet Explorer
		...
		{ // enter key pressed
			if(gnIsSelectCtrl==0)
			{
				if (nextfield == gSubmitFunction || nextfield=='submitfunction')
			 	{
...
function verifyAmount(strData,isCanInputNegative,intDigit,decimalFractionDigit){
 ...
 /**
 * 创建金额控件，如果没有输入金额，焦点在控件上时，显示为"0.0"，移出焦点，显示为空。
 *
 * @param strFormName
 * @param strCtrlName 控件名称
 * @param strData 数据
 ...
 * @param nCurrencyID 币种
 */
 function createAmountCtrl2(strFormName,strCtrlName,strData,strNextCtrlName,strChineseCtrl,nCurrencyID,sProperty)
 {
	//alert("n c=" + nCurrencyID);
 	if(isNaN(nCurrencyID)) 
 	{
 		...
 		document.writeln("<input class=tar   type='text' name='"+ strCtrlName + "' value='" + formatAmount2(strData) +"' size='18' maxlength='15' onblur='adjustAmount2(\"" + strFormName +"\",\"" + strCtrlName +"\",1,\"" + strChineseCtrl +"\"," + nCurrencyID+")'  onfocus='adjustAmount2(\"" + strFormName +"\",\"" + strCtrlName +"\",2,\"" + strChineseCtrl +"\"," + nCurrencyID + ")' " + sProperty + "> <input type='hidden' name='" + strCtrlName + "NextCtrlName' value='" + strNextCtrlName + "'>");
 }
 ...
 * @param strCtrlName
 * @param strData
 * @param strNextCtrlName
 */
function createInterestRateCtrl(strFormName,strCtrlName,strData,strNextCtrlName,strChineseCtrl,nCurrencyID,sProperty){
		if(isNaN(nCurrencyID)) 
	 	{
	 		nCurrencyID=1;//缺省人民币
	 	}
 		...
 		document.writeln(" <input class=tar   type='text' name='"+ strCtrlName + "' value='" +  strData +"' size='18' maxlength='15' onblur='if (parseFloat(this.value)>100){alert(\"利率不允许大于100%!\");this.value=100;}adjustInterestRate(\"" + strFormName +"\",\"" + strCtrlName +"\",1,\"" + strChineseCtrl +"\"," + nCurrencyID+")'  onfocus='adjustInterestRate(\"" + strFormName +"\",\"" + strCtrlName +"\",2,\"" + strChineseCtrl +"\"," + nCurrencyID + ")' " + sProperty + "> <input type='hidden' name='" + strCtrlName + "NextCtrlName' value='" + strNextCtrlName + "'>");
}
 ...
 * @param strNextCtrlName
 */
function createConvertRateCtrl(strFormName,strCtrlName,strData,strNextCtrlName,strChineseCtrl,nCurrencyID,sProperty,strOnChange){
		if(isNaN(nCurrencyID)) 
	 	{
	 		nCurrencyID=1;//缺省人民币
	 	}
 		...
 		document.writeln(" <input class=tar   type='text' name='"+ strCtrlName + "' value='" +  strData +"' size='18' maxlength='5' onblur='adjustAmount(\"" + strFormName +"\",\"" + strCtrlName +"\",1,\"" + strChineseCtrl +"\"," + nCurrencyID+")'  onfocus='adjustAmount(\"" + strFormName +"\",\"" + strCtrlName +"\",2,\"" + strChineseCtrl +"\"," + nCurrencyID + ")' onchange='"+ strOnChange +"' " + sProperty + "> <input type='hidden' name='" + strCtrlName + "NextCtrlName' value='" + strNextCtrlName + "'>");
}
 ...
 * @return 返回格式化的金额
 */
 function formatAmount2(strData)
 {
 	if(isInnerFloat(strData))
 	{
 		if(strData!="")
 ...
 * @param nType 类型1,format;2,reverseFormat
 */
 function adjustAmount2(strFormName,strCtrlName,nType,strChineseCtrl,nCurrencyID)
 {
	if (window.event.keyCode!=13)//
	{
	 	var strOldData=eval(strFormName+"."+strCtrlName+".value");
	 	...
	 	if (isInnerFloat(strOldData))//是否是浮点数
	 	{
			...
			{
    	 			if (strOldData=="0.0000" )
    	 			{
    	 		  		strNewData = "";
    	 			}
    	 			else
    	 			{
					strNewData=formatAmount2(strOldData);
				}
				if(strChineseCtrl!="")
				{
					eval(strFormName + "." + strChineseCtrl + ".value='" + showChinese(strOldData,nCurrencyID) +"'");
	 	...
	 	else
	 	{
			if(nType==2)
			{
				eval(strFormName + "." +strCtrlName + ".value='0.0000'");
				eval(strFormName + "." +strCtrlName + ".select()");
...
function createRateCtrl(strFormName,strCtrlName,strRateName,lRateID,lOfficeID,strTitle,strNextCtrlName,strFirstTD,strSecondTD)
{
		...
		document.writeln("<td " + strFirstTD + ">" + strTitle + "：<a href='javascript:showRateCtrl(\"" + strFormName + "\",\"" + strCtrlName + "\",\"" + lOfficeID +"\",\"" + strNextCtrlName + "\")'><img border= '0' src='/websett/icon.gif' width='15' height='15'></a> "
				+ "  "
				+ " <INPUT TYPE='HIDDEN' VALUE='"+lRateID+"' ID='"+strCtrlName+ "' NAME='"+strCtrlName+"'>"
 ...
 */
	...
	function showChinese(sAmount,nCurrencyID)
	{
		...
		if (sAmount == null || sAmount=="" || (parseFloat(reverseFormatAmount(sAmount))).toFixed(2)==0.00) 
		{
				sResult = "零元";
		} 
		else 
 ...
 */
		...
		num = fntrimzero(num);
 		...
 		if(num != ""){
	 		...
	 		if (intPart.length>3){
	 			...
	 			if (deciPart != null && deciPart.length>0){		//如果有小数位
	 				tmpToReturn = signPart + intPart + "." + deciPart;
	 				}
	 			else{
	 				tmpToReturn = signPart + intPart
	 				}
	 			}		
 		}
 		return tmpToReturn;
	...
	/**
	 * 数字控件的onChange事件校验
	 * @param amountObj 数字控件对对象
	 * @param 整数位数
	 * @param 小数位数
	 * @Author: mzh_fu(付明正)
	 * Date: 2007/06/28
	 */
	function onNumberChangeEven(amountObj,isCanInputNegative,intDigit,decimalFractionDigit){
		amountObj.value = amountObj.value.replace(/\s/gim,"");
		amountObj.value = amountObj.value.replace(/,/g,'');
		/*
		if(!verifyAmount(amountObj.value,isCanInputNegative,intDigit,decimalFractionDigit)){	
	 ...
	 * 可以写多个同名的金额控件,内部使用接口
	 */
	function createMutiAmountBaseFun(strFormName,strCtrlName,strData,deci,sProperty,strNextCtrlName,txtStyle,strMaxlength,strOnChange){
 	 	document.writeln(" <input class='" + txtStyle + "' type='text' name='"+ strCtrlName + "' value='" + formatNumber( strData,deci ) +"' size='18' maxlength='"+ strMaxlength +"' onchange='"+strOnChange+"' onkeypress='filterKey(this)' onblur='adjustNumber(\"" + strNextCtrlName + "\",this," + deci +",1)'  onfocus='adjustNumber(\"" + strNextCtrlName + "\",this," + deci +",2)' " + sProperty + ">");
 	}
	 ...
	 */
 	function createMutiAmountCtrl(strFormName,strCtrlName,strData,strNextCtrlName){
 	 	createMutiAmountBaseFun(strFormName,strCtrlName,strData,2,"",strNextCtrlName,"tar","18");
 	}
 	 	/**
	 * 创建数字控件,可以控制小数位数
	 */
	function createNumberCtrl(strFormName,strCtrlName,strData,deci,strNextCtrlName){
		createMutiAmountBaseFun(strFormName,strCtrlName,strData,deci,"",strNextCtrlName,"tar","18");
	}
	
	/**
	 * 创建可以控制小数位数,并且可以控制文本框样式的数字控件
	 */
	function createNumberCtrl(strFormName,strCtrlName,strData,deci,strNextCtrlName,txtStyle){
		createMutiAmountBaseFun(strFormName,strCtrlName,strData,deci,"",strNextCtrlName,txtStyle,"18");
	}
			
	/**
	 * 创建数字控件,可以控制小数位数,并且有自己的onchange方法和属性
	 */
	function createNumberCtrl2(strFormName,strCtrlName,strData,deci,sProperty,strNextCtrlName,strOnChange){
		createMutiAmountBaseFun(strFormName,strCtrlName,strData,deci,sProperty,strNextCtrlName,"tar","18",strOnChange);
	}
	
	/**
	 * 创建数字控件,可以控制小数位数,控制Maxlength和自己的onchange方法和属性
	 * added by leiyang
	 */
	function createNumberCtrl3(strFormName,strCtrlName,strData,deci,sProperty,strNextCtrlName,strMaxlength,strOnChange){
		createMutiAmountBaseFun(strFormName,strCtrlName,strData,deci,sProperty,strNextCtrlName,"tar",strMaxlength,strOnChange);
	}
	
	/**
	 * added by leiyang date 2007/11/29
	 */
	//证券业务基本业务函数
	//自动更新BargainAmount文本框和Amount隐藏域的值
	function setBargainAmount(frm){
		if(frm.price.value=="" || frm.quantity.value==""){
			frm.BargainAmount.value="";
		}else{
			if(isFloat(frm.price.value) && isInt(frm.quantity.value)){
				frm.BargainAmount.value = formatAmount(eval(reverseFormatAmount1(frm.price.value))*eval(frm.quantity.value));
				frm.amount.value = frm.BargainAmount.value;
			}
		}
	}
	
	/**
	 * added by leiyang date 2007/12/14
	 */
	//证券业务基本业务函数
	//自动更新BargainAmount文本框和Amount隐藏域的值
	function setBargainAmount1(frm){
		if(frm.netPrice.value=="" || frm.quantity.value==""){
			frm.BargainAmount.value="";
		}else{
			if(isFloat(frm.netPrice.value) && isInt(frm.quantity.value)){
				frm.BargainAmount.value = formatAmount(eval(reverseFormatAmount1(frm.netPrice.value))*eval(frm.quantity.value));
				frm.amount.value = frm.BargainAmount.value;
			}
		}
	}
	 ...
	 /**
	  * added by leiyang date 2007/09/17
	  */
 ...
 * @return 返回格式化的金额
 */
function formatAmount3(strData1)
 {
 	strData1 = strData1 + "";
 	//added by mzh_fu 2007/07/13 解决金额控件的问题
 	if(strData1 !=null && strData1 !=""){
	...
	var strData=fntrimzero(strData1);
	//add by minzhao 2009-07-29 如果输入值为“0”，则去首位0后为空	
	if(strData=="")
	{
		strData="0.00";
	}
