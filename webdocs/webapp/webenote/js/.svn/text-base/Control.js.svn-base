 ...
 * 格式化数字
 * @param strData 需要格式化的数据
 * @param decimals 小数点保留几位
 * @return 返回格式化的数字
 */
function formatNumber(oInput, strMessage , decimals)
{
	var strInputValue = Trim(oInput.value);
	var strData = "";

	if(strInputValue != null && strInputValue != ""){
		strInputValue = strInputValue.replace(/\s/gim,"");
		strInputValue = strInputValue.replace(/,/g,'');
	}
	else {
		return false;
	}
	...
	if(strInputValue!='0')
	{
		strData = Trim(fntrimzero(strInputValue));
	}
	else
	{
		strData = Trim(strInputValue);
	}
	//检查数据合法性
	var regStr = "^\\d*(\\.\\d{1,6})?$";
	var regExp;
	/*if(decimals == 0){
		...
		regStr = "^\\d*(\\.\\d{1,"+ decimals +"})?$";
	}*/
	regExp = new RegExp(regStr);
	if(regExp.test(strInputValue) == false){
		if(strMessage == ""){
			alert("数据格式不正确！");
		}
		else {
			alert(strMessage + " 的数据格式不正确！");
		}
		oInput.value = "";
		oInput.focus();
		return false;
	...
	//将小数点前和后的数据分别取出来
	var nPoint = strData.indexOf(".");
	var strFront = strData;
	var strEnd = "";
	if(nPoint != -1){
		strFront = strData.substring(0, nPoint);
		strEnd = strData.substring(nPoint+1, strData.length);
	...
	//补或者截小数点后面的值，保持两位
	if(strEnd.length > decimals){
		strEnd = strEnd.substring(0, decimals);
	}

	if(strEnd.length < decimals){
		for(var i=strEnd.length; i<decimals; i++){
			strEnd = strEnd + "0";
		} 			
	}
