 ...
 * @return 返回格式化的金额
 */
 function formatAmountForBG(strData)
 {
 	if(isInnerFloat(strData))
 	{
 		if(strData!="")
 ...
 * @param nType 类型1,format;2,reverseFormat
 */
 function adjustAmountForTemplateTree(strCtrlName,nType)
 {
	if (window.event.keyCode!=13)//
	{
	 	var strOldData=eval("document.all."+strCtrlName+".value");
	 	if (isInnerFloat(strOldData))//是否是浮点数
	 	{
	 		var strNewData="";
	 		if(nType==2)//说明是离开
			...
			else
			{
    	 			...
    	 			{
					strNewData=formatAmountForBG(strOldData);
				}
			}
			if(strNewData=="")//
			{
			...
			eval("document.all." +strCtrlName + ".value='"+strNewData+"'");
			if(nType==2)
			{
				eval("document.all." +strCtrlName + ".select()");
	 	...
	 	else
	 	{
			if(nType==2)
			{
				eval("document.all." +strCtrlName + ".value='0'");
				eval("document.all." +strCtrlName + ".select()");
...
*	新焦点控制方法
*/	
var allElementsValue = new Array();
var allElements = new Array();
function firstFocus()
{
	getElements();
	if (allElementsValue.length>0)
	{
		getElementByValue(allElementsValue[0]).focus();
	}
}
function getElements()
{
	var array = new Array();
	var feild = document.all;
	var m = 0;
	for (var i=0;i<feild.length;i++)
	{
		if (feild[i].tab)
		{
			allElementsValue[m] = new Number(feild[i].tab);
			allElements[m] = feild[i];
			m++;
		}
	}
	QuickSort(allElementsValue);
}
function getElementByValue(tab)
{

	for (var i=0;i<allElements.length;i++)
	{
		if (allElements[i].tab == tab)
		{
...
{ 
	...
	{ 
		...
		for (var i=0;i<allElementsValue.length;i++)
		{
			if (allElementsValue[i] > e.tab)
			{
				getElementByValue(allElementsValue[i]).focus();
				break;
			}
		}
