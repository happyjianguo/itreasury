...
/**
	...
	dhtmlxCombo控件，用于信用评级时显示分数下拉列表
	支持下拉选项是大段文字时，可以换行显示
	本js和dhtmlx官方发布的js有所区别，主要调整了三点：
	1.在控件是回车时，显示或隐藏下拉列表（原来没有此功能）；
	2.解决了该控件放置在table里显示时，下拉列表框位置不对的问题。
	3.解决了选择完成后,控件没有处在选中状态的问题
*/
...
function dhtmlXCombo(parent,name,width,optionType,tabIndex)
{
	if (typeof(parent)=="string")
 	...
 	dhtmlXCombo.prototype.enableFilteringMode = function(mode,url,cache,autosubload){
 			...
 			if (url){this._xml=url;this._autoxml=convertStringToBoolean(autosubload)};
 						...
 						if((ev.keyCode >= 48 && ev.keyCode <= 57)||(ev.keyCode >= 65 && ev.keyCode <= 90))
 						{
 							for(var i=0;i<that.optionsArr.length;i++){
 								...
 								if(text.toString().toUpperCase().indexOf(String.fromCharCode(ev.keyCode)) == 0){
 									that.selectOption(i);break}};
 									ev.cancelBubble=true}}}};
 									dhtmlXCombo.prototype.getOption = function(value){
 										for(var i=0;i<this.optionsArr.length;i++)
 											if(this.optionsArr[i].value==value)return this.optionsArr[i];
 											return null};
 									dhtmlXCombo.prototype.getOptionByLabel = function(value){
 										for(var i=0;i<this.optionsArr.length;i++)
 										if(this.optionsArr[i].text==value || this.optionsArr[i]._ctext==value)return this.optionsArr[i];
 										return null};
 									...
 									dhtmlXCombo.prototype.deleteOption = function(value){var ind=this.getIndexByValue(value);if(ind<0)return;if (this.optionsArr[ind]==this._selOption)this._selOption=null;this.optionsArr.splice(ind, 1);this.redrawOptions()};
 									...
 									dhtmlXCombo.prototype.addOption = function(options){if (!arguments[0].length || typeof(arguments[0])!="object")args = [arguments];else
 ...
 {var _sID = this._inID;this.DOMParent.removeChild(this.DOMelem);this.DOMlist.parentNode.removeChild(this.DOMlist);var s=dhx_glbSelectAr;this.DOMParent=this.DOMlist=this.DOMelem=0;this.DOMlist.combo=this.DOMelem.combo=0;for(var i=0;i<s.length;i++){if(s[i]._inID == _sID){s[i] = null;s.splice(i,1);return}}};
 									...
 									dhtmlXCombo.prototype._createSelf = function(selParent, name, width, tab){
	 										if (width.toString().indexOf("%")!=-1){
	 											...
	 											window.setInterval(
	 												function(){if (!selParent.parentNode)return;var ts=selParent.parentNode.offsetWidth*resWidht-2;if (ts<0)return;if (ts==self._lastTs)return;self.setSize(self._lastTs=ts)},
	 												500);
 											...
 											if(this.rtl)this.DOMlist.className = 'dhx_combo_list_rtl';
 											...
 											if (_isIE){this.DOMlistF = document.createElement("IFRAME");
 											...
 											this.DOMlist.onmousedown = function(){this._skipBlur=true};
 ...
 that._fetchOptions(i+1,that._lasttext||"")}};
							         ...
							         var parentDOMelem=this.DOMelem.parentNode;
							         var tdWidth=0;
							         while(parentDOMelem){
							         	if(parentDOMelem.tagName=='td'||parentDOMelem.tagName=='TD'){
							         		tdWidth=parentDOMelem.offsetHeight;
							         		break;
							         	}
							         	parentDOMelem = parentDOMelem.parentNode;
							        }
							         if(this.ListPosition == 'Bottom'){
							         		var iTop = (tdWidth==0)?(pos[1]+this.DOMelem.offsetHeight):(pos[1]+this.DOMelem.offsetHeight-(tdWidth-this.DOMelem.offsetHeight)/2);
							            this.DOMlist.style.top = iTop + "px";           
 									...
 									dhtmlXCombo.prototype.getPosition = function(oNode,pNode){if(!pNode)var pNode = document.body
 ...
 return this.selectOption(z,false,false);z+=step}};
 									dhtmlXCombo.prototype._onKeyF = function(e){var that=this.parentNode.combo;var ev=e||event;ev.cancelBubble=true;
 									if(ev.keyCode=="13"){that.toggleSelect();that.DOMelem_input.select();}else if (ev.keyCode=="9" ){that._confirmSelection();that.closeAll()}else
 ...
 obj._correctSelection()};var selected=xml.doXPath("//option[@selected]");if (selected.length)obj.selectOption(obj.getIndexByValue(selected[0].getAttribute("value")),false,true);obj.callEvent("onXLE",[])};
 									...
 									dhtmlXCombo.prototype._confirmSelection = function(data,status){if(arguments.length==0){var z=this.getOptionByLabel(this.DOMelem_input.value);data = z?z.value:this.DOMelem_input.value;status = (z==null);if (data==this.getActualValue()) return};this.DOMelem_hidden_input.value=data;this.DOMelem_hidden_input2.value = (status?"true":"false");this.callEvent("onChange",[]);this._activeMode=false};
 ...
 {(e||event).cancelBubble = true;var node=(_isIE?event.srcElement:e.target);var that=this.combo;while (!node._self){node = node.parentNode;if (!node)return};var i=0;for (i;i<that.DOMlist.childNodes.length;i++){if (that.DOMlist.childNodes[i]==node)break};that.selectOption(i,false,true);that.closeAll();that.callEvent("onBlur",[]);that.DOMelem_input.select();
 that._activeMode=false};
 									...
 									dhtmlXCombo.prototype.toggleSelect = function(){
 									...
 									dhtmlXCombo.prototype._fetchOptions=function(ind,text){if (text==""){this.closeAll();return this.clearAll()};var url=this._xml+((this._xml.indexOf("?")!=-1)?"&":"?")+"pos="+ind+"&mask="+encodeURIComponent(text);this._lasttext=text;if (this._load)this._load=url;else this.loadXML(url)};
 										...
 										if(window.dhx_glbSelectAr)
 											for (var i=0;i<dhx_glbSelectAr.length;i++){
 												...
 												function dhtmlXRange(InputId, Start, End){
 													...
 													var Length = Input.value.length;Start--;
 												...
 												dhtmlXCombo_defaultOption.prototype.init = function(){
 														...
 														this.selected = false;
 														this.css = ""};
 												dhtmlXCombo_defaultOption.prototype.select = function(){if (this.content)this.content.className="dhx_selected_option"};
												...
												dhtmlXCombo_optionTypes['default'] = dhtmlXCombo_defaultOption;//(c)dhtmlx ltd. www.dhtmlx.com
//v.2.1 build 90226
