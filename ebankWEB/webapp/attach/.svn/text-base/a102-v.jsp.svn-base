...
<FORM METHOD="post" name="form_1"
 ACTION="<%=Env.EBANK_URL%>/UpLoadServlet?DocType=<%=Constant.DocType.EBANKUPLOAD%>&DestJsp=<%=Env.EBANK_URL%>attach/a106-c.jsp&UserID=<%=sessionMng.m_lUserID%>" 
...
function validate(iType,iNo)
{
  ...
  if (iType == 1)
  {
	...
	form1.action = "<%=Env.EBANK_URL%>UpLoadServlet?DocType=<%=Constant.DocType.LOANCONTRACTCONTENT%>&ParentID=<%=ParentID%>&ParentIDType=<%=ParentIDType%>&ClientName=" + strDesc + "&DestJsp=/attach/a106-c.jsp&UserID=<%=sessionMng.m_lUserID%>";
	//alert("<%=Env.EBANK_URL%>UpLoadServlet?DocType=<%=Constant.DocType.LOANCONTRACTCONTENT%>&ParentID=<%=ParentID%>&ParentIDType=<%=ParentIDType%>&ClientName=" + strDesc + "&DestJsp=<%=Env.EBANK_URL%>attach/a106-c.jsp&UserID=<%=sessionMng.m_lUserID%>");
  }
  else
  {
  	if ( confirm("确定删除吗？") )
	{
    	form1.action = "<%=Env.EBANK_URL%>attach/a103-c.jsp?fileID=" + iNo ;
