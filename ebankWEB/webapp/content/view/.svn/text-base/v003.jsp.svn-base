...
<TABLE border=0 class=top width="80%">
  ...
  <tr>
	<td height="39" width="583">
		年份: <INPUT TYPE="text" maxlength=4 NAME="strYear" onFocus="nextfield ='strSeason';" >  季度:  <INPUT TYPE="text" NAME="strSeason" maxlength=1 onFocus="nextfield ='searchlow';" >  
	</td>
  </tr>
	<tr>
	<td>
...
%>
  ...
  <tr>
    ...
    <table width="80%" border="0"  class="ItemList">
      <tr>
        <td class=ItemTitle width="33%" align="center">编号</td>
        <td class=ItemTitle width="33%" align="center">年份</td>
        <td class=ItemTitle width="33%" align="center">季度</td>
      </tr>
<%
			Iterator it = null;
			int count=1;
...
%>
      <tr>
		...
		<A href="javascript:queryDetail(<%=info.getID()%>,<%=info.getCode()!=null&&info.getCode().length()>=5?info.getCode().substring(0,info.getCode().length()-1):""%>,<%=info.getCode()!=null&&info.getCode().length()>=5?info.getCode().substring(info.getCode().length()-1):""%>);">
		...
		<%=info.getCode()!=null&&info.getCode().length()>=5?info.getCode().substring(0,info.getCode().length()-1):""%>
		</td>
		<td class=ItemBody align="center">
		
		<%=info.getCode()!=null&&info.getCode().length()>=5?info.getCode().substring(info.getCode().length()-1):""%>
		</td>
      </tr>
<%
				}
...
function query()
{
		...
		if (!InputValid(frm.strSeason,0,"int",1,1,4,"季度"))			
		{
			  return false;
		}
...
{ 
	...
	if (!validate() )
	{
		...
		frm.strSuccessPageURL.value = "../view/v102.jsp";
		frm.strAction.value = "add";	//操作--新增
		frm.strFailPageURL.value = "../view/v102.jsp";		
		showSending();
		frm.submit();
...
function queryDetail(id,year,season)
{
	
	
	frm.hdnlContentID.value = id;
	frm.hdnstrYear.value = year;
	frm.hdnstrSeason.value = season;
...
function validate()
{
	...
	if (!InputValid(frm.strSeason,1,"int",1,1,4,"季度"))			
		{
			  return false;
		}
	return true;
...
<%
	...
	{
		//OBHtml.showExceptionMessage(out,sessionMng,ie,request,response,strTableTitle, Constant.RecordStatus.VALID);
		out.flush();
		return;
	}
%>
