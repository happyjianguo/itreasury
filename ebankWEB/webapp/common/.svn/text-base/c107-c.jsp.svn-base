...
<%
	...
	try
	{
		...
		strTmp =request.getParameter("typeID");
        if( strTmp != null && strTmp.length() > 0 )
		{
		     typeID = Long.parseLong(strTmp.trim());
		}
		strTmp =request.getParameter("parentTypeID");
        if( strTmp != null && strTmp.length() > 0 )
		{
		     parentTypeID = Long.parseLong(strTmp.trim());
		}
		...
		request.setAttribute("transCode", String.valueOf(transCode));   
		request.setAttribute("firstIn","true");
		String strURL = "/AttachFrame.jsp";
...
(pageControllerInfo));
	...
	{
		OBHtml.showExceptionMessage(out, sessionMng, ie, "附件上传","",1);
		ie.printStackTrace();
		out.flush();
		return;
	}
