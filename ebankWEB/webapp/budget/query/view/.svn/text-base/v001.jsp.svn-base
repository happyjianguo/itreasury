...
try
{
	//请求检测
	/** 权限检查 **/
	String strTableTitle = "分析考核 - 预算历史对比分析";
	...
	*/
	...
	<TABLE width="80%" height="10" border=0 class=top><TBODY><TR><TD class=FormTitle height=2 width="100%"><B>		分析考核 - 预算历史对比分析</B></TD></TR><TR>
	<TD width="100%"  vAlign=bottom><table width="100%" border=0>
<TR><td><input type=hidden name=clientID value=<%=clientId%>>
<%
...
<tr>
<td ><font color=red>*</font> 预算区间A：从</td>
<td>
	<fs_c:calendar 
   	  	name="startDate"
    	value="" 
...
<A href="javascript:show_calendar('form_1.startDate');" onMouseOut="window.status='';return true;"onMouseOver="window.status='Date Picker';return true;"><IMG border="0" height="16"src="/websett/image/calendar.gif" width="17"></A>
 --></td>
<td >到：</td>
<td><input type="text" name="endDate" size="20" maxlength="30" class="box" onFocus="nextfield='relativeStartDate';" readonly></td>
</TR>
<tr>
<td ><font color=red>*</font> 预算区间B：从</td>
<td>
	<fs_c:calendar 
       	    name="relativeStartDate"
         	value="" 
         	properties="javascript:getRelativeEndDate();nextfield='relativeEndDate';" 
         	size="20"/>
   	<script>
   		$('#relativeStartDate').blur(
   			function(){
				getRelativeEndDate();
			}
		);
				          	...
				          	<!-- 
<input type="text" name="relativeStartDate" size="20" maxlength="30" class="box" onFocus="javascript:getRelativeEndDate();nextfield='relativeEndDate';" onblur="javascript:getRelativeEndDate();">&nbsp;
<A href="javascript:show_calendar('form_1.relativeStartDate');" onMouseOut="window.status='';return true;"onMouseOver="window.status='Date Picker';return true;"><IMG border="0" height="16"src="/websett/image/calendar.gif" width="17"></A>
 --></td>
<td >到：</td>
<td><input type="text" name="relativeEndDate" size="20" maxlength="30" class="box" onFocus="nextfield='submitfunction';" readonly></td>
</TR>
</TABLE></TD>
         </TR>
         <TR>
             ...
             <TABLE align=center height="15" width="97%">
               <TBODY>
                 <TR>
				   ...
				   <input class="button" name="Submit32" onclick="doQuery();" type="button" value=" 查 找 ">
...
function doQuery()
{
		...
		beginDT1 = document.form_1.relativeStartDate.value ;
		endDT1 = document.form_1.relativeEndDate.value ;
		if( beginDT1.length != 0 && endDT1.length != 0 &&  beginDT1 > endDT1){
			alert('预算区间B－从不能大于－到!');
			document.form_1.relativeEndDate.focus();
			return false;
		} 
		showSending();//显示正在执行
		document.form_1.strSuccessPageURL.value="../view/v002.jsp";	//定义操作成功后跳往的页面
		document.form_1.strFailPageURL.value="../view/v002.jsp";		//定义失败后跳往的页面
		document.form_1.strAction.value="<%=Constant.Actions.NEXTSTEP%>";	//定义操作代码
		//document.form_1.action.value="../control/c001.jsp";
		form_1.submit();
}
function allFields()
{
		this.a0 = new Array("budgetSystemID"," 预算体系","magnifier",1);
		//this.a1 = new Array("clientIDCtrl"," 预算单位","string",1);
		this.a2 = new Array("budgetPeriodID"," 预算周期","magnifier",1);
		this.a3 = new Array("startDate"," 预算区间A－从","date",1);
		//this.a4 = new Array("endDate"," 预算区间A－到","date",1);
		this.a5 = new Array("startDate"," 预算区间A－从","date",0);
		//this.a6 = new Array("endDate"," 预算区间A－到","date",0);
		this.a7 = new Array("relativeStartDate"," 预算区间B－从","date",1);
		//this.a8 = new Array("relativeEndDate"," 预算区间B－到","date",1);
		this.a9 = new Array("relativeStartDate"," 预算区间B－从","date",0);
		//this.a10 = new Array("relativeEndDate"," 预算区间B－到","date",0);
}
function getEndDate()
{
...
function getRelativeEndDate()
{
var strStartDate = form_1.relativeStartDate.value;
if (strStartDate != "" && form_1.BudgetPeriodType.value != "-1")
	{
		var startDate = new Date(strStartDate.substring(0,4),new Number(strStartDate.substring(5,7)),new Number(strStartDate.substring(8,10)));
		var newDate = startDate;
		...
		if (month<10)month="0"+month;
		if (day<10)day="0"+day;
		form_1.relativeEndDate.value = year + "-" + month + "-" + day;
	}
}
</script>
<%	
