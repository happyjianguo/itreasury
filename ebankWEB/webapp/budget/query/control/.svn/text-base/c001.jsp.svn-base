...
{	
	...
	Collection historyColl= new ArrayList();
	if(String.valueOf(Constant.Actions.NEXTSTEP).equals(strAction)){
		...
		//查询所有符合条件的记录（单位条件除外）
		historyColl=delegation.queryHistory(qInfo);
		request.setAttribute("maxLevel",String.valueOf(level));
		System.out.println(qInfo.getBudgetSystemID()+"单位 ---"+qInfo.getClientID());
		request.setAttribute("historyColl",historyColl);
		pageInfo.setStrSuccessPageURL("../view/v002.jsp");
	}
	/**
	 * 操作成功
	 */
