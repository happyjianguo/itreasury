...
try
{
	...
	strTmp = (String)request.getAttribute("source");
		source = strTmp;
...
%>
	...
	<form name="frmV002" method="post" action="../control/c003.jsp">
		...
		<table width="100%" border="0" class="top" height="100">
			<tr>
				<td height="2" class="FormTitle" width="100%" colspan="2"><b>预算编制及审批 - 预算调整</b>
				</td>
			</tr>
			<tr>
...
function compute(form)
{
	...
	{
		eval("form.txtAmount6_"+form.hdnItemNo[i].value+".value = (form.txtAmount5_"+form.hdnItemNo[i].value+".value * form.quotiety.value)");
	}
...
{	
	showSending();
	document.location.href="../control/c003.jsp?strSuccessPageURL=../view/v006.jsp&strFailPageURL=../view/v006.jsp&strAction=7&budgetFlag=1";		
}
...
{	
	if (confirm("确认保存?"))
	{
		...
		form.id.value="";	//清空ID列,否则更新原先的编制
		form.strSuccessPageURL.value="../control/c001.jsp?strSuccessPageURL=../view/v006.jsp&strFailPageURL=../view/v006.jsp&strAction=7&budgetFlag=2";	//定义操作成功后跳往的页面
		form.strFailPageURL.value="../view/v007.jsp";		//定义失败后跳往的页面
...
{
	showSending();
	form_1.action="../control/c003.jsp";
	form_1.strSuccessPageURL.value="../view/v007.jsp";	//定义操作成功后跳往的页面
	form_1.strFailPageURL.value="../view/v007.jsp";		//定义失败后跳往的页面
