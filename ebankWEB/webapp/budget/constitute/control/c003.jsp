...
try {
	...
	*/
	...
	if (pageInfo.getStrAction().equals(String.valueOf(Constant.Actions.MATCHSEARCH)))	//查找
	{
		...
		Collection c = delegation.findOriginalBudget(info);
	...
	else if (pageInfo.getStrAction().equals(String.valueOf(Constant.Actions.NEXTSTEP)))	//下一步
	{
		...
		if (showColumn > 0)
			...
			BUDGETConstant.BudgetColumnList.ADJUSTSCALE,
			showColumn,
			BUDGETConstant.BudgetColumnList.CURRENTAMOUNT
			});
		else
			info.setShowColumn(new long[]{
				BUDGETConstant.BudgetColumnList.ORIGINALAMOUNT,
		...
		Collection c = null;
		if (versionNo.indexOf("YS") > -1)
		{
			versionNo = delegation.getNewVersionNo(info);
			request.setAttribute("source","ys");
			c = delegation.findNewConstituteTemplate(info);	//重新做调整，需要先检查已经做过调整
		} 
		else
		{
			c = delegation.findAll(info);	//查看以前的调整信息
		}
