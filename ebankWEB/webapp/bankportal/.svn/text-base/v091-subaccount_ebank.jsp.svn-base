...
<%
    try
	{
       ...
       // 		out.flush();
		...
		JSPLogger.info("*******进入页面--bankportal\\v091-subaccount_ebank.jsp*******");
		/**页面校验结束*/
...
$(document).ready(function() {
	$("#flexlist").flexigridenc({
		...
		title:'账户对账单',
		classMethod : 'com.iss.itreasury.ebank.system.action.BillPrintAction.querySubAccountBillPrintInfo',//要调用的方法
...
{	
	window.open('<%=strContext%>/bankportal/c091-account_ebank.jsp?p_SuccessPageURL=/bankportal/v092-account_ebank.jsp&p_FailPageURL=/bankportal/v091-subaccount_ebank.jsp&p_Action=findById&accountId='+acctId+'&transactionStartDateString='+queryStartDate+'&transactionEndDateString='+queryEndDate+'&bankType='+bankType);
}
