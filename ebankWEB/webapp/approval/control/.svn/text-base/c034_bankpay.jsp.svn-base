...
<%
    ...
    try
	{
		...
		long lDesc=-1;
		String strTemp = null;
		strTemp = request.getParameter("lOrderField");
		if (strTemp != null && strTemp.trim().length() > 0)
		{
			lOrderField = Long.valueOf(strTemp).longValue();
		}
		//业务类型
		strTemp = request.getParameter("transactionTypeID");

		if (strTemp != null && strTemp.trim().length() > 0)
		{
		
			if("-1".equals(strTemp)){
				transactionTypeID = OBConstant.SettInstrType.ONLINEBANK_BANKPAY;
			}else{
		...
		//执行日由
		strTemp = request.getParameter("dtExecuteFrom");
		if (strTemp != null && strTemp.trim().length() > 0)
		{
			dtExecuteFrom = DataFormat.getDateTime(strTemp);
		}
		//执行日到
		strTemp = request.getParameter("dtExecuteTo");
		if (strTemp != null && strTemp.trim().length() > 0)
		{
		...
		OBQueryApprovalRecordBiz biz=new OBQueryApprovalRecordBiz();
			...
			if(transactionTypeID==OBConstant.SettInstrType.ONLINEBANK_BANKPAY){
				pageLoader = biz.getHistoryListBankPay(conditionworkInfo,lDesc,lOrderField);
			}
		...
		request.setAttribute("strSuccessPageURL","/approval/view/v034_bankpay.jsp");
		request.setAttribute("strFailPageURL","/approval/view/v034_bankpay.jsp");
