...
<%
		...
		String strTableTitle = "委托贷款免还申请—免还通知单明细";
try
{
		//判断是否登录
...
///////control/////////////////////////////////////////////
		...
		strTmp = (String)request.getAttribute("nTmpID");//免还ID
		if( strTmp != null && strTmp.length() > 0 )
		{
		...
		if (strControl.equals("view"))
		{
			...
			strFreeReason=DataFormat.formatString(freeinfo.getFreeReason());						//免还原因
			strConsignClientAccount=DataFormat.formatString(freeinfo.getConsignClientAccount());	//委托账户
			lStatusID=freeinfo.getStatusID();							//免还状态
			lInputUserID=freeinfo.getInputUserID();//录入人ID
			lCheckUserID=freeinfo.getCheckUserID();//审核人ID
			strInputUserName=DataFormat.formatString(freeinfo.getInputUserName());//录入人名称
			strCheckUserName=DataFormat.formatString(freeinfo.getCheckUserName());//审核人名称
			...
			dChangeRate=contractinfo.getChargeRate();//手续费率
			dBalance =contractinfo.getBalance();					//贷款余额，未还金额
			strLoanPurpose=DataFormat.formatString(contractinfo.getLoanPurpose());			//借款用途
			...
			//---------------------------//
			strConsignClientName=DataFormat.formatString(payinfo.getConsignClientName());//委托单位名称
			strLoanPayCode=DataFormat.formatString(payinfo.getCode());					//放款通知单编号
			dPayAmount=payinfo.getAmount();						//放款金额
			dPayBalance=payinfo.getBalance();					//放款余额
			//dPayBalance=10000;								//放款余额
			dtEndDate = payinfo.getEnd();						//到期日
			dtStartDate=payinfo.getStart();						//发放日
			//----------------------------//
...
<tr align="center">
<td class="td-bottom" colspan="4" height="30">华　能　财　务　有　限　责　任　公　司</td>
</tr>
	<tr>
