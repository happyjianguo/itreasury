...
<%
	try
	{
	   /* 用户登录检测 */
		...
		long lIsFromSubmitPage = -1;  //判断是否从提交页面返回>0则表示是
System.out.println("lIsFromSubmitPagelIsFromSubmitPagelIsFromSubmitPage==="+lIsFromSubmitPage);
    	// 定义变量
...
//----------------------------------------------do add------------------------------------------------------------
			...
			strTmp = ((String)request.getAttribute("lIsFromSubmitPage"));
			if (strTmp != null && strTmp.length() > 0 && !strTmp.equals("null")) 
			{
				lIsFromSubmitPage = Long.valueOf(strTmp).longValue();
			} 
...
System.out.println("lextendID=="+lExtendID);
if(lExtendID < 0 )
{
			...
			strTmp = (String)request.getAttribute("extype");
			if ( strTmp != null && strTmp.length() >0) 
			{
				lEType = Long.parseLong(strTmp);
			}
