...
<%
  try
  {
	Log.print("*******进入页面--ebank/capital/loanrepayment/l005-c.jsp*******");
	   ...
	   /* 用户登录检测 */
		...
		long lTransType = OBConstant.SettInstrType.TRUSTLOANRECEIVE;
		long lPayerAcctID = -1;
		long lPayeeAcctID = -1;
		double dBalance = 0.0;
		double dAmount = 0.0;
	    ...
	    double dInterestRate = 0.00; //贷款利息利率	
		 ...
		 if (strTemp != null && strTemp.trim().length() > 0)
		 {
			...
			if (isYT.equals("1"))
			{
				 lTransType = OBConstant.SettInstrType.YTLOANRECEIVE;
			}
		 }
		...
		//获取信息
		 ...
		 if (strTemp != null && strTemp.trim().length() > 0)
		 {
		  ...
		  strTemp = (String)request.getAttribute("lPayeeAcctID");
		 if (strTemp != null && strTemp.trim().length() > 0)
		 {
		...
		financeInfo.setClearInterest(tsExecuteDate);//在自营贷款清还中，设计为(结息日 = 执行日)		
		...
		financeInfo.setRealInterest(dInterest);
        financeInfo.setRealCompoundInterest(dCompoundInterest);
   		financeInfo.setRealOverdueInterest(dOverDueInterest);
   		financeInfo.setRealSuretyFee(dSuretyFee);
		...
		financeInfo.setRealInterestReceiveable(dInterestReceiveable);
		financeInfo.setRealInterestIncome(dInterestIncome);
	...
	pageControllerInfo.setUrl("/capital/loanrepayment/l006-v.jsp");
	//分发
