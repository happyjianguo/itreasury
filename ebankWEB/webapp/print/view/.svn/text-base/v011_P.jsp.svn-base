				  	...
				  	<td>
				  		<table align=center border=0 width="97%">
				  			<tr>
								...
								<td>
<%
									OBHtmlCom.showOfficeListControl(out,"officeList",sessionMng.m_lOfficeID,"onchange=\"showExcuteDate(this.value,frm.currencyId.value);\"",false,false,sessionMng.m_lCurrencyID);
 %>
								</td>				  					
				  			</tr>
				  		</table>
				 ...
				 <TR class="MsoNormal">
					<TD class="MsoNormal">
				  		<TABLE align=center border=0 width="97%">
							<TR>
						  		<TD height=20 width="36%" colspan = "2">
...
{	
	if(!check(frm.tsExecuteStartDate.value,frm.tsExecuteEndDate.value,frm.hideOpenDate.value))
	{
		return false;
	}
	...
	form.action = "<%=strContext%>/print/control/c011_P.jsp";
	showSending();
	form.submit();
}
