...
try
{
	...
	Log.print("***进入页面 ： /zoom/ShowBankPayZoom.jsp ***");
...
<%
	...
	//获取记录集
	ctResult = OBMagnifier.getOBBankPaySelectList(strMainFields, strReturnFields, strDisplayFields, strSQL);
	for(int i=0;i<strMainFields.length;i++)
	  System.out.println(strMainFields[i]+"////"+i);
	for(int i=0;i<strReturnFields.length;i++)
	  System.out.println(strReturnFields[i]+"****"+i);  
	for(int i=0;i<strDisplayFields.length;i++)
	  System.out.println(strDisplayFields[i]+"++++"+i);  
	if(ctResult != null)
	{
