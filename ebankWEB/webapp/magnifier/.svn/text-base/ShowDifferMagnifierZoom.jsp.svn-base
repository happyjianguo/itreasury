...
<%@ page import="java.util.*,
				...
				com.iss.itreasury.loan.loanpaynotice.dao.LoanPayNoticeDao,
...
try
{
	...
	//Log.print("***进入页面 ： cpfsec/common/ShowMagnifierZoom.jsp ***");
	...
	String strNextControls = request.getParameter("strNextControls");//返回主页面后，获得焦点的控件名
...
<%
	if (ctResult!=null)
	{
	    ...
	    for(int i=1; it.hasNext(); i++)
		{
			...
			double dTemp = 0; 
			if (oReturnCols[0] != null )
			{
				System.out.println("******************************oReturnCols[0]1="+oReturnCols[0]);
				LoanPayNoticeDao lpndao = new LoanPayNoticeDao();
				dTemp = lpndao.getLatelyRate(Long.parseLong(String.valueOf(oReturnCols[0])),null);
				oMainCols[2] =DataFormat.formatRate(dTemp);
			}
...
<%
			for(int j=0; j<strDisplayNames.length; j++)
			{
				...
				{
%>
      	<td class="ItemBody"><A href="#" onClick='<%=sSelectFunction%>'><div align="center"><%=oDisplayCols[j]%></div></A></td>
<%
				}else{
%>
		<td class="ItemBody"><div align="center"><%=oDisplayCols[j]%></div></td>
<%
				}
			}
%>
