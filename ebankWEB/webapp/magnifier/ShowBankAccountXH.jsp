<%
/**
 * 程序名称：ShowBankAccountXH.jsp
 * 功能说明：中交,网银,下划,付款方放大镜的显示小窗口
 * 作　　者：Forest Ming
 * 完成日期：2003.08.01
*/
%>
...
try
{
	...
	Log.print("***进入页面 ： /zoom/ShowBankAccountXH.jsp ***");
...
<%
	...
	//获取记录集
	ctResult = OBMagnifier.getOBAccountCommonSelectListXH(strMainFields, strReturnFields, strDisplayFields, strSQL);
	if(ctResult != null)
	{
		bOne = ctResult.size() == 1 ? true : false;
	}
%>
<table width="100%" border="0" class="table" class="ItemList" cellspacing="1">
<%
	if(ctResult == null)
	{
%>
...
<%
			for(int j=0; j<strDisplayNames.length; j++)
			{
				...
				{
%>
      	<td class="ItemBody" align="center"><A href="#" onClick='<%=sSelectFunction%>'><%=oDisplayCols[j]%></A></td>
<%
				}else{
%>
