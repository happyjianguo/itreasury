...
<br/>
      ...
      <p ><br>
        银行汇款 将于签认后才提交到<%=NameRef.getOfficeNameByID(sessionMng.m_lOfficeID)%>！<br>
			  <br>
              该笔交易有待签认人签认！
			  <br>
              <br>
 ...
 <br>
      ...
      <td width="80%" align="right"> 
      	<% if(info.getNstatus() == OBConstant.OBBankPayStatus.SIGN){ %>
		<input class=button1 name=sign type=button value=" 取消签认 " onClick="Javascript:cancelsignme();" >
		<% }else{ %>
		<input class=button1 name=sign type=button value=" 签  认 " onClick="Javascript:signme();" >
		<% } %>
		<input class=button1 name=back type=button value=" 返  回 " onClick="javascript:returnme()" >
      </td>
        </tr>
