...
<%		
 ...
 %>
 		<tr>
			...
			%>					
				<iframe src="../../AttachFrame.jsp?lID=<%=longtime%>&lTypeID=<%=OBConstant.AttachParentType.OBBUDGET%>&sCaption=<%=URLEncoder.encode("链接附件")%>&TransTypeID=<%=Constant.ModuleType.BUDGET%>&TransSubTypeID=<%=Constant.ModuleType.BUDGET%>&control=add&showOnly=false" width=600 height="100" scrolling="Auto" frameborder="0" name="currentSignID"></iframe>
			</td>
...
<input name="inputdate" type="hidden" value=<%=DataFormat.getStringDateTime().substring(0,10)%> >--%>
  ...
  <input name="sysdate" type="hidden" value=<%=DataFormat.getStringDateTime().substring(0,10)%>>
	  ...
	  <TR>
		  <TD colspan="3">
			 <%--<iframe src="/NASApp/iTreasury-ebank/HistoryOpinionFrame.jsp?transNo=<%=info.getId()+ ""%>&&transType=<%=OBConstant.SettInstrType.BUDGETADJUST%>" width="100%" height="100" scrolling="Auto" frameborder="0" name="currentSignID"></iframe>--%>
...
<script language="Javascript">
	...
	function doApproval()
	{
		...
		{					
			...
			frm.strFailPageURL.value='../view/vApp010.jsp';
