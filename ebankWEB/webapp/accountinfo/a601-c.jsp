...
<%
	...
	long lID 						= -1;								//ID
	Sett_TransGrantLoanDAO loanDelegation = new Sett_TransGrantLoanDAO();		//定义delegation
    try
	{
		Log.print("*******进入页面--control/a601-c.jsp*******");
		//判断是否登录
		...
		if (dRate > 0.0)
		{
			...
			else if (lTransactionTypeID==SETTConstant.TransactionType.BANKGROUPLOANGRANT){	//如果是银团
				strTemp = (String)request.getAttribute("lPayInterestAccountID");
				if (strTemp != null && strTemp.trim().length() > 0)
				{
				    lPayInterestAccountID = Long.valueOf(strTemp).longValue();
        ...
        if("Query".equals(strAction))
        {
			...
			if (resultinfo != null)
			{
				...
				if(payforminfo != null )
	            {
					...
					bankLoancol =bankLoanQuery.findByFormID(resultinfo.getLoanNoteID());
					request.setAttribute("searchBankLoanDrawResults",bankLoancol);
				}
			}
		}
