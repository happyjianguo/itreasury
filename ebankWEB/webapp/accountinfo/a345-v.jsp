...
<%
    Log.print("\n*******进入页面--ebank/capital/accountinfo/a345-v.jsp******\n");
    //标题变量
	...
	<form name="frmV045" method="post" action="">
		...
		<TABLE width="99%" border="0" class="top" height="328">
			<TR class="tableHeader">
				<TD height="2" class="FormTitle" width="100%"><B>支票付款</B>
				</TD>
			</TR>
			<TR>
						...
						<TR>
							<TD height="2" width="95%">
								<TABLE width="97%" height="2" align="center">
									...
									<TR>
										<TD align="left" valign="middle" height="20" width="12%">	转账支票号：
										</TD>
										<TD align="left" valign="middle" height="20" width="18%">											
...
function checkSuccess()
{
	...
	document.location.href="<%=strContext%>/settlement/tran/current/view/v044.jsp";	
}
...
function doPrint()
{
	window.open("<%=strContext%>/accountinfo/a401-v.jsp?lID=<%=lId%>&lTransactionTypeID=<%=lTransactionTypeID%>&lReturn=<%=lReturn%>&strSuccessPageURL=/iTreasury-ebank/accountinfo/a401-v.jsp&strFailPageURL=/iTreasury-ebank/accountinfo/a345-v.jsp");
}
...
function doReturn()
{
	...
	{%>
		document.location.href="<%=strContext%>/settlement/tran/current/control/c003.jsp?lStatusID=<%=SETTConstant.TransactionStatus.CHECK%>&lTransactionTypeID=<%=SETTConstant.TransactionType.CHECKPAY%>&strSuccessPageURL=../view/v046.jsp&strFailPageURL=../view/v046.jsp&strAction=<%=SETTConstant.Actions.LINKSEARCH%>";
	<%
	}else
	{
	%>
...
%>
<script language="JavaScript">
firstFocus(document.frmV045.strCheckAbstractStr);
