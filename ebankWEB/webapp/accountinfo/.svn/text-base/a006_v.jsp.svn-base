...
<safety:resources />
	...
	<form name="frmV002" action="../a004_c.jsp">		
		...
		<input type="hidden" name="strAction" value="notice">
...
$(document).ready(function() {
	...
	$("#flexlist").flexigridenc({
		colModel : [
			{display: '交易编号',  name : 'clientname', width : 200, sortable : false, align: 'center'},
			{display: '交易类型',  name : 'clientname', width : 200, sortable : false, align: 'center'},
			{display: '子账号',  name : 'officename', width : 200, sortable : false, align: 'center'},
			{display: '交易日',  name : 'loginno', width : 200, sortable : false, align: 'center'},
			{display: '交易金额',  name : 'balance', width : 200, sortable : false, align: 'center'},
			{display: '利率',  name : 'rate', width : 200, sortable : false, align: 'center'},
			{display: '利息',  name : 'interest', width : 200, sortable : false, align: 'center'}
		],//列参数
		title:'通知存款账户：<%=strAccountNo%>',
		classMethod : 'com.iss.itreasury.ebank.obaccountinfo.action.OBAccountQueryAction.queryNoticeBalaceDetail',//要调用的方法
...
<%--    document.frmV002.submit();--%>
    window.open('<%=strContext%>/accountinfo/a004_c.jsp?strSuccessPageURL=/accountinfo/a005-p.jsp&strFailPageURL=/accountinfo/a005-p.jsp&strAction=notice&strDepositNo=<%=strDepositNo%>&lSubAccountID=<%=lSubAccountID%>&lContractID=<%=lContractID%>&lAccountID=<%=lAccountID%>&strStartClientCode=<%=NameRef.getClientCodeByID(sessionMng.m_lClientID)%>&strEndClientCode=<%=NameRef.getClientCodeByID(sessionMng.m_lClientID)%>&lAccountStatusID=1&lIsFiltrate=-1&UrlType=ebank&e_lOfficeID=<%=sessionMng.m_lOfficeID%>&lContractID=<%=lContractID%>&e_lCurrencyID=<%=sessionMng.m_lCurrencyID%>',"NewWin",'width=800,height=600,scrollbars=1,resizable=1,menubar=0,toolbar=0,locations=0,directories=0, screenX=0,screenY=0,left=0,top=0;');
}
