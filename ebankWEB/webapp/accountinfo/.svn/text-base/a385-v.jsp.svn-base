...
<%
    Log.print("\n*******进入页面--ebank/capital/accountinfo/a385-v.jsp******\n");
    //标题变量
		...
		<TABLE border="0" class="top" height="290" width="99%">
			<TBODY>
				<TR class="tableHeader">
					<TD class="FormTitle" height="2" width="100%"><B>保证金存款</B>
					</TD>
				</TR>
				<TR>
...
document.location.href="<%=strContext%>/settlement/tran/current/view/v084.jsp";
...
function doCheck()
{
	...
	{
		isSubmited = true;
		document.frmV085.strAction.value="<%=SETTConstant.Actions.CHECK%>";
		document.frmV085.submit();
	}
}
function doUndoCheck()
{
	...
	{
		if(!validateFields(frmV085)) return;

		isSubmited = true;
		document.frmV085.strAction.value="<%=SETTConstant.Actions.CANCELCHECK%>";
		document.frmV085.submit();
	}
}
...
function doPrint()
{
	window.open("<%=strContext%>/accountinfo/a401-v.jsp?lID=<%=lId%>&lTransactionTypeID=<%=lTransactionTypeID%>&lReturn=<%=lReturn%>&strSuccessPageURL=/iTreasury-ebank/accountinfo/a401-v.jsp&strFailPageURL=/iTreasury-ebank/accountinfo/a385-v.jsp");
}
...
function doReturn()
{
	...
	{%>
		document.location.href="<%=strContext%>/settlement/tran/current/control/c003.jsp?lStatusID=<%=SETTConstant.TransactionStatus.CHECK%>&lTransactionTypeID=<%=SETTConstant.TransactionType.CAUTIONMONEYSAVE%>&strSuccessPageURL=../view/v086.jsp&strFailPageURL=../view/v086.jsp&strAction=<%=SETTConstant.Actions.LINKSEARCH%>";
...
%>
<script language="JavaScript">
firstFocus(document.frmV085.strCheckAbstractStr);
