...
<%
    Log.print("\n*******进入页面--ebank/capital/accountinfo/a305-v.jsp******\n");
    //标题变量
	...
	<form name="frmV005" method="post" action="">
		...
		<table width="99%" border="0" class="top" height="290">
			<tr class="tableHeader">
				<td height="2" class="FormTitle" width="100%"><b>票汇付款</b>
				</td>
			</tr>
			<tr>
...
document.location.href="<%=strContext%>/settlement/tran/current/view/v004.jsp";
...
function doPrint()
{
	window.open("<%=strContext%>/accountinfo/a401-v.jsp?lID=<%=lId%>&lTransactionTypeID=<%=lTransactionTypeID%>&lReturn=<%=lReturn%>&strSuccessPageURL=/iTreasury-ebank/accountinfo/a401-v.jsp&strFailPageURL=/iTreasury-ebank/accountinfo/a305-v.jsp");
}
...
function doReturn()
{
	...
	{
	%>
		document.location.href="<%=strContext%>/settlement/tran/current/control/c003.jsp?lStatusID=<%=SETTConstant.TransactionStatus.CHECK%>&lTransactionTypeID=<%=SETTConstant.TransactionType.DRAFTPAY%>&strSuccessPageURL=../view/v006.jsp&strFailPageURL=../view/v006.jsp&strAction=<%=SETTConstant.Actions.LINKSEARCH%>";
	<%
	}else 
	{%>
		document.location.href="<%=strContext%>/settlement/tran/current/view/v004.jsp";
