...
function sendback1()
{
	...
	{
		if (document.frmv006.jzd.checked == true)//进账单
		{
		...
		if (document.frmv006.txffpz.checked == true)//进账单
		{
			strfrmList = strfrmList + "r";
		}
		else
		{
			strfrmList = strfrmList + "a";
		}		
		window.open('../common/p001-c.jsp?lID=<%=lID%>&printPage=../accountinfo/a908-c.jsp&lTransactionTypeID=<%=lTransactionTypeID%>&strPrintPage='+strfrmList);
		window.close();
	}
}
...
<body bgcolor="#d6d3ce"  text="#000000">
	<form name="frmv006" method="post" action="../accountinfo/a908-c.jsp">
  ...
  <% if (lOBReturn==OBConstant.CheckSettType.RecAndPayID){  %> 
    ...
    <tr bordercolor="#d6d3ce"> 
     <td colspan="2" height="30">  
        <input type="checkbox" name="txffpz" value="checkbox" onClick="check(this)">
        贴现发放凭证</td>
    </tr>
  ...
  <% } else {  %>
	<tr bordercolor="#d6d3ce"> 
     <td colspan="2" height="30">  无可打印票据 </td>
      ...
      <td width="124" height="30">
        <div align="center"> 
          <input type="button" name="Submit" value=" 关 闭 " onClick="closeThis()" class="button">
        </div>
      </td>
      <td height="30" width="14">&nbsp;</td>
    </tr>
