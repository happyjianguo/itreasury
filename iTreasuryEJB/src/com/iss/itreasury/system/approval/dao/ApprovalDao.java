...
public class ApprovalDao extends Object implements java.io.Serializable
{
	...
	public long findApprovalUserLevel(long lApprovalID, long lUserID) throws Exception
	{
		...
		try
		{
			strSQL = " select nLevel from loan_ApprovalItem where nApprovalID = ? and nUserID = ? ";
		...
		try
		{
			...
			strSQL =
				" select a.nUserID,b.sName from Loan_ApprovalItem a,UserInfo b "
					...
					+ " and a.nUserID = b.ID "
					+ " and b.nStatusID != 0 "
					+ " and ','||b.sCurrencyID||',' like '%," + lCurrencyID 
	...
	public ApprovalSettingInfo findApprovalItemAboveLevel(long lID, long lLevel, long lUserID, long lTopLevel, long lCurrencyID, long lOfficeID) throws Exception
	{
		...
		try
		{
			//请空临时的vector(下级审核人里不包括虚拟用户！！！）
			vTmp = new Vector();
			strSQL = " select a.nUserID,b.sName from Loan_ApprovalItem a,UserInfo b "
					+ " where nApprovalID = ? and nLevel > ? "
					+ " and a.nUserID = b.ID "
					+ " and ','||b.sCurrencyID||',' like '%,"+lCurrencyID+",%' and b.nOfficeID = " + lOfficeID  //加入办事处币种
					+ " and b.nStatusID != 0 "
					+ " and b.nIsVirtualUser != "
					+ Constant.YesOrNo.YES
	...
	public long saveApprovalTracing(ApprovalTracingInfo ATInfo) throws Exception
	{
		...
		try
		{
		    ...
		    strSQL =
				" insert into loan_ApprovalTracing(id,nApprovalContentID,"//nApprovalID
...
//			ps.setLong(nIndex++, ATInfo.getApprovalID());
	...
	public long findApprovalTracingMaxID() throws Exception
	{
		...
		try
		{
			strSQL = " select nvl(max(id),0)+1 as id from loan_ApprovalTracing ";
			ps = m_Conn.prepareStatement(strSQL);
			rs = ps.executeQuery();
			if (rs.next())
			{
	...
	public long findApprovalTracingSerialID(long lModuleID, long lLoanTypeID, long lActionID, long lOfficeID, long lCurrencyID, long lApprovalContentID) throws Exception
	{
		...
		try
		{
			strSQL = "select nvl(max(nSerialID)+1,1) as nSerialID from loan_approvalTracing where nModuleID = ? and nLoanTypeID = ? and nActionID = ? and nOfficeID = ? and nCurrencyID = ? and nApprovalContentID = ? ";
			ps = m_Conn.prepareStatement(strSQL);
	...
	public Collection findApprovalTracing(long lModuleID, long lLoanTypeID, long lActionID, long lOfficeID, long lCurrencyID, long lApprovalContentID, long lLevel, long lDesc) throws Exception
	{
		...
		try
		{
			strSQL = " select a.*,b.sName from loan_ApprovalTracing a,UserInfo b where a.nModuleID = ? and a.nLoanTypeID = ? and a.nActionID = ? " 
...
//			Log.print("sql="+strSQL);
	 ...
	 * 查询审批级别
	 */
	public long getApprovalLevel(long userid,long lModuleID, long lLoanTypeID, long lActionID, long lOfficeID, long lCurrencyID,long lSubLoanID) throws Exception{
		PreparedStatement ps = null;
		ResultSet rs = null;
		long lReturn = -1;
		String strSQL = "";
		try{
			strSQL = "select  nvl(c.nlevel,-1) levelid from loan_approvalitem c where c.napprovalid =" +
					"(select  t.approvalid  from loan_approvalrelation t where 1=1 and t.moduleid = ? " +
					"and t.officeid = ? and t.currencyid = ? and t.loantypeid =? and t.subloantypeid =? " +
					"and t.actionid = ? ) and c.nuserid = ? ";
			ps = m_Conn.prepareStatement(strSQL);
			ps.setLong(1,lModuleID);
			ps.setLong(2,lOfficeID);
			ps.setLong(3,lCurrencyID);
			ps.setLong(4,lLoanTypeID);
			ps.setLong(5,lSubLoanID);
			ps.setLong(6,lActionID);
			ps.setLong(7,userid);
			rs = ps.executeQuery();
			if(rs.next()){
				lReturn = rs.getLong("levelid");
			}
		}catch(Exception e){
			if (rs != null){
				rs.close();
	...
	public long deleteApprovalTracing(long lModuleID, long lLoanTypeID, long lActionID, long lOfficeID, long lCurrencyID, long lApprovalContentID, long lTypeID) throws Exception
	{
		...
		try
		{
			if (lTypeID == 1) //物理删除
			{
				strSQL = " delete from loan_approvalTracing where nModuleID = ? and nLoanTypeID = ? and nActionID = ? and nOfficeID = ? and nCurrencyID = ? and nApprovalContentID = ? ";
				ps = m_Conn.prepareStatement(strSQL);
				ps.setLong(1, lModuleID);
				ps.setLong(2, lLoanTypeID);
				ps.setLong(3, lActionID);
			...
			{
				strSQL = "update loan_approvalTracing set nStatusID = ? where nModuleID = ? and nLoanTypeID = ? and nActionID = ? and nOfficeID = ? and nCurrencyID = ? and nApprovalContentID = ? ";
				ps = m_Conn.prepareStatement(strSQL);
	...
	public long updateApprovalTracing(long lModuleID, long lLoanTypeID, long lActionID, long lOfficeID, long lCurrencyID, long lApprovalContentID, long lTypeID) throws Exception
	{
		...
		try
		{
			...
			{
			    strSQL = "update loan_approvalTracing set nLevel = ? where nModuleID = ? and nLoanTypeID = ? and nActionID = ? and nOfficeID = ? and nCurrencyID = ? and nApprovalContentID = ? ";
				ps = m_Conn.prepareStatement(strSQL);
				ps.setLong(1, -1);
				ps.setLong(2, lModuleID);
				ps.setLong(3, lLoanTypeID);
	...
	public String findApprovalChangeUser(long lApprovalID, long lUserID) throws Exception
	{
		...
		try
		{
			...
			{
				strSQL = " select * from loan_approvalChangeItem " + " where nUserID = " + lUserID + " and nApprovalID = " + lApprovalID + " and nStatusID = " + Constant.RecordStatus.VALID;
				ps = m_Conn.prepareStatement(strSQL);
				rs = ps.executeQuery();
				if (rs.next())
				{
				...
				else //如果有被别人传过来的权限
				{
					...
					strSQL = " select nUserID from loan_approvalChangeItem " + " where nNewUserID = " + lUserID + " and nApprovalID = " + lApprovalID + " and nStatusID = " + Constant.RecordStatus.VALID;
					ps = m_Conn.prepareStatement(strSQL);
					rs = ps.executeQuery();
					while (rs.next())
					{
	...
	public boolean checkApprovalUserLevel(long lApprovalID, long lUserID, long lLevel) throws Exception
	{
		...
		try
		{
			strSQL = " select count(*) from loan_ApprovalItem where nApprovalID = ? and nUserID = ? and nLevel = ? ";
	...
	/**
	 * 查询审批意见信息，如果有被退回得审批记录，那么只返回最新审批流程得信息
	 * 操作数据库表ApprovalSetting,ApprovalItem,ApprovalTracing
	 * @param       lApprovalID             审批设置标示
	 * @param       lApprovalContentID      审批内容标示
	 * @param       lDesc                   顺序还是倒序
	 * @return      Collection              返回审批意见信息(ApprovalTracingInfo)
	 */
	public Collection findValidateApprovalTracing(long lModuleID, long lLoanTypeID, long lActionID, long lOfficeID, long lCurrencyID, long lApprovalContentID, long lLevel, long lDesc) throws Exception
	{
		PreparedStatement ps = null;
		ResultSet rs = null;
		Collection c = null;
		...
		try
		{
			strSQL = " select a.*,b.sName from loan_ApprovalTracing a,UserInfo b where a.nUserID = b.ID and a.nModuleID = ? and a.nLoanTypeID = ? and a.nActionID = ? " 
			       + " and a.nApprovalContentID = ? and a.nOfficeID = ? and a.nCurrencyID = ? and a.nlevel>0 ";
			if (lLevel > 0)
			{
				strSQL += " and a.nLevel = " + lLevel;
			}
...
//			Log.print("sql="+strSQL);
	 ...
	 * @return      ApprovalSettingInfo    返回审批设置信息
	 */
	public long getApprovalID1(long lModuleID, long lLoanTypeID, long lActionID, long lOfficeID, long lCurrencyID,double amount) throws Exception
	{

		long lApprovalID = -1;
		ApprovalRelationDao approvalRelationDao = new ApprovalRelationDao();
		    lApprovalID = approvalRelationDao.findApprovalID1(lModuleID,lLoanTypeID,lActionID,lOfficeID,lCurrencyID,amount);
			System.out.println("==================lApprovalID="+lApprovalID);
