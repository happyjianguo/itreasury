...
public class SYSMagnifier
{
	...
	*/
		...
		try
		{
			...
			String sOnKeydown =
				"if("
					...
					+ Env.URL_PREFIX
					+ "/iTreasury-system/magnifier/ShowMagnifierZoom.jsp?"
					+ strParam
					+ "', 'SelectAnything', '"
					+ sFeatures
					+ "', false);}";
			...
			if (iPos == -1)
			{
				out.println(
					"<td "
						...
						+ "<a href=#><img name=\""
						+ strButtonName
						+ "\" src='/websys/image/icon.gif' border=0 onclick=\""
						+ sOnKeydown
						+ "\"></a></td>");
			}
			else
			{
				out.println("<td " + strFirstTD + " >" + strTitle + ":&nbsp;" + "<a href=#><img name=\"" + strButtonName + "\" src='/websys/image/icon.gif' border=0 ></a></td>");
			}
	 ...
	 * @throws Exception
	 */
	public static void CreateClientCtrl1(long lOfficeID, long lCurrencyID, JspWriter out, String strFormName, String strPrefix, String strMainProperty, String strNextControls,long lClientID) throws Exception
	{
		try
		{
			//输出SQL到页面
			...
			out.println("	sql += \" from client a, office b \";");
			out.println("	sql += \" where a.nofficeid = b.id and a.nStatusID > 0\";");
			out.println("	sql += \" and a.id != "+lClientID+"\";");
			out.println(" ");
			out.println("   if (nOfficeID > 0)");
			out.println("	{");
			out.println("		sql += \" and a.nofficeid = \" + nOfficeID; ");
	...
	public static void showApprovalZoomCtrl(JspWriter out,String strMagnifierName, String strFormName, String strNextControls,String[] strTitles, String[] strReturnNames,String[] strMainNames,String[] strDisplayNames) throws IOException, IException
	{
		
		
		String strTmp = "iTreasury-system";
		String pathParam = strTmp+"/magnifier/ShowApprovalMagnifierZoom.jsp?";
		if(strFormName!=null)
		{
		  pathParam+="strFormName="+strFormName;
		  
		}else{
			throw new IException(ZOOMERRORMSG);
		}
		pathParam+="&strNextControls="+strNextControls;
		pathParam+="&strMagnifierName="+strMagnifierName;
		String sFeatures = null;
		if(strDisplayNames.length < 3)
		{
			sFeatures = "toolbar=no,menubar=no,resizable=no,location=no,width=520,height=450,scrollbars=yes";
		...
		if(strDisplayNames!=null && !strDisplayNames.equals(""))
		{
			for(int i = 0;i<strDisplayNames.length;i++)
			{
				pathParam+="&strDisplayNames="+strDisplayNames[i];
		...
		if(strReturnNames!=null && !strReturnNames.equals(""))
		{
			for(int j=0;j<strReturnNames.length;j++)
			{
				pathParam+="&strReturnNames="+strReturnNames[j];
			}
		}else{
			throw new IException(ZOOMERRORMSG);
		}
		
		if(strMainNames!=null && !strMainNames.equals(""))
		{
			for(int j=0;j<strMainNames.length;j++)
			{
				pathParam+="&strMainNames="+strMainNames[j];
			}
		}else{
			throw new IException(ZOOMERRORMSG);
		}
		...
		String sOnKeydown = "window.open('"+Env.URL_PREFIX+"/"+pathParam+"','SelectAnything','"+sFeatures+"',false)";
		if(strTitles!=null && strTitles.length==strMainNames.length)
		{
		   for(int j = 0;j<strMainNames.length;j++)
		   {  
			   //out.println("<tr>");
		       out.println("<td>"+strTitles[j]+"&nbsp;<img name=\"image\" src='/webloan/image/icon.gif' style=\"cursor: hand;\" border=0 onclick=\""+sOnKeydown+"\"></td>");
		       out.println("<td><input type=\"text\" name=\""+ strMainNames[j] + "\""+" onKeyDown=\"if(event.keyCode==13) "+sOnKeydown+"\" /></td>");
		       //out.println("</tr>");
		   }
		}else{
			
			throw new IException("strTitles的长度必须和strMainNames相等");
		}
		...
		for(int i = 0;i<strReturnNames.length;i++)
		{
		   //out.println("<tr>");
		   //out.println("<td>");
		   out.println("<input type=\"hidden\" name=\""+ strReturnNames[i] + "\" value=\"\"/>");
		   //out.println("</td>");
		   //out.println("</tr>");
		}
	}
	
	public static void showCodingRuleZoomCtrl(JspWriter out,String strMagnifierName, String strFormName, String strNextControls,String[] strTitles, String[] strReturnNames,String[] strMainNames,String[] strDisplayNames) throws IOException, IException
	{
		
		
		String strTmp = "iTreasury-system";
		String pathParam = strTmp+"/magnifier/ShowCodingRuleMagnifierZoom.jsp?";
		if(strFormName!=null)
		{
		  pathParam+="strFormName="+strFormName;
		...
		if(strTitles!=null && strTitles.length==strMainNames.length)
		{
		   ...
		   {  
			   //out.println("<tr>");
		       out.println("<td nowrap>"+strTitles[j]+"&nbsp;<img name=\"image\" src='/webloan/image/icon.gif' style=\"cursor: hand;\" border=0 onclick=\""+sOnKeydown+"\"></td>");
		       out.println("<td><input type=\"text\" name=\""+ strMainNames[j] + "\""+" onKeyDown=\"if(event.keyCode==13) "+sOnKeydown+"\" /></td>");
		       //out.println("</tr>");
		   }
		}else{
	...
	{
		String strMagnifierName = "客户";
		String strMainProperty = " maxlength='30' ";
		String strPrefix = "";
		...
		String[] strDisplayNames = { "客户编号", "客户名称","所属分支机构"};
		...
		{
			case 1:
				strSQL = "getAddClientSQL(" + lOfficeID + "," + strCtrlName + "Ctrl.value)";
				break;
			case 2:
				strSQL = "getAllClientSQL(" + lOfficeID+" ,"+ strCtrlName + "Ctrl.value)";
				break;
			case 3:
				strSQL = "getCheckedClientSQL(" + lOfficeID + "," + strCtrlName + "Ctrl.value)";
			default :
				break;
		}
		...
		try
		{
			SYSMagnifier.showZoomMatchingCtrl(
				out,
				strMagnifierName,
				strFormName,
				strPrefix,
	 ...
	 * @param strRtnClientNameCtrl 返回值（客户名称）对应的控件名
	 */
	public static void createClientCtrlByOffice(
		JspWriter out,
		String OfficeCtrl,
		String strFormName,
		String strCtrlName,
		...
		String strRtnClientNameCtrl)
	{
		...
		strSQL = "getAllClientSQL(" + OfficeCtrl+".value ,"+ strCtrlName + "Ctrl.value)";
	...
	* @throws Exception
	*/
public static void showZoomMatchingCtrl(
	JspWriter out,
	String strMagnifierName,
	String strFormName,
	String strPrefix,
...
public static void showZoomCtrl(
	...
	try
	{
		...
		{
			out.println("<td " + strFirstTD + " >" + strTitle + ":&nbsp;" + "</td><td "	+ strSecondTD+ "><a href=#><img name=\"" + strButtonName[0] + "\" src='/websys/image/icon.gif' border=0 ></a></td>");
		}
		...
		if (blnIsOptional == true)
		{
			if (blnIsRateCtrl == true)
			{
				out.println("<td " + strThirdTD + " ><input type=\"text\" name=\"" + strMainNames[0] + "\" value=\"" + strReturnInitValues + "\" class=\"tar\" " + strMainProperty + ">%</td>");
			}
			else
			{
				out.println("<td " + strThirdTD + " ><input type=\"text\" name=\"" + strMainNames[0] + "\" value=\"" + strReturnInitValues + "\" class=\"box\" " + strMainProperty + "></td>");
			}
		}
		else
		{
			if (blnIsRateCtrl == true)
			{
				out.println(
					"<td "
						...
						+ "\" onKeyUp=\""
						+ sOnKeyUp
						+ "\">%<font color='#FF0000'> <span class=\"red\" id=\""+strMainNames[0]+"message\" >"+strMessage[0]+"</span></font></td>");
			}
			else
			{
				out.println(
...
public static void showCodingRuleZoomCtrl(JspWriter out,String strMagnifierName, String strFormName, String strNextControls,String[] strTitles, String[] strReturnNames,String[] strMainNames,String[] strDisplayNames,String[] strButtonNames,String strMessage) throws IOException, IException
{
	...
	if(strTitles!=null && strTitles.length==strMainNames.length)
	{
	   ...
	   {  
		   //out.println("<tr>");
	       out.println("<td nowrap>"+strTitles[j]+"&nbsp;</td><td nowrap><img name="+ strButtonNames[j] + " src='/webloan/image/icon.gif' style=\"cursor: hand;\" border=0 onclick=\""+sOnKeydown+"\"></td>");
	       out.println("<td><input type=\"text\" name=\""+ strMainNames[j] + "\""+" onKeyDown=\"if(event.keyCode==13) "+sOnKeydown+"\" /><font color='#FF0000'> <span class=\"red\" id=\""+strMainNames[j]+"message\" >"+strMessage+"</span></font></td>");
	       //out.println("</tr>");
	   }
	}else{
