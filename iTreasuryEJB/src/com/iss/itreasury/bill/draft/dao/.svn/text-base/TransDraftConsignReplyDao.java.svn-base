...
public final class TransDraftConsignReplyDao extends BillDAO {
     public TransDraftConsignReplyDao() {
        super("Bill_TransDraftConsignReply");
        super.setUseMaxID();
    }
     ...
     public Vector queryDraftConsignReplyByCondition(MultiBillQueryCondition multiBillQueryCondition) throws BillException{
     	...
     	try {
     		initDAO();			
 	    	String sql="select consignReply.* from BILL_TRANSDRAFTCONSIGNREPLY consignReply,LOAN_DISCOUNTCONTRACTBILL bill \n" +
 					" where consignReply.BILLID=bill.ID and consignReply.INPUTUSERID="+multiBillQueryCondition.getNInputUserID()+" \n"+
 	    			" and bill.NSTATUSID="+Constant.RecordStatus.VALID+" and consignReply.STATUSID="+Constant.RecordStatus.VALID+" \n";
 	    	...
 	    	if(multiBillQueryCondition.getDtEndEnd()!=null)
 	    		sql+=" and bill.DTEND <= ? \n";
 	    	if(multiBillQueryCondition.getDtConsignStart()!=null)
 	    		sql+=" and consignReply.AMOUNTRECEIVEDATE >= ? \n";
 	    	if(multiBillQueryCondition.getDtConsignEnd()!=null)
 	    		sql+=" and consignReply.AMOUNTRECEIVEDATE <= ? \n";
 			...
 			{	
 				case 0: OrderStr=" consignReply.TRANSCODE ";break;
 	    	...
 	    	this.executeQuery();
 	    	//获得实体
 	    	Iterator ResultIt=getDataEntitiesFromResultSet(new TransDraftConsignReplyInfo().getClass()).iterator();
 	    	DiscountContractBillDao discountContractBillDao=new DiscountContractBillDao();
 	    	while(ResultIt.hasNext())
 	    	{
 	    		TransDraftConsignReplyInfo transDraftConsignReplyInfo=(TransDraftConsignReplyInfo)ResultIt.next();
 	    		DiscountContractBillInfo discountContractBillInfo=(DiscountContractBillInfo)discountContractBillDao.findByID(transDraftConsignReplyInfo.getBillID(),new DiscountContractBillInfo().getClass());
 	    		DraftConsignReplyAssembleInfo draftConsignReplyAssembleInfo=new DraftConsignReplyAssembleInfo();
 	    		draftConsignReplyAssembleInfo.setDiscountContractBillInfo(discountContractBillInfo);
 	    		draftConsignReplyAssembleInfo.setTransDraftConsignReplyInfo(transDraftConsignReplyInfo);
 	    		Results.add(draftConsignReplyAssembleInfo);
 	    	}
...
// 				fieldNameArray[fieldNameArray.length-1] = lastOne;
 		...
 		/**
 	     ...
 	     protected long getMaxID()  throws ITreasuryDAOException
 	     {
 	 		...
 	 		sb.append(" select nvl(max(ID)+1,1) ID from ( " +
 	 				  " select max(ID) as id from BILL_TRANSDRAFTCONSIGN union " +
 	 				  " select max(ID) as id from BILL_TRANSDRAFTCONSIGNREPLY )");
  	...
  	public Collection findByBillID(long lBillID) throws BillException
	{
  		...
  		Collection c = null;
  		DraftConsignReplyAssembleInfo info = null;
  		TransDraftConsignReplyInfo consignReplyInfo = new TransDraftConsignReplyInfo();
		...
		consignReplyInfo.setBillID(lBillID);
		consignReplyInfo.setStatusID(Constant.RecordStatus.VALID);
  		try
		{
			c =	findByCondition(consignReplyInfo);
		...
		if (c!=null && c.size() > 0)
		{
			...
			while (it.hasNext())
			{
				consignReplyInfo = (TransDraftConsignReplyInfo)it.next();
				info = new DraftConsignReplyAssembleInfo();
				info.setTransDraftConsignReplyInfo(consignReplyInfo);
  	 ...
  	 * @throws BillException
  	 */
  public TransDraftConsignReplyInfo findDraftConsignReplyByCode(long lID) throws BillException{
	  TransDraftConsignReplyInfo info=null;
	  ResultSet rs=null;
	  try{
		  String strSQL="select replystatusid,amountreceivedate,amountreceive,refusereason,refusebookcode,abstract from Bill_TransDraftConsignReply where billid="+lID+" order by id desc";
		  System.out.println("查询托收回复信息SQL"+strSQL);
		  initDAO();
		  prepareStatement(strSQL);
		  rs=executeQuery();
		  if(rs.next()){
			  info=new TransDraftConsignReplyInfo();
			  info.setReplyStatusID(rs.getLong("replystatusid"));
			  info.setAmountReceiveDate(rs.getTimestamp("amountreceivedate"));
			  info.setAmountReceive(rs.getDouble("amountreceive"));
			  info.setRefuseReason(rs.getString("refusereason"));
			  info.setRefuseBookCode(rs.getString("refusebookcode"));
	...
	* @throws 
   */
  public DraftConsignReplyAssembleInfo findDraftConsignReplyByID(long lID) throws BillException
  {
  	//集合类
  	DraftConsignReplyAssembleInfo info = new DraftConsignReplyAssembleInfo();
  	//查找构造票据信息类实体
	...
	try {
		consignReplyInfo=(TransDraftConsignReplyInfo)consignReplyDao.findByID(lID,TransDraftConsignReplyInfo.class);
	} catch (ITreasuryDAOException e1) {
		e1.printStackTrace();
		throw new BillException("Bill_E012", e1);
	}
  	...
  	try {
		discountContractBillInfo=(DiscountContractBillInfo) discountContractBillDao.findByID(consignReplyInfo.getBillID(),DiscountContractBillInfo.class);
	} catch (ITreasuryDAOException e) {
		e.printStackTrace();
		throw new BillException("Bill_E012", e);
	}
