 ...
 */
package com.iss.itreasury.craftbrother.attach.dao;

import java.util.*;
import java.sql.*;
import com.iss.itreasury.util.*;
import com.iss.itreasury.craftbrother.attach.dataentity.*;
	...
	*/
	public Collection query(long lAttachTypeID, long lParentID) throws Exception
	{
		...
		try
		{
			...
			sbSQL.append(" FROM CRA_DOCINFO a,FileInfo b");
			...
			while (rs.next())
			{
				...
				info.setID(rs.getLong("docID")); //文件ID(表CRA_DOCINFO的ID)
	...
	*/
	public long delete(long lID) throws Exception
	{
		...
		try
		{
			...
			StringBuffer sbSQL = new StringBuffer();
			sbSQL.append(" UPDATE CRA_DOCINFO ");
	...
	public long insert(AttachInfo info) throws Exception
	{
		...
		try
		{
			if (info != null)
			{
				...
				sbSQL.setLength(0);
				sbSQL.append(" INSERT INTO CRA_DOCINFO ");
