 ...
 *
 */
public class Sett_SubAccountTypeSettingBiz {
	...
	public PagerInfo querySubAccount(long lAccountGroupID, long lAccountTypeID, long lOfficeID, long lCurrencyID) throws Exception{
		...
		try {
			...
			map.put("accountgroupid", lAccountGroupID);
			 ...
			 pagerInfo.setExtensionMothod(Sett_SubAccountTypeSettingBiz.class, "resultSetHandle",map);
	    ...
	    try
	    {
	      if (rs != null)
	      {
	        while (rs.next())
	        {
	          ...
	          PagerTools.returnCellList(cellList, strAccountCode + "," +Id);
	          if (accountTypeInfo.getIsClient() > 0) {
	        	  PagerTools.returnCellList(cellList, clientCodeByID);
	          	}
	          if (accountTypeInfo.getIsAccount() > 0) {
	        	  PagerTools.returnCellList(cellList, accountNoByID);
	          }
	          if (accountTypeInfo.getIsDeposit() > 0) {
	        	  PagerTools.returnCellList(cellList, sDepositNO);
	          }
	          ...
	          if( lAccountGroupID == SETTConstant.AccountGroupType.MARGIN )
				 {
				 
	        	  PagerTools.returnCellList(cellList, CommissionSubject);
				
				 }else {
				 
					 PagerTools.returnCellList(cellList, NegotiateInterestSubject);
				 }
