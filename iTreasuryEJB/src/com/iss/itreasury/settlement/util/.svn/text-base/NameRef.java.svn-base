...
//					.getCommonSelectList(strSQL, "ssegmentname2");
	...
	/**
	 * 功能：通过还款通知单ID获取 业务主键（合同编号+放款通知单编号+还款通知单编号）
	 * @author JBPan 20120612 
	 * @param aheadRepayID
	 * @return 合同编号-放款通知单编号-还款通知单编号
	 */
	...
	public static long getBalanceDirection(long lGLTypeID,long officeid,long currencyId) {
		long strReturn = -1;
		try {
//			凡是查询用到Sett_GlSubjectDefinition的，都改成视图sett_vglsubjectdefinition
...
//					+ subjectCode + "'";
	...
	*@version 创建时间：Mar 23, 2011 7:54:17 PM
	*@功能：通过数据来源查找数据来源的名称
	*@param lSource
	*@return
	 */
	...
	/**
	 * 功能：通过子账户ID查找合同编号 shuangniu
	 * 区别于方法getContractcodeBySubAccountID，该方法可以取到贴现的合同号
	 * 自营，委贷，贴现结息和计提打印显示合同使用
	 * @param  lSubAccountID
	 * @return String
	 */
	...
	/**
	 * 功能：根据结算交易编号查找 定期支取或者 通知支取 未到期时的活期利率，网银指令接收使用
	 * @return
	 */
	...
	/**
	 * 功能：根据保证金开立（担保收款）的主账户ID 和 保证金开立存单号 查出其对应子账户
	...
	/**
	 * 功能：通过放款通知单ID获取 业务主键 （合同编号+放款通知单编号）
	 * @author JBPan 20120612 
	 * @param payID
	 * @return 合同编号+放款通知单编号
	 */
