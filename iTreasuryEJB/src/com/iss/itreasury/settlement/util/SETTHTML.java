...
//                     + SETTConstant.SystemStatus.getName(process.getSystemStatusID(sessionMng.m_lOfficeID, sessionMng.m_lCurrencyID));
    ...
    private static void displyHtmlHeader(HtmlHeaderInfo htmlHeader) throws IException, IOException
    {
        ...
        htmlHeader.getJspWriter().println("<meta http-equiv=\"X-UA-Compatible\" content=\"IE=EmulateIE8\" > ");
		     ...
		     //   htmlHeader.getJspWriter().println("	window.parent.open('', '_self', '')");
        ...
        htmlHeader.getJspWriter().println("\n");
	        ...
	        htmlHeader.getJspWriter().println("<script language=\"JavaScript\" src=\"/websett/js/Control.js\" ></script>\n");
	        htmlHeader.getJspWriter().println("<script language=\"JavaScript\" src=\"/websett/js/SettCheck.js\" ></script>\n");
	        htmlHeader.getJspWriter().println("<script language=\"JavaScript\" src=\"/websett/js/Check.js\" ></script>\n");

	        htmlHeader.getJspWriter().println("<script language=\"JavaScript\" src=\"/websett/js/MagnifierSQL.js\" ></script>\n");
	        htmlHeader.getJspWriter().println("<script language=\"JavaScript\" src=\"/websett/js/appointAccountCtr.js\" ></script>\n");
	        ...
	        htmlHeader.getJspWriter().println("<script language=\"JavaScript\" src=\"/websett/js/CheckInput.js\"></script>\n");
	        
	        htmlHeader.getJspWriter().println("<link rel=\"stylesheet\" href=\"/websett/css/style.css\" type=\"text/css\">\n");
            ...
            boolean isNeedAll, String strProperty, boolean isNeedBlank) throws Exception
    {
        ...
        try
        {
            ...
            out.println("<select style=\"height:20px;background-color: #FFFFFF;\" name=\"" + strControlName + "\"" + strProperty + " onchange=\"onChanged(this)\">");
            if (isNeedBlank == true)
            {
                out.println("<option value=\"-1\">&nbsp;</option>");
            }
            ...
            throws Exception
    {
        ...
        sbSQL.append(" where a.nStatusID=1 and nAccountTypeID=b.id and b.nAccountGroupID = " + SETTConstant.AccountGroupType.FIXED + " and nFixedDepositMonthID < 10000 and ");
        sbSQL.append("       a.nofficeid=b.officeid and a.ncurrencyid=b.currencyID and a.nOfficeID = " + lOfficeID + " and a.nCurrencyID=" + lCurrencyID);
        sbSQL.append(" union");
   ...
   /* public static void showExtendAttributeListCtrl(JspWriter out,long ExtendAttributeTypeID,long lOfficeID, String strControlName, long lSelectValue, boolean isNeedAll, String strProperty,boolean isNeedBlank) throws Exception
    ...
    {
        StringBuffer sbSQL = new StringBuffer();
        sbSQL.append(" select id,sname from Sett_ExtendAttribute where nAttributeID= " + ExtendAttributeTypeID);
        sbSQL.append(" and nStatus=" + Constant.RecordStatus.VALID);
        //sbSQL.append(" and nOfficeID=" + lOfficeID);//表里没有这个字段
        sbSQL.append(" order by id");
        String strDisplayField = "sname";
        String strID = "id";
        showCommonListControl(out, strControlName, sbSQL.toString(), strDisplayField, strID, lSelectValue, isNeedAll, strProperty, isNeedBlank);
     ...
     * @throws Exception
     */
    public static void showFixedDepositMonthListCtrl(JspWriter out, String strControlName,long transType ,long lSelectValue, boolean isNeedAll, String strProperty,long lOfficeID,long lCurrencyID)
            throws Exception
    {
        StringBuffer sbSQL = new StringBuffer();
        sbSQL.append(" select transtime DepositMonthID,decode(transtype,2,TRANSTIME||'个月',3,(TRANSTIME-10000)||'天','') DepositMonthDesc from SETT_FixedDepositMonth  where 1=1 and statusid <> 0 ");
        sbSQL.append(" and OfficeID = " + lOfficeID + " and CurrencyID=" + lCurrencyID);
        if(transType>0)
        {
        	sbSQL.append(" and transtype = "+transType);
        }
        sbSQL.append(" order by transtime asc");
        String strDisplayField = "DepositMonthDesc";
        String strID = "DepositMonthID";
        showCommonListControl(out, strControlName, sbSQL.toString(), strDisplayField, strID, lSelectValue, isNeedAll, strProperty, true);
    }
     ...
     * @throws Exception
     */
    public static void showFixedDepositAccountListCtrl(JspWriter out, String strControlName,long accountGroupType ,long lSelectValue, boolean isNeedAll, String strProperty,long lOfficeID,long lCurrencyID,long lClientID)
            throws Exception
    {
        StringBuffer sbSQL = new StringBuffer();
        //long lon = SETTConstant.AccountStatus.NORMAL;
        
        sbSQL.append(" select t1.saccountno saccountno,t1.id id \n");
        sbSQL.append(" from sett_account t1,sett_accounttype t2 \n");
        sbSQL.append(" where t1.naccounttypeid = t2.id \n");
        sbSQL.append(" and  t1.ncurrencyid = " + lCurrencyID + "\n");
        sbSQL.append(" and t1.nclientid = " + lClientID + "\n");
        sbSQL.append(" and t1.nstatusid = " + SETTConstant.AccountStatus.NORMAL + "\n");
        if(accountGroupType>0)
        {
        	sbSQL.append(" and t2.naccountgroupid = " + accountGroupType);
        }
        sbSQL.append(" order by t1.saccountno asc \n");
        String strDisplayField = "saccountno";//sql语句返回的字段名
        String strID = "id";//sql语句返回的字段名
        showCommonListControl(out, strControlName, sbSQL.toString(), strDisplayField, strID, lSelectValue, isNeedAll, strProperty, true);
    }
    
    public static void showEnterpriseTypeListCtrl(JspWriter out, String strControlName, long lSelectValue, boolean isNeedAll, String strProperty,long lOfficeID,long lCurrencyID,int sqlType)
    throws Exception
	{
		StringBuffer sbSQL = new StringBuffer();
		switch (sqlType) {
		case 1:
			sbSQL.append(" select id ,Name as value from client_ExtendAttribute where AttributeID=1 and statusid=1 and officeid = " + lOfficeID+" and currencyID =" +lCurrencyID);
			break;
		case 2:
			sbSQL.append(" select id ,Name as value from client_ExtendAttribute where AttributeID=1 and statusid=2 and officeid = " + lOfficeID+" and currencyID =" +lCurrencyID);
			break;
		case 3:
			sbSQL.append(" select id ,Name as value from client_ExtendAttribute where AttributeID=1 and statusid=3 and officeid = " + lOfficeID+" and currencyID =" +lCurrencyID);
			
			break;
		case 4:
			sbSQL.append(" select id ,Name as value from client_ExtendAttribute where AttributeID=1 and statusid=4 and officeid = " + lOfficeID+" and currencyID =" +lCurrencyID);
			break;
		case 5:
			sbSQL.append(" select id ,Name as value from client_ExtendAttribute where AttributeID=1 and statusid=5 and officeid = " + lOfficeID+" and currencyID =" +lCurrencyID);
			break;
		case 6:
			sbSQL.append(" select id ,Name as value from client_ExtendAttribute where AttributeID=1 and statusid=6 and officeid = " + lOfficeID+" and currencyID =" +lCurrencyID);
			break;
		}
		String strDisplayField = "value";
		String strID = "id";
		showCommonListControl(out, strControlName, sbSQL.toString(), strDisplayField, strID, lSelectValue, isNeedAll, strProperty, true);
	}
}