...
public class Sett_ClientDAO extends SettlementDAO
{
	...
	public String getNewClientCode(long lOfficeID) throws Exception
	{
		...
		try
		{
			...
			//sb.append("        (	select to_number(substr(saccountno,7,4)) no from sett_Account  where  1=1 \n");
			...
			//sb.append("        		where  1=1 and ascii(substr(scode,4,1)) > 47 and ascii(substr(scode,4,1)) < 58 \n");
	...
	public Collection findByCondition(QueryClientConditionInfo qcci) throws Exception
	{
		...
		try
		{
			...
			sbSQL.append(" where  (c.ISINSTITUTIONALCLIENT <=0 or c.ISINSTITUTIONALCLIENT is null ) and nStatusID = " + SETTConstant.RecordStatus.VALID);
			...
			sbSQLOther.append(" where  (c.ISINSTITUTIONALCLIENT <=0 or c.ISINSTITUTIONALCLIENT is null ) and nStatusID = " + SETTConstant.RecordStatus.VALID);
			//appends TypeID to the query where condition
			if (qcci.getOfficeID() > 0)
			{
			...
			switch ((int) lOrder)
			{
				...
				case 7 :
					...
					break;
				case 8 :
					sbSQLOther.append(" order by nOfficeID ");
					break;
				default :
					sbSQLOther.append(" order by sCode ");
			}
	...
	private void getInfoFromResultSet(ClientInfo ci, ResultSet rs) throws Exception
	{
		try
		{
			...
			ci.setIndustryTypeID(rs.getLong("nindustrytypeid")); // 行业分类
