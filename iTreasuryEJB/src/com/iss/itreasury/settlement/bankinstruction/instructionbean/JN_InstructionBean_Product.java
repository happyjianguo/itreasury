 ...
 * @author 王忠成
 * 
 * 增加原因:京能项目采用了工行二级户模式，以及其它原因，对结算柜台业务创建指令的业务逻辑作了比较大的改动，
 * 为了减少风险，特将此类从原来的InstructionBean_Product分开
 * 
 * 相对于InstructionBean_Product修改的主要功能点：
 * 1.实现了对工行二级户模式的支持；
 * 2.如果付款方是门户模式的内部户，收款方是二级户模式的内部户，在原来的实现类里，需要设置门户模式的内部户在该业务下发指令，
 * 	 这笔指令才能产生，这个问题在这个类里，已修改；
 * 3.当付款方是开户行时，增加判断此开户行是否发送银行指令，
 * 	 当开户行发送银行指令设置为“否”的情况时（位置：结算－设置－开户行编码设置），此笔指令是不发送的，修改适用范围：
 *	 	1.开户行（付）－－开户行（收）（特殊业务）
		2.开户行（付）－－外部账户（收）（存在此情况的所有业务）
		注:开户行（付）－－活期户（收）（此情况暂时没有修改）
   3.实现了付款方对应了工行的银行账户时，汇款速度都为“加急”，该功能可配；
   4.修改组装指令判断条件，付款或收款方时活期户时，才判断其账户模式；开户行和外部户的情况不判断账户模式
 *
 */
public class JN_InstructionBean_Product implements IBankInstruction{
	 ...
	 * 
	 */
	public JN_InstructionBean_Product() { 
		super();
	}
...
//	 * @throws Exception
//	 */
//	private boolean isCreateBankInstructionByBankId(
//			BankInstructionSettingInfo info, long bankId) throws Exception {
//		boolean blnReturn = false;
//		try {
//			blnReturn = new BankInstructionSettingDelegation().isSendByBankId(
//					info, bankId);
//		} catch (Exception e) {
//			throw e;
//		}
...
//	 * @throws Exception
//	 */
//	private long getAccountModuleByAccountId(long accountId) throws Exception {
//		long lnReturn = -1;
//		try {
//			lnReturn = new BankInstructionSettingDelegation()
//					.getAccountModuleByAccountId(accountId);
//		} catch (Exception e) {
//			throw e;
//		}
//		return lnReturn;
...
//	 * @throws Exception
//	 */
//	private long getAccountModuleByBankId(long bankId) throws Exception {
//		long lnReturn = -1;
//		try {
//			lnReturn = new BankInstructionSettingDelegation()
//					.getAccountModuleByBankId(bankId);
//		} catch (Exception e) {
//			throw e;
//		}
//		return lnReturn;
//	}
	...
	public void createBankInstruction(CreateInstructionParam param) throws IException {
		...
		log.info("BankType："+param.getBankType());
		log.info("CheckUserID："+param.getCheckUserID());
		log.info("CurrencyID："+param.getCurrencyID());
		log.info("getOfficeID："+param.getOfficeID());
		log.info("TransactionTypeID："+param.getTransactionTypeID());
		log.info("TransNo："+param.getTransNo());
		...
		try {
         ...
         //形成转贴现发放指令
			...
			log.error("结算柜台业务，生成银行转账指令失败，原因：" + e.getMessage());
            e.printStackTrace();
            throw new IException(e.getMessage());
		}
	}		
					...
					if(_bankInstructionSettingInfo.getAccountModule() == SETTConstant.AccountModule.TWOLEVEL)
					{
						...
						if(_bankInstructionSettingInfo.getPayModule() == SETTConstant.PayModule.COMMON_PAY)
						{
							//付款方：活期账户对应的银行二级账户；收款方：银行二级账户对应的上级银行账户
								...
								bankInstructionInfo.setAmount(UtilOperation.Arith.round(transInfo.getCorpusAmount(),2)+UtilOperation.Arith.round(transInfo.getInterestAmount(),2)-UtilOperation.Arith.round(transInfo.getBailAmount(),2));				
		...
		try{
			...
			if (subAccountInfo.getSubAccountLoanInfo().getPaySuretyAccountID()>0&& subAccountInfo.getSubAccountLoanInfo().getReceiveSuretyAccountID()>0) {
				...
				if((bankInstructionSettingInfo.getIsSend() == Constant.TRUE && bankInstructionSettingInfo1.getIsSend() == Constant.TRUE)
					&&(bankInstructionSettingInfo.getAccountModule() == SETTConstant.AccountModule.TWOLEVEL && bankInstructionSettingInfo1.getAccountModule() == SETTConstant.AccountModule.TWOLEVEL))
				{
    	...
    	try {
			...
			if(transInfo.getNpaybankid()>0 && transInfo.getNreceivebankid()>0) { 
				...
				if(_bankInstructionSettingInfo.getIsSend() == Constant.TRUE)
				{
						...
						if(branchInfo_pay.getIsAutoVirementByBank()==Constant.YesOrNo.YES)
						{
								...
								Bs_BankAccountInfoDAO bdao = new Bs_BankAccountInfoDAO();
								//取开户行设置对应的账户名称 modify by zcwang 2009-03-25
								//String accname = bdao.getAcctNameByNo(branchInfo_pay.getBankAccountCode());
								//bankInstructionInfo.setPayAccountName(accname);//付款账户名称    				
								...
								//取开户行设置对应的账户名称 modify by zcwang 2009-03-25
								//accname = bdao.getAcctNameByNo(branchInfo_receive.getBankAccountCode());
								//bankInstructionInfo.setReceiveAccountName(accname);//收款账户名称  
								...
								//bankInstructionInfo.setReceiveBranchName(branchInfo_receive.getEnterpriseName());
			...
			if((transInfo.getNpaybankid()>0 && transInfo.getNreceiveaccountid()>0)||(transInfo.getNpayaccountid()>0 && transInfo.getNreceivebankid()>0)) {
				...
				if (transInfo.getNpaybankid()>0 && transInfo.getNreceiveaccountid()>0)
				{
					...
					/**二级户账户模式*/
					if(_bankInstructionSettingInfo1.getAccountModule() == SETTConstant.AccountModule.TWOLEVEL)
						{
					...
					if(_bankInstructionSettingInfo.getIsSend() == Constant.TRUE&&_bankInstructionSettingInfo1.getIsSend() == Constant.TRUE)
					{
						...
						if(_bankInstructionSettingInfo1.getAccountModule() == SETTConstant.AccountModule.TWOLEVEL){
							...
							if(_bankInstructionSettingInfo1.getPayModule() == SETTConstant.PayModule.COMMON_PAY){
								...
								//取开户行设置对应的账户名称 modify by zcwang 2009-03-25
								//bankInstructionInfo.setReceiveAccountName(accname); //账户名称	
					...
					BankInstructionSettingInfo _bankInstructionSettingInfo = getSettingInfoByTransType(bankInstructionSettingQueryInfo);
						...
						{ 
							...
							if(branchInfo.getIsAutoVirementByBank()==Constant.YesOrNo.YES)
							{
								...
								bankInstructionInfo.setPayDepartmentName(branchInfo.getBankServiceName());//付款人单位全称           
		//						bankInstructionInfo.setPayBranchName(branchInfo.getBankServiceName());//付款人开户行名称
								//2007/04/27 modified by mzh_fu 根据项目经理胡志强的意见:以branch表的账户名称作为这里的开户行名称
								bankInstructionInfo.setPayBranchName(branchInfo.getEnterpriseName());
    		...
    		if(transInfo.getSignBillAccountID()  > 0 && transInfo.getInterestOfSign() > 0)
			{
    			...
    			if(_bankInstructionSettingInfo.getIsSend() == Constant.TRUE) 
    			{
					...
					if(_bankInstructionSettingInfo.getAccountModule() == SETTConstant.AccountModule.TWOLEVEL){
	    				...
	    				ReqGetGeneralBankAcctInfo instruction=new ReqGetGeneralBankAcctInfo();
		    			instruction.setSystemId(1);//系统id
		    			instruction.setReferenceCode(accounthome.findAccountByID(transInfo.getSignBillAccountID()).getAccountNo());
		    			RespGetGeneralBankAcctInfo  generalbankacctinfo = BPClientAgent.getGeneralBankAcctInfo(instruction);
	    				instruction.setSystemId(1);//系统id
				...
				if(info.getCapitalAndInterstDealway()==SETTConstant.CapitalAndInterestDealWay.DIVIDE_DEAL)
				{
					...
					else if(info.getBankID()>0 && info.getConsignDepositAccountID() > 0)
					{
						...
						if((_bankInstructionSettingInfo.getIsSend() == Constant.TRUE&& _bankInstructionSettingInfo1.getIsSend() == Constant.TRUE)
							...
							if(_bankInstructionSettingInfo1.getPayModule() == SETTConstant.PayModule.COMMON_PAY)
								{
								//逻辑：付款方:财务公司开户行；收款方：活期户对应的银行二级户
								...
								if(_bankInstructionSettingInfo.getAccountModule() == SETTConstant.AccountModule.TWOLEVEL){
									...
									if(_bankInstructionSettingInfo.getPayModule() == SETTConstant.PayModule.COMMON_PAY)
										...
										//逻辑：付款方: 内部活期账户对应的银行二级户；收款方：银行二级户对应的上级银行账户
											...
											//金额，币种 等其他信息
											bankInstructionInfo.setTransactionNo(info.getTransNo());
											bankInstructionInfo.setAmount(UtilOperation.Arith.round(info.getRealInterest(),2)+UtilOperation.Arith.round(info.getRealCompoundInterest(),2)+UtilOperation.Arith.round(info.getOverDueInterest(),2) );				
											bankInstructionInfo.setCurrencyID(info.getCurrencyID());
											bankInstructionInfo.setOfficeId(info.getOfficeID());
											bankInstructionInfo.setAbstract(info.getAbstract());
											bankInstructionInfo.setCreateTime(Env.getSystemDateTime());
									...
									if(_bankInstructionSettingInfo.getAccountModule() == SETTConstant.AccountModule.TWOLEVEL){
										...
										if(_bankInstructionSettingInfo.getPayModule() == SETTConstant.PayModule.COMMON_PAY)
											...
											//逻辑：付款方: 银行二级户对应的上级银行账户；收款方：内部活期账户对应的银行二级户
												...
												//金额，币种 等其他信息
												bankInstructionInfo.setTransactionNo(info.getTransNo());
												bankInstructionInfo.setAmount(UtilOperation.Arith.round(info.getRealInterest(),2)+UtilOperation.Arith.round(info.getRealCompoundInterest(),2)+UtilOperation.Arith.round(info.getOverDueInterest(),2)-UtilOperation.Arith.round(info.getRealInterestTax(),2) );				
												bankInstructionInfo.setCurrencyID(info.getCurrencyID());
												bankInstructionInfo.setOfficeId(info.getOfficeID());
												bankInstructionInfo.setAbstract(info.getAbstract());
												bankInstructionInfo.setCreateTime(Env.getSystemDateTime());
							...
							{ 
								...
								if(_bankInstructionSettingInfo.getAccountModule() == SETTConstant.AccountModule.TWOLEVEL){
									...
									if(_bankInstructionSettingInfo.getPayModule() == SETTConstant.PayModule.COMMON_PAY)
									{
										...
										bankInstructionInfo.setTransType(TransType.NORMAL);
										//金额，币种 等其他信息
										bankInstructionInfo.setAmount(UtilOperation.Arith.round(info.getAmount(),2)+UtilOperation.Arith.round(info.getRealInterest(),2)+UtilOperation.Arith.round(info.getRealCompoundInterest(),2)+UtilOperation.Arith.round(info.getOverDueInterest(),2));				
										bankInstructionInfo.setCurrencyID(info.getCurrencyID());
										bankInstructionInfo.setOfficeId(info.getOfficeID());
										bankInstructionInfo.setAbstract(info.getAbstract());
										bankInstructionInfo.setCreateTime(Env.getSystemDateTime());
							...
							{ 
								...
								if(_bankInstructionSettingInfo.getAccountModule() == SETTConstant.AccountModule.TWOLEVEL){
									...
									if(_bankInstructionSettingInfo.getPayModule() == SETTConstant.PayModule.COMMON_PAY)
									{
										...
										//逻辑：付款方: 银行二级账户得上级银行账户；收款方：内部活期账户对应的银行二级户
										RespGetGeneralBankAcctInfo  generalbankacctinfo = new RespGetGeneralBankAcctInfo();
										ReqGetGeneralBankAcctInfo instruction=new ReqGetGeneralBankAcctInfo();
										instruction.setSystemId(1);
										instruction.setReferenceCode(accounthome.findAccountByID(info.getConsignDepositAccountID()).getAccountNo());
										...
										bankInstructionInfo.setTransType(TransType.NORMAL);
										//金额，币种 等其他信息
										bankInstructionInfo.setAmount(UtilOperation.Arith.round(info.getAmount(),2)+UtilOperation.Arith.round(info.getRealInterest(),2)+UtilOperation.Arith.round(info.getRealCompoundInterest(),2)+UtilOperation.Arith.round(info.getOverDueInterest(),2)-UtilOperation.Arith.round(info.getRealInterestTax(),2));				
										bankInstructionInfo.setCurrencyID(info.getCurrencyID());
										bankInstructionInfo.setOfficeId(info.getOfficeID());
										bankInstructionInfo.setAbstract(info.getAbstract());
										bankInstructionInfo.setCreateTime(Env.getSystemDateTime());
   ...
   try {
	        				...
	        				if(_bankInstructionSettingInfo.getIsSend() == Constant.TRUE) 
	        				{
		        				...
		        				if(_bankInstructionSettingInfo.getAccountModule() == SETTConstant.AccountModule.TWOLEVEL){
		        					...
		        					if(_bankInstructionSettingInfo.getPayModule() == SETTConstant.PayModule.COMMON_PAY){
		        						...
		        						bankInstructionInfo.setReceiveBranchAreaNameOfProvince(generalbankacctInfo.getAccountInfo().getBranchProvince());//收款人开户行所在地省名称
		        						bankInstructionInfo.setReceiveBranchAreaNameOfCity(generalbankacctInfo.getAccountInfo().getBranchCity());//收款人开户行所在地城市名称
		        						bankInstructionInfo.setAmount(UtilOperation.Arith.round(transInfo.getRealInterest(),2)+UtilOperation.Arith.round(transInfo.getRealCompoundInterest(),2)+UtilOperation.Arith.round(transInfo.getRealOverDueInterest(),2) );	
		        						bankInstructionInfo.setTransType(TransType.NORMAL);
				        				//币种 等其他信息
				        				bankInstructionInfo.setTransactionNo(transInfo.getTransNo());
				        				bankInstructionInfo.setCurrencyID(transInfo.getCurrencyID());
    				...
    				if(_bankInstructionSettingInfo.getIsSend() == Constant.TRUE) 
    				{
	    				...
	    				if(_bankInstructionSettingInfo.getAccountModule() == SETTConstant.AccountModule.TWOLEVEL){
	    					...
	    					if(_bankInstructionSettingInfo.getPayModule() == SETTConstant.PayModule.COMMON_PAY){
	    						...
	    						bankInstructionInfo.setReceiveBranchAreaNameOfProvince(generalbankacctInfo.getAccountInfo().getBranchProvince());//收款人开户行所在地省名称
	    						bankInstructionInfo.setReceiveBranchAreaNameOfCity(generalbankacctInfo.getAccountInfo().getBranchCity());//收款人开户行所在地城市名称
	    						bankInstructionInfo.setAmount(UtilOperation.Arith.round(transInfo.getAmount(),2) + UtilOperation.Arith.round(transInfo.getRealInterest(),2)+UtilOperation.Arith.round(transInfo.getRealCompoundInterest(),2)+UtilOperation.Arith.round(transInfo.getRealOverDueInterest(),2) );	
	    						bankInstructionInfo.setTransType(TransType.NORMAL);
			    				//币种 等其他信息
			    				bankInstructionInfo.setTransactionNo(transInfo.getTransNo());
			    				bankInstructionInfo.setCurrencyID(transInfo.getCurrencyID());
					...
					if (_bankInstructionSettingInfo.getAccountModule() == SETTConstant.AccountModule.TWOLEVEL) { 						
						...
						if(_bankInstructionSettingInfo.getPayModule() == SETTConstant.PayModule.COMMON_PAY)
						{
							...
							if(transInfo.getNIsReturned() > 0){
								...
								double disAmount = UtilOperation.Arith.round(transInfo.getMReturnedAmount(),2)+UtilOperation.Arith.round(transInfo.getMOverDueInterest(),2);
								BankInstructionInfo bankInstructionInfo=new BankInstructionInfo();
    ...
    private Collection createWithdrawFixedDepositIntruction(CreateInstructionParam param) throws IException {	
    	...
    	try {
    		...
    		else
    		{
	    				...
	    				if(_bankInstructionSettingInfo.getAccountModule() == SETTConstant.AccountModule.TWOLEVEL){
	    					...
	    					if(_bankInstructionSettingInfo.getPayModule() == SETTConstant.PayModule.COMMON_PAY){
	    	        			...
	    	        			if (transInfo.getTransactionTypeID() == SETTConstant.TransactionType.FIXEDTOCURRENTTRANSFER)
	    	            		{
	    	            			//定期转活期（定期支取）
	    	            			log.info("------定期转活期--------");
	    	            			transAmount = UtilOperation.Arith.round(transInfo.getDrawAmount(),2)+UtilOperation.Arith.round(transInfo.getTotalInterest(),2);
	    	            		}
	    	            		else if (transInfo.getTransactionTypeID() == SETTConstant.TransactionType.NOTIFYDEPOSITDRAW )
	    	            		{ //通知存款支取
	    	            			log.info("------通知存款支取--------");
	    	            			transAmount = UtilOperation.Arith.round(transInfo.getDrawAmount(),2)+UtilOperation.Arith.round(transInfo.getNoticeTotalInterest(),2);
	    	            		}	
	    	        			bankInstructionInfo.setAmount(transAmount);				
	    	        			bankInstructionInfo.setCurrencyID(transInfo.getCurrencyID());
	    	        			bankInstructionInfo.setOfficeId(transInfo.getOfficeID());
    			...
    			//本金＋利息，处理方式，汇总：本金和利息的付款方是同一个银行(银行付款)
    			else if(transInfo.getBankID() >0) 
    			{
    				BankInstructionInfo bankInstructionInfo = new BankInstructionInfo();
    				BankInstructionSettingInfo bankInstructionSettingQueryInfo=new BankInstructionSettingInfo();
				...
				if(_bankInstructionSettingInfo.getIsSend() == Constant.TRUE) 
				{
					...
					if(_bankInstructionSettingInfo.getAccountModule() == SETTConstant.AccountModule.TWOLEVEL){
						...
						if(_bankInstructionSettingInfo.getPayModule() == SETTConstant.PayModule.COMMON_PAY){
							...
							//金额，币种 等其他信息
							bankInstructionInfo.setTransactionNo(transInfo.getTransNo());
							bankInstructionInfo.setAmount(UtilOperation.Arith.round(transInfo.getPayableInterest(),2) + UtilOperation.Arith.round(transInfo.getPreDrawInterest(),2));				
							bankInstructionInfo.setCurrencyID(transInfo.getCurrencyID());
							bankInstructionInfo.setOfficeId(transInfo.getOfficeID());
							bankInstructionInfo.setAbstract(transInfo.getAbstract());
							bankInstructionInfo.setCreateTime(Env.getSystemDateTime());
    	...
    	try {
			...
			else
			{
				...
				if(_bankInstructionSettingInfo_pay.getIsSend()== Constant.TRUE)
				{
					...
					if(_bankInstructionSettingInfo_pay.getAccountModule() == SETTConstant.AccountModule.TWOLEVEL)
					{
						//普通付款方式
			...
			if(id_receive>0)
			{
				...
				if(_bankInstructionSettingInfo_receive.getIsSend()== Constant.TRUE)
				{
					...
					if(_bankInstructionSettingInfo_receive.getAccountModule() == SETTConstant.AccountModule.TWOLEVEL)
					{
						//普通付款方式
    ...
    private Collection createConsignIntruction(CreateInstructionParam param) throws IException {
    	...
    	private Collection createInterestPay(CreateInstructionParam param) throws IException{
    		...
    		AccountInfo accountInfo=new AccountInfo();
			Sett_AccountDAO dao=new Sett_AccountDAO();
			//自营贷款收回,用此属性:transInfo.getReceiveSuretyAccountID();
			
			//委托贷款收回用此属性:transInfo.getReceiveInterestAccountID();
           ...
           if(transInfo.getPayInterestAccountID()>0 &&(transInfo.getRealInterest() + transInfo.getRealCompoundInterest() + transInfo.getRealOverDueInterest())>0)
           {
    		...
    		if(transInfo.getReceiveInterestAccountID()>0){
    			BankInstructionInfo bankInstructionInfo=new BankInstructionInfo();
				//added by mzh_fu 2007/03/30 是否生成指令检验
				long lAccouontID=transInfo.getReceiveInterestAccountID();
				...
				if(_bankInstructionSettingInfo.getIsSend() == Constant.TRUE) 
				{
					...
					if(_bankInstructionSettingInfo.getAccountModule() == SETTConstant.AccountModule.TWOLEVEL){
						...
						{
							//逻辑：付款方：银行二级账户的上级银行账户；收款方：活期账户对应的银行二级账户
							accountInfo=dao.findByID(transInfo.getReceiveInterestAccountID());	
			    			log.info("委托贷款收款方");
							//收款方为活期账户
							//取得上级账户信息
							ReqGetGeneralBankAcctInfo instruction=new ReqGetGeneralBankAcctInfo();
							...
							bankInstructionInfo.setTransType(TransType.NORMAL);
							//金额，币种 等其他信息
							bankInstructionInfo.setAmount(UtilOperation.Arith.round(transInfo.getRealInterest(),2) + UtilOperation.Arith.round(transInfo.getRealCompoundInterest(),2) + UtilOperation.Arith.round(transInfo.getRealOverDueInterest(),2)-UtilOperation.Arith.round(transInfo.getRealInterestTax(),2));				
							bankInstructionInfo.setCurrencyID(transInfo.getCurrencyID());
							bankInstructionInfo.setOfficeId(transInfo.getOfficeID());
							bankInstructionInfo.setAbstract(transInfo.getAbstract());
							bankInstructionInfo.setTransactionNo(transInfo.getTransNo());
        ...
        //从付息开户行到利息收入账户号(委托专用)
		else if(transInfo.getInterestBankID()>0 && transInfo.getReceiveInterestAccountID() > 0 && (transInfo.getRealInterest() + transInfo.getRealCompoundInterest() + transInfo.getRealOverDueInterest()-transInfo.getRealInterestTax())>0)
		{
				//added by mzh_fu 2007/03/30 是否生成指令检验
				/*********************begin*****************************/
				...
				if(_bankInstructionSettingInfo1.getAccountModule() == SETTConstant.AccountModule.TWOLEVEL){
					...
					if( _bankInstructionSettingInfo1.getPayModule() == SETTConstant.PayModule.COMMON_PAY)
						...
						//逻辑：付款方:财务公司开户行；收款方：银行二级户的上级银行账户
							...
							bankInstructionInfo.setPayDepartmentName(branchInfo.getBankServiceName());//付款人单位全称
							//收方信息
							bankInstructionInfo.setReceiveReferenceCode((dao.findByID(transInfo.getReceiveInterestAccountID())).getAccountNo());	
           ...
           //从活期账户收取手续费(如果存在手续费)
			if(transInfo.getPayInterestAccountID() > 0 && transInfo.getRealCommission() > 0)
			{
				long lAccouontID=transInfo.getPayInterestAccountID();
				...
				{ 
					...
					if(_bankInstructionSettingInfo.getAccountModule() == SETTConstant.AccountModule.TWOLEVEL){
							...
							if(_bankInstructionSettingInfo.getPayModule() == SETTConstant.PayModule.COMMON_PAY)
							{
								...
								instruction.setSystemId(1);//系统id
								instruction.setReferenceCode(dao.findByID(transInfo.getPayInterestAccountID()).getAccountNo());
								generalbankacctinfo = BPClientAgent.getGeneralBankAcctInfo(instruction);//获得付息账户的上级银行账号单位
								if(generalbankacctinfo==null || generalbankacctinfo.getAccountInfo()==null)
				    			{
				    				throw new Exception("未找到活期账户对应的上级账户信息");
								...
								//付方信息
								bankInstructionInfo.setPayAccountNo((dao.findByID(transInfo.getPayInterestAccountID())).getAccountNo());                                  
			...
			//从活期账户付出担保费到活期账户(付收担保费账户号都存在,自营专用)
			if(transInfo.getPayInterestAccountID() >0 && transInfo.getReceiveSuretyAccountID() > 0 && transInfo.getRealSuretyFee()>0)
			{
				BankInstructionSettingInfo bankInstructionSettingQueryInfos = new BankInstructionSettingInfo();
				bankInstructionSettingQueryInfos.setNOfficeId(param.getOfficeID());
				bankInstructionSettingQueryInfos.setNCurrencyId(param.getCurrencyID());
    				...
    				if(_bankInstructionSettingInfo.getIsSend() == Constant.TRUE)
    				{
	    				...
	    				if(_bankInstructionSettingInfo.getAccountModule() == SETTConstant.AccountModule.TWOLEVEL){
	    					...
	    					if(_bankInstructionSettingInfo.getPayModule() == SETTConstant.PayModule.COMMON_PAY){
								...
								ReqGetGeneralBankAcctInfo instruction=new ReqGetGeneralBankAcctInfo();
								instruction.setSystemId(1);
								accountInfo = accounthome.findAccountByID(transInfo.getReceiveSuretyAccountID());
								instruction.setReferenceCode(accountInfo.getAccountNo());
								RespGetGeneralBankAcctInfo generalbankacctInfo = BPClientAgent.getGeneralBankAcctInfo(instruction);
								if(generalbankacctInfo==null || generalbankacctInfo.getAccountInfo()==null)
				    			{
		...
		//从开户行付出担保费到活期账户(自营专用)
		if (transInfo.getInterestBankID() >0 && transInfo.getReceiveSuretyAccountID() > 0 && transInfo.getRealSuretyFee()>0)
		{
			BankInstructionInfo bankInstructionInfo=new BankInstructionInfo();
			BankInstructionSettingInfo bankInstructionSettingQueryInfo=new BankInstructionSettingInfo();
			bankInstructionSettingQueryInfo.setNOfficeId(param.getOfficeID());
    	...
    	try {
			...
			if(_bankInstructionSettingInfo.getAccountModule() == SETTConstant.AccountModule.TWOLEVEL){
				...
				if(_bankInstructionSettingInfo.getPayModule() == SETTConstant.PayModule.COMMON_PAY)
				{
					...
					bankInstructionInfo.setAmount(UtilOperation.Arith.round(transInfo.getInterest(),2)+UtilOperation.Arith.round(transInfo.getNegotiateInterest(),2));				
			...
			if(transInfo.getPayInterestAccountID()>0)
			{
				...
				if(_bankInstructionSettingInfo.getAccountModule() == SETTConstant.AccountModule.TWOLEVEL){
					...
					if(_bankInstructionSettingInfo.getPayModule() == SETTConstant.PayModule.COMMON_PAY)
					{
						...
						bankInstructionInfo.setPayAccountNo(accountInfo.getAccountNo());
						if(transInfo.getPayInterestAccountID()>0 && (transInfo.getInterest()>0 || transInfo.getCompoundInterest()>0 || transInfo.getForfeitInterest()>0)  && (transInfo.getInterestType() == SETTConstant.InterestFeeType.INTEREST || transInfo.getInterestType() == SETTConstant.InterestFeeType.COMPOUNDINTEREST || transInfo.getInterestType() == SETTConstant.InterestFeeType.FORFEITINTEREST)){
...
//							金额，币种 等其他信息
			...
			if(transInfo.getRecieveInterestAccountID()>0)
			{
				...
				if(_bankInstructionSettingInfo.getAccountModule() == SETTConstant.AccountModule.TWOLEVEL){
					...
					if(_bankInstructionSettingInfo.getPayModule() == SETTConstant.PayModule.COMMON_PAY)
					{
						...
						if(transInfo.getRecieveInterestAccountID()>0 && (transInfo.getInterest()>0|| transInfo.getCompoundInterest()>0 || transInfo.getForfeitInterest()>0) && (transInfo.getInterestType() == SETTConstant.InterestFeeType.INTEREST || transInfo.getInterestType() == SETTConstant.InterestFeeType.COMPOUNDINTEREST || transInfo.getInterestType() == SETTConstant.InterestFeeType.FORFEITINTEREST)){
							...
							if(transInfo.getInterestType() == SETTConstant.InterestFeeType.INTEREST)
							{
								bankInstructionInfo.setAmount(UtilOperation.Arith.round(transInfo.getInterest(),2)-UtilOperation.Arith.round(transInfo.getInterestTaxCharge(),2) );	
							}
							else if(transInfo.getInterestType() == SETTConstant.InterestFeeType.COMPOUNDINTEREST )
							{
								bankInstructionInfo.setAmount(transInfo.getCompoundInterest() );	
	...
	/**
	    ...
	    * @param stransNo 交易号
	    */
	   public void cancelCheck(String stransNo) throws SettlementException 
	   {
	     if (Config.getBoolean(Config.INTEGRATION_SERVICE_ISVALID,false) != true )
	   return ;
		...
		if (respinfo != null) {
			...
			if(resultInfo != null)
			{
				for(int i = 0;i < resultInfo.length; i++)
				{
					...
					{
						 throw new SettlementException("存在未撤销的指令,无法取消复核", null);
					}
				}
			}
		}
	  // bankInstructionInfo = bankInstrction.findBankInstructionByTransactionNoOfSett(stransNo);
	  }catch(Exception e)
	  {
	   Log.print("根据交易号查询指令时出现异常"+e.getMessage());
	   throw new SettlementException("Sett_E308", e);
	  }
	  Log.print("-------------------查询指令结束-----------------------");
//	  if (bankInstructionInfo!=null && bankInstructionInfo.length > 0)
//	  {
//	   boolean tmpBool = true;
//	   for (int i=0;i<bankInstructionInfo.length;i++)
//	   {
//	    if (bankInstructionInfo[i].getStatusID()!=SETTConstant.BankInstructionStatus.FAILED)
//	    {
//	     tmpBool = false;
//	     break;
//	    }
...
//	     Log.print("-------------当前指令状态为失败---------------");
//	     //把此指令处理结果置为已删除
//	    // Sett_BankInstructionDAO bankInstructionDAO = new Sett_BankInstructionDAO();
//	     for (int i=0;i<bankInstructionInfo.length;i++)
//	     {
//	                  bankInstructionInfo[i].setStatusID(SETTConstant.BankInstructionStatus.DELETED);
//	                  //bankInstructionDAO.update(bankInstructionInfo[i]);
//	     }
//	                 Log.print("-------------状态置为删除 成功---------------");
//	    }catch(Exception e)
//	    {
//	     Log.print("把交易指令设置为删除时出现异常");
//	     throw new SettlementException("Sett_E308", e);
//	    }
//	   }
//	   else
//	   {
//	    Log.print("指令状态不为失败,无法取消复核");
//	    throw new SettlementException("Sett_E309", null);
//	   }
//	  }
//	  else
//	  {
	    ...
	    private Collection createGrantTransDiscountIntruction(CreateInstructionParam param) throws IException {
	    	...
	    	try {
				...
				if(transInfo.getNBankID()>0 ) { 
					...
					bankInstructionInfo.setPayAccountName(branchInfo_pay.getEnterpriseName());//付款账户名称	
					bankInstructionInfo.setPayDepartmentName(branchInfo_pay.getBankServiceName());//付款人单位全称           
					bankInstructionInfo.setAmount(UtilOperation.Arith.round(transInfo.getMDiscountBillAmount(),2)-UtilOperation.Arith.round(transInfo.getMInterest(),2));//金额 
