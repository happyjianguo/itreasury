...
public class PayandrepayPlanDao {
	...
	public long add(PayandrepayPlanInfo info) throws Exception
	{
		long lret = -1;
		String strSQL = "INSERT INTO loan_PayandrepayPlan VALUES((SELECT NVL(MAX(id)+1,1) id FROM loan_PayandrepayPlan),?,?,to_date(?,'yyyy-mm-dd'),?)";
		try{
	...
	public long modify(PayandrepayPlanInfo info) throws Exception
	{
		long lret = -1;
		StringBuffer strSQL = new StringBuffer("UPDATE loan_PayandrepayPlan SET ");
	...
	{
		long lret = -1;
		StringBuffer strSQL = new StringBuffer("DELETE FROM loan_PayandrepayPlan WHERE id IN(");
	...
	public PayandrepayPlanInfo findById(long id) throws Exception
	{
		PayandrepayPlanInfo info = null;
		String strSQL = "SELECT * FROM loan_PayandrepayPlan WHERE id=?";
		try{
			conn = Database.getConnection();
			pstmt = conn.prepareStatement(strSQL);
			pstmt.setLong(1,id);
	...
	public List findAll(long id) throws Exception{
		List cret = null;
		String strSQL = "SELECT * FROM loan_PayandrepayPlan WHERE bankLoanID=? ORDER BY id DESC";
		try{
			conn = Database.getConnection();
			pstmt = conn.prepareStatement(strSQL);
			pstmt.setLong(1,id);
			...
			cret = new ArrayList();
			while(rs.next()){
				PayandrepayPlanInfo info = new PayandrepayPlanInfo();
				info.setId(rs.getLong("id"));
				info.setBankLoanID(rs.getLong("bankLoanID"));
				info.setAmount(rs.getDouble("amount"));
				info.setExecuteDate(rs.getDate("executeDate").toString());
