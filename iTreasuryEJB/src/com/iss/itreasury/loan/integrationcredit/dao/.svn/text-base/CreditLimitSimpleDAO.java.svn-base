 ...
 * Window>Preferences>Java>Code Generation>Code and Comments
 */
public class CreditLimitSimpleDAO
    extends LoanDAO {
  protected Log4j log4j = new Log4j(Constant.ModuleType.LOAN, this);
  public CreditLimitSimpleDAO() {
    super("loan_creditLimit");
  }

  public CreditLimitSimpleDAO(Connection conn) {
 ...
 * @throws SQLException 
   ...
   * @throws ITreasuryDAOException
   */
  public double getClientLoanApplyAmount(CreditLimitInfo creditLimitInfo, boolean isHistory)
  	throws ITreasuryDAOException{
        ...
        try{
        	...
        	if(creditLimitInfo.getCreditModeID() == LOANConstant.CreditMode.SINGLECORP){
        		...
        		//集团授信查找客户自己和下属单位所有的信息
   	            strSQL += " and a.nBorrowClientID in (select b.id from client_clientinfo b,client_clientinfo a where b.levelcode like a.levelcode || '%' and a.id = "+ creditLimitInfo.getClientID() +" and (b.levelid = a.levelid + 1 or b.levelid = a.levelid))";
        	}
        	...
        	if(isHistory == true){
			    if(creditLimitInfo.getStartDate()!=null){
   ...
   * @throws ITreasuryDAOException
   */
  public double getClientLoanContractAmount(CreditLimitInfo creditLimitInfo, boolean isHistory)
  	throws ITreasuryDAOException{
   ...
   * @throws ITreasuryDAOException
   */
  public double getClientCircleLoanAmount(CreditLimitInfo creditLimitInfo, boolean isHistory)
  	throws ITreasuryDAOException{
        	...
        	if(creditLimitInfo.getCreditTypeID() == LOANConstant.CreditProduct.ZY || creditLimitInfo.getCreditTypeID() == LOANConstant.CreditProduct.ZHSX)
        	{
	        	...
	        	if(creditLimitInfo.getCreditModeID() == LOANConstant.CreditMode.SINGLECORP){
	        		...
	        		//集团授信查找客户自己和下属单位所有的信息
	   	            ...
	   	            strSQL += " and b.nBorrowClientID in (select b.id from client_clientinfo b, client_clientinfo a where b.levelcode like a.levelcode || '%' and a.id = "+ creditLimitInfo.getClientID() +" and (b.levelid = a.levelid + 1 or b.levelid = a.levelid))";
	        	}
	            //非循环贷款时还款单金额
    ...
    try {
          ...
          try{
	          ...
	          if(qInfo.getInputDate() != null){
			      strSQL += " and STARTDATE <= to_date('"+DataFormat.formatDate(qInfo.getInputDate())+"','yyyy-mm-dd') ";
			      strSQL += " and ENDDATE >= to_date('"+DataFormat.formatDate(qInfo.getInputDate())+"','yyyy-mm-dd') ";
	          }
