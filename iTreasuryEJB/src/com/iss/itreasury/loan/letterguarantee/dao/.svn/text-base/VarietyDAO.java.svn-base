package com.iss.itreasury.loan.letterguarantee.dao;
...
public class VarietyDAO extends LoanDAO{
	...
	public boolean isUsed(String code) throws Exception
	{
		...
		try
        {
            strSQL.append("select count(*) from LOAN_LETTER_GUARANTEE where NVARIETY=?");     
            log4j.print( strSQL.toString());	
            //获得数据库连接，初使化PreparedStatement
            conn = Database.getConnection();
            pstm = conn.prepareStatement(strSQL.toString());
            ...
            {
            	yes = false;
            	log4j.print("The variety is not used!");
            }
            else
            {
            	yes = true;
            	log4j.print("The variety is used!");
            }  
            finalize();
        }
        catch(Exception e)
