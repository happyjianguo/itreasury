...
import com.iss.itreasury.loan.integratedCredit.customerfeedback.dataentity.*;
...
public class ClientDao
{
	...
	/**
		...
		throws Exception,IException
		{
			...
			long lCreditLevel = clientinfo.getCreditLevelID(); //信用等级
			long lVentureLevel = clientinfo.getRiskLevelID(); //风险评级
			...
			try
			{
				...
				if (lClientID > 0) //修改
				{
					...
					if (lCreditLevel > -1)
					{
						ps.setLong(nIndex, lCreditLevel); //32
						nIndex++;
					}
					if (lManagerDeptID > 0)
					{
