...
import  com.iss.itreasury.loan.integratedCredit.customerfeedback.dataentity.ClientOtherShareInfo;
	...
	public LoanClientInfo findClientByID(long lClientID) throws Exception {
		...
		try {
			...
			if (rs != null && rs.next()) {
				...
				//ci.setStatutoryotherduties(rs.getString("STATUTORYOTHERDUTIES"));// 法定代表人其它职务
				//ci.setStatutorysphone(rs.getString("STATUTORYSPHONE"));// 法定代表人电话
				//ci.setSauthorizedagentname(rs.getString("SAUTHORIZEDAGENTNAME"));// 授权代理人姓名
				...
				ci.setScorporatequalifications(rs.getString("scorporatequalifications"));// 学历
				...
				//ci.setMilitaryandciviliangoods(rs.getLong("militaryandciviliangoods"));// 军民品
				...
				//ci.setTheremaininginvestors(rs.getLong("theremaininginvestors"));// 其余投资人
				...
				//ci.setOrganizationalstructure(rs.getString("organizationalstructure"));// 关于组织结构其他需要说明的事项
				...
				//ci.setNupportthedevelopment(rs.getLong("nupportthedevelopment"));
				...
				ci.setNClienttypeID1(rs.getLong("nClienttypeID1"));//客户属性1
				ci.setNClienttypeID2(rs.getLong("nClienttypeID2"));//客户属性2
				ci.setNExtendattribute1(rs.getLong("nExtendattribute1"));//客户扩展属性1
				ci.setNExtendattribute2(rs.getLong("nExtendattribute2"));//客户扩展属性2
