 ...
 * @author yfan
 */
public class LoanCommonSettingEJB implements SessionBean
{
		...
		throws IException,RemoteException
	{
		...
		try
		{
			...
			//modify by zcwang 如果无贷款类型，传回-1
			if(lLoanTypeTmp.length > 0)
			{
	...
	public long changeContractManager(long lContractID[], long lContractManagerID,String reason) 
    throws IException,RemoteException{
		return this.changeContractManagerImpl(lContractID, lContractManagerID, reason);
	}
	 ...
	 * @throws RemoteException
	 */
	 private synchronized long changeContractManagerImpl(long lContractID[], long lContractManagerID,String reason) 
    throws IException,RemoteException
	{
		PreparedStatement ps = null;
		ResultSet rs = null;
		...
		logInfo.setBusinessType("合同管理人权限转移");
        // add by jbpan 20120607 end
		try
		{
			logInfo.setTransCode(NameRef.getUserNameByID(lContractManagerID));  //业务主键-合同编号（合同管理员标识）
			con = Database.getConnection();
			log4j.info("更改合同管理人");
			//更改合同信息
            ...
            i = 0;
            nLength = lContractID.length;
            strSQL = "insert into loan_loanmngermodify_hst(id,SCONTRACTCODE,"
				+ " OLDUSERID,NEWUSERID,MODIFYDATE,REASON) "
				+ " values(?,?,?,?,sysdate,?)";
            while(i<nLength){
            	strSQL1 = " select nvl(max(id),0)+1 from loan_loanmngermodify_hst ";
    			ps = con.prepareStatement(strSQL1);
    			rs = ps.executeQuery();
    			if (rs.next()) {
    				lMaxID = rs.getLong(1);
    			...
    			ps.setLong(4, lContractManagerID);
    ...
    throws IException,RemoteException
	{
        ...
        LoggerBtnLevelInfo logInfo = new LoggerBtnLevelInfo();
		logInfo.setActionTypeID(Constant.LoggerOfOperationType.MODIFYSAVE);
		logInfo.setBusinessType("申请书管理人权限转移");
        // add by jbpan 20120607 end
		try
		{
			logInfo.setTransCode(NameRef.getUserNameByID(lLoanManagerID));  //业务主键-申请书管理人
			//更改申请书管理人
			con = Database.getConnection();
	 ...
	 * @throws RemoteException
	 */
	public long saveClientInfo(ClientInfo clientinfo) throws IException,
			RemoteException {
		long lReturn = -1;
		LoanCommonSettingDao loanCommonSettingDao = new LoanCommonSettingDao();
	...
	/**
	 * saveAtTermAwake 信贷部分到期业务提醒 进行到期业务提醒设置，提前XX天提醒客户,提醒XX天。
	 * 操作loan_attermawakesetting 数据表 更新相应字段 与办事处相关，与币种无关
	 * 
	 * @param lTypeID
	 *            提醒类型
	 * @param lAheadDays
	 ...
	 */
	public long saveAtTermAwake(long lTypeID[], long lAheadDays[], long lAwakeDays[], long lOfficeID) throws java.rmi.RemoteException, IRollbackException
	{
		PreparedStatement ps = null;
		ResultSet rs = null;
		Connection con = null;
	 ...
	 * @throws IRollbackException 
	 */
	public long saveAtTermAwakeSetting(long lTypeID[], long lAheadDays[], long lAwakeDays[], long lOfficeID,long lCurrencyID) throws RemoteException, IRollbackException
	{
		Connection con = null;
