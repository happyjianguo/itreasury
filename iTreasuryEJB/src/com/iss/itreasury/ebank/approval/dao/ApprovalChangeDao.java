...
import com.iss.itreasury.ebank.approval.dataentity.ApprovalChangeInfo;
...
public class ApprovalChangeDao extends Object implements java.io.Serializable
{
	...
	public Collection findApprovalItemForChange(long lApprovalID, long lOfficeID, long lCurrencyID, long lUserID, long lPageLineCount, long lPageNo, long lOrderParam, long lDesc) throws Exception
	{
		...
		try
		{
			...
			strSQL = " from ob_approvalitem aa, ob_ApprovalSetting bb, ob_Approvalchangeitem cc, ob_user dd, ob_user ee where aa.nApprovalID = bb.ID ";
			...
			System.out.println("sql1=" + strSelect + strSQL);
			...
			strSQL = " select aa.*,bb.ID,bb.sName,cc.nNewUserID,cc.dtInputDate,dd.sname as sUserName,ee.sname as sNewUserName  " + strSQL;
	...
	public ApprovalChangeInfo findChangeInfoByID(long lApprovalID, long lUserID) throws Exception
	{
		...
		{
			strSQL =
				" select aa.nLevel,bb.*,cc.nUserID,cc.nNewUserID,cc.dtInputDate,dd.sname as sUserName,ee.sname as sNewUserName "
					+ " from ob_approvalitem aa ,ob_ApprovalSetting bb,ob_Approvalchangeitem cc ,ob_user dd,ob_user ee "
	...
	public long moveCheckRight(long lApprovalID, long lOfficeID, long lCurrencyID, long lUserID, long lNewUserID) throws Exception
	{
		PreparedStatement ps = null;
		ResultSet rs = null;
		Collection c = null;
		...
		try
		{
			...
			strSQL = " select aa.* from ob_approvalitem aa,ob_ApprovalSetting bb "
					+ " where aa.napprovalid = bb.id "
					+ " and bb.nstatusid = "
					+ Constant.ApprovalStatus.SUBMIT
					+ " and aa.nuserid = "
			...
			while (rs != null && rs.next())
			{
				...
				{
					if (moveRight(lTempApprovalID, lUserID, lNewUserID) < 0)
					{
						lReturn = -1;
	...
	public long returnCheckRight(long lApprovalID, long lOfficeID, long lCurrencyID, long lUserID) throws Exception
	{
		...
		{
			//审批取消（将状态置为无效）
			strSQL = " update ob_Approvalchangeitem set nstatusid = "
					+ Constant.RecordStatus.INVALID
					+ " where nuserid = "
					+ lUserID
					+ " and napprovalid in "
					+ " (select id from ob_ApprovalSetting "
					+ " where 1=1 ";
	...
	private long checkNewUserInThisLevel(long lApprovalID, long lLevel, long lNewUserID) throws Exception
	{
		...
		try
		{
		    ...
		    //检查审批设置里该用户是不是这个级别的审核人，如果是，才允许进行权限转移
			strSQL = " select * from ob_approvalitem aa,ob_ApprovalSetting bb "
					+ " where aa.napprovalid = bb.id "
					+ " and aa.nuserid = "
					+ lNewUserID
					+ " and bb.nstatusid = "
	...
	private long IsRightMoved(long lApprovalID, long lUserID, long lNewUserID) throws Exception
	{
		...
		try
		{
			...
			strSQL = " select * from ob_Approvalchangeitem where "
					+ " nApprovalID = "
					+ lApprovalID
					+ " and nStatusID = "
					+ Constant.RecordStatus.VALID
					...
					+ lUserID
					+ " union "
					+ " select * from ob_Approvalchangeitem where "
					+ " nApprovalID = "
					+ lApprovalID
					+ " and nStatusID = "
					+ Constant.RecordStatus.VALID
	 ...
	 * @return      long
	 */
	private long moveRight(long lApprovalID, long lUserID, long lNewUserID) throws Exception
	{
		PreparedStatement ps = null;
		ResultSet rs = null;
		long lReturn = Constant.YesOrNo.NO;
		...
		try
		{
			...
			strSQL = " select * from ob_Approvalchangeitem " + " where napprovalid = " + lApprovalID + " and nUserID = " + lUserID + " and nstatusID = " + Constant.RecordStatus.VALID;
			...
			if (lTmp == 1) //新增操作
			{
				strSQL = " insert into ob_Approvalchangeitem(napprovalid," + " nuserid,nnewuserid,dtinputdate,nstatusid) " + " values(?,?,?,sysdate,?) ";
				ps = m_Conn.prepareStatement(strSQL);
			...
			{
				strSQL = " update ob_Approvalchangeitem set nnewuserid = ?, "
						+ " dtinputdate=sysdate "
						+ " where napprovalid = "
						+ lApprovalID
						+ " and nuserid = "
