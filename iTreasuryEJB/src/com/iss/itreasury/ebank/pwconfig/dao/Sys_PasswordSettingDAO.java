package com.iss.itreasury.ebank.pwconfig.dao;
...
public class Sys_PasswordSettingDAO extends ITreasuryDAO{
    ...
    //获得用户的密码和上次修改密码日期
    public List getOBUserPwdInfo(long id) throws ITreasuryDAOException, SQLException{
    	List list = new ArrayList();
    	initDAO();
    	String strSql = "select spassword,dtChangePassword from ob_user where id = "+id;
    	try{
	...
	public void changeUserPassword(long lUserID, String strPassword) throws Exception{
		...
		try{
			...
			sb.append(" update ob_user set SPASSWORD=?, DTCHANGEPASSWORD=? where id=? ");
