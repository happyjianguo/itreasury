 ...
 */
package com.iss.itreasury.securities.attach.dao;

import java.util.*;
import java.sql.*;
import com.iss.itreasury.util.*;
import com.iss.itreasury.securities.attach.dataentity.*;
...
public class AttachDao
{
	...
	public AttachDao()
	{
		log4j = new Log4j(Constant.ModuleType.SECURITIES, this);
	}
	...
	*/
	public Collection query(long lAttachTypeID, long lParentID) throws Exception
	{
		...
		try
		{
			...
			sbSQL.append(" FROM SEC_DocInfo a,FileInfo b");
			...
			while (rs.next())
			{
				...
				info.setID(rs.getLong("docID")); //文件ID(表SEC_DocInfo的ID)
	...
	*/
	public long delete(long lID) throws Exception
	{
		...
		try
		{
			...
			StringBuffer sbSQL = new StringBuffer();
			sbSQL.append(" UPDATE SEC_DocInfo ");
	...
	public long insert(AttachInfo info) throws Exception
	{
		...
		try
		{
			if (info != null)
			{
				...
				sbSQL.setLength(0);
				sbSQL.append(" INSERT INTO SEC_DocInfo ");
