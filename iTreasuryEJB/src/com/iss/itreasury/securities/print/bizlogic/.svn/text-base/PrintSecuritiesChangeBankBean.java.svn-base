 ...
 * 窗口 > 首选项 > Java > 代码生成 > 代码和注释
 */
public class PrintSecuritiesChangeBankBean {
	...
	private void getSQL(PrintSecuritiesStockChangeParam queryParam) {
		...
		sbFrom.append("  my_DailyStock.NetCost as endCostNet--实际成本(净价) --期末库存余额(已冻结)\n");//搞了半天，就是为了这个字段
	...
	private void getMyTable(
		...
		sbFrom.append("      sec_dailyStock.FrozenQuantity, --库存冻结量 \n");
		...
		sbFrom.append( "and SEC_Securities.TypeID in (10,11,12,13,14,15) \n");				
		//条件一：业务通知单录入日期开始日
		...
		sbFrom.append("     ) my_DailyStock  --银行间\n");
	}
	 ...
	 *         SecuritiesException
	 */
	public PageLoader printSecuritiesChangeBank(PrintSecuritiesStockChangeParam printParam)
