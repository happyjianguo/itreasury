...
public class SEC_ApplyDAO extends SecuritiesDAO
{
	...
	public SEC_ApplyDAO(Connection conn)
	{
		super("SEC_ApplyForm", conn);
	}
	 ...
	 * 加入金额审批效验
	 */
	public Collection findByMultiOption2(ApplyQueryInfo qInfo,long ActionID) throws SecuritiesDAOException
	{
		String sql=""; 
	...
	public Collection findByMultiOption(ApplyQueryInfo qInfo) throws SecuritiesDAOException
	{
		...
		long interestTypeID = qInfo.getInterestTypeID();
		long currencyID = qInfo.getCurrencyId();
		long acceptStatus = qInfo.getAcceptStatus();
		/*boxu add 2006-09-19*/
		double startcostAmount = 0;  //回购成本开始
		double endcostAmount = 0;  //回购成本结束
		long startterm = -1;  //开始期限
		...
		if (queryPurpose == 1) //for modify
		{
			...
			//Modified by zwsun, 2007/9/11, 对选择了状态的根据状态查询， -1 的时候查询所有状态
			if ( statusID != -1  )
			{
				strSQL += " and StatusID = " + statusID;
			}			
...
*/			
			...
			if (queryPurpose == 2) //for examine
			{
				...
				if (queryPurpose == 3) //for attornment query
				{
					...
					//恢复了贷款转让申请书编号查询
					//strSQL = " from SEC_ApplyForm aa " + " where 1=1 " + " and TransactionTypeID in("+SECConstant.BusinessType.CAPITAL_REPURCHASE.REPURCHASE_NOTIFY+","+SECConstant.BusinessType.CAPITAL_REPURCHASE.BREAK_NOTIFY+")" + " and StatusID = " + SECConstant.ApplyFormStatus.CHECKED;
					//strSQL+=" and id not in(select repurchaseApplyId from SEC_AttornmentApplyForm where statusId in (1,2,3,5)) ";
					strSQL = " from SEC_ApplyForm aa where 1=1 and aa.TransactionTypeID in(" + SECConstant.BusinessType.CAPITAL_REPURCHASE.REPURCHASE_NOTIFY + "," + SECConstant.BusinessType.CAPITAL_REPURCHASE.BREAK_NOTIFY+")" + " and StatusID = " + LOANConstant.ContractStatus.CHECK; 
				}
		...
		if (queryPurpose == 6) //for attornment query
		{
			strSelect = " select count(*) ";
			//strSQL = " from SEC_ApplyForm aa " + " where 1=1 " + " and TransactionTypeID = " + transactionTypeID + " and StatusID = " + SECConstant.ApplyFormStatus.CHECKED;
			//恢复了贷款转让申请书编号查询
			//strSQL = " from SEC_ApplyForm aa " + " where 1=1 " + " and TransactionTypeID in("+SECConstant.BusinessType.CAPITAL_REPURCHASE.REPURCHASE_NOTIFY+","+SECConstant.BusinessType.CAPITAL_REPURCHASE.BREAK_NOTIFY+")" + " and StatusID = " + SECConstant.ApplyFormStatus.CHECKED;
			//strSQL+=" and id not in(select repurchaseApplyId from SEC_AttornmentApplyForm where statusId in (1,2,3,5)) ";
			//strSQL = " from SEC_ApplyForm aa where 1=1 and aa.TransactionTypeID in(" + SECConstant.BusinessType.CAPITAL_REPURCHASE.REPURCHASE_NOTIFY + "," + SECConstant.BusinessType.CAPITAL_REPURCHASE.PAYBACK_NOTIFY + "," + SECConstant.BusinessType.CAPITAL_REPURCHASE.BREAK_NOTIFY+")" + " and StatusID = " + SECConstant.ApplyFormStatus.CHECKED;
			//查询待接收状态
			if(acceptStatus == 1)
			{
				strSQL = " from SEC_ApplyForm aa where 1=1 and StatusID = " + SECConstant.ApplyFormStatus.CHECKED;
				//Boxu Add 2008年11月12日 如果同业申请单已经使用,将不能再做贷款模块的合同转让功能
				strSQL+= " and aa.id not in ( select repurchaseApplyId from SEC_AttornmentApplyForm where statusId in ( ";
				strSQL+= " "+ LOANConstant.AttornmentStatus.SAVE +", ";
				strSQL+= " "+ LOANConstant.AttornmentStatus.SUBMIT +", ";
				strSQL+= " "+ LOANConstant.AttornmentStatus.CHECK +", ";
				strSQL+= " "+ LOANConstant.AttornmentStatus.USED +", ";
				strSQL+= " "+ LOANConstant.AttornmentStatus.APPROVALING +" ";
				strSQL+= " ) ) ";
			}
			//查询处理中状态
			if(acceptStatus == 2)
			{
				strSQL = " from SEC_ApplyForm aa where 1=1 and StatusID = " + SECConstant.ApplyFormStatus.CHECKED;
				//Boxu Add 2008年11月12日 如果同业申请单已经使用,将不能再做贷款模块的合同转让功能
				strSQL+= " and aa.id in ( select repurchaseApplyId from SEC_AttornmentApplyForm where statusId in ( ";
				strSQL+= " "+ LOANConstant.AttornmentStatus.SAVE +", ";
				strSQL+= " "+ LOANConstant.AttornmentStatus.SUBMIT +", ";
				strSQL+= " "+ LOANConstant.AttornmentStatus.APPROVALING +" ";
				strSQL+= " ) ) ";
		...
		*/
		...
		{
			strSQL += " and transactionTypeID=" + qInfo.getTransactionTypeId();
		}else if (qInfo.getTransactionTypeId()==0){
			strSQL += " and transactionTypeID in (" + SECConstant.BusinessType.CAPITAL_REPURCHASE.BREAK_NOTIFY+ ","+ SECConstant.BusinessType.CAPITAL_REPURCHASE.REPURCHASE_NOTIFY+")";
		}
	...
	public Collection findByMultiOption1(ApplyQueryInfo qInfo,long ActionID) throws SecuritiesDAOException
	{
		...
		try
		{
			...
			*/
			...
			sql += " select aa.*,rr.moneysegment,rr.approvalid from SEC_ApplyForm aa,loan_approvalrelation rr";
			//增加关于币种的判断-mhjin-东方电气
			sql += " where rr.moduleid="+Constant.ModuleType.SECURITIES+" and rr.officeid = "+officeID+" and rr.currencyid =" +currencyID+ " and aa.statusid="+SECConstant.ApplyFormStatus.SUBMITED;
			
			//可转债--债转股业务
			if ( transactionTypeID == SECConstant.BusinessType.DEBT_TO_EQUITY.DEBT_TO_EQUITY )
			{
				sql += " and aa.price>rr.moneysegment ";
			}
			else
			{
				sql += " and aa.amount>rr.moneysegment ";
			}
	 ...
	 * 
	 */
	public void doAfterCheckOver(long lApplyID) throws SecuritiesDAOException,IException,Exception
	{
		ApplyInfo aInfo = new ApplyInfo();
		try
		{
			...
			{
				//资产转让业务
				if(aInfo.getTransactionTypeId()==SECConstant.BusinessType.CAPITAL_REPURCHASE.REPURCHASE_NOTIFY||
				   aInfo.getTransactionTypeId()==SECConstant.BusinessType.CAPITAL_REPURCHASE.AVERAGE_NOTIFY||
				   aInfo.getTransactionTypeId()==SECConstant.BusinessType.CAPITAL_REPURCHASE.INVEST_BREAK||
				   aInfo.getTransactionTypeId()==SECConstant.BusinessType.CAPITAL_REPURCHASE.BREAK_NOTIFY)
				{
					//获取流水号
					HashMap map = new HashMap();
					map.put("officeID",String.valueOf(aInfo.getOfficeId()));
					map.put("currencyID",String.valueOf(aInfo.getCurrencyId()));
					map.put("moduleID",String.valueOf(Constant.ModuleType.CRAFTBROTHER));
					map.put("transTypeID",String.valueOf(CRAconstant.SameBusinessAttribute.SAME_BUSINESS));
					map.put("actionID",String.valueOf(CRAconstant.TransactionType.SAME_BUSINESS_CONTRACT));
					map.put("subActionID",String.valueOf(aInfo.getTransactionTypeId()));
					String applyCode =CreateCodeManager.createCode(map);
					aInfo.setCode(applyCode);
					
				}
				if (aInfo.getTransactionTypeId() == SECConstant.BusinessType.CAPITAL_REPURCHASE.CAPITAL_REPURCHASE
					|| aInfo.getTransactionTypeId()==SECConstant.BusinessType.CAPITAL_REPURCHASE.REPURCHASE_NOTIFY
					|| aInfo.getTransactionTypeId()==SECConstant.BusinessType.CAPITAL_REPURCHASE.AVERAGE_NOTIFY
					|| aInfo.getTransactionTypeId()==SECConstant.BusinessType.CAPITAL_REPURCHASE.BREAK_NOTIFY
	...
	private ApplyInfo doFinishApply(ApplyInfo aInfo) throws SecuritiesDAOException
	{
		...
		try
		{
			...
			if (tsTransactionEndDate!=null&&tsToday.after(tsTransactionEndDate))
			{
			...
			//则在审批完成时，系统自动将“审批完成日”定为“成交日期开始日”
				...
				if (tsTransactionStartDate!=null&&tsToday.after(tsTransactionStartDate))
				{
	 ...
	 * 
	 */
	public long doCreateContract(ApplyInfo aInfo) throws SecuritiesDAOException
	{
		System.out.println("进入声称合同方法==========================");
		...
		{
			//转让日期与回购日期之间间隔天数
			interestDays = DataFormat.getIntervalDays(aInfo
					.getTransactionStartDate(), aInfo.getTransactionEndDate());
			//转让日期到回购日期之间的利息
			interest = UtilOperation.Arith.div(UtilOperation.Arith.mul(
					UtilOperation.Arith
							.mul(aInfo.getAmount(), aInfo.getIncomeRate()),
					interestDays-1), 36000);
		}
		try
		{
			if(aInfo.getCode().equals(""))
			{
				applyCode = dao.getContractCode(aInfo.getTransactionTypeId());
			}
			else
			{
				applyCode = aInfo.getCode();
			}			
			cInfo.setCode(applyCode); //合同编号
			...
			cInfo.setPrice(aInfo.getPrice());//转让价格
			cInfo.setMargin(aInfo.getMargin());//差额for买入买断
			...
			cInfo.setBargainCode(aInfo.getBargainCode());
			
			cInfo.setTransferBalance(aInfo.getPrice());  //转让款项余额
			cInfo.setAvailableTransferBalance(aInfo.getPrice());  //转让款项可用余额
			cInfo.setRepurchaseBalance(aInfo.getPrice());  //待回购余额
			cInfo.setAvailableRepurchaseBalance(aInfo.getPrice());  //待回购可用余额
			cInfo.setInterestBalance(interest);//转让日期到回购日期之间的利息
			cInfo.setReceivedInterest(0);  //已收利息（已收款）
			cInfo.setWaitReceivedInterest(0);  //已收利息（待收款）
			cInfo.setPaidInterest(0);  //已付利息（已支付）
			cInfo.setWaitPaidInterest(0);  //已付利息（待支付）
			cInfo.setAttachId(new Date().getTime());//预留附件关联
	 ...
	 * 
	 */
/*	public long doCreateContract(ApplyInfo aInfo) throws SecuritiesDAOException,IException
	{
		System.out.println("进入声称合同方法==========================");
		//super("SEC_APPLYCONTRACT");
		SecuritiesContractDao dao = new SecuritiesContractDao();
		...
		try
		{
			...
			cInfo.setBargainCode(aInfo.getBargainCode());
			cInfo.setPrice(aInfo.getPrice());  //转让价格
			cInfo.setMargin(aInfo.getMargin()); //差价
	...
	*/
	...
	public boolean cancelContract(long applyId) throws SecuritiesDAOException{
		...
		sqlBuf.append( "update  sec_applycontract s");
		sqlBuf.append(" set s.statusid = "+SECConstant.ContractStatus.CANCEL);
	...
	public boolean checkStatuID(long applyId) throws SecuritiesDAOException{
		...
		StringBuffer sqlBuf = new StringBuffer();
		sqlBuf.append( "select count(*) from sec_applycontract s");
		sqlBuf.append(" where s.statusid in (");
		for(int i=0;i<checkStatusId.length;i++){
			sqlBuf.append(checkStatusId[i]);
			if(i!=(checkStatusId.length-1)){
	 ...
	 * @return
	 */
	public boolean checkAttornStatuID(long applyId) throws SecuritiesDAOException{
		boolean flag=false;
		long[] checkStatusId={

				LOANConstant.AttornmentStatus.SAVE,
				LOANConstant.AttornmentStatus.SUBMIT,
				LOANConstant.AttornmentStatus.APPROVALING,
				LOANConstant.AttornmentStatus.CHECK,
				};
		StringBuffer sqlBuf = new StringBuffer();
		sqlBuf.append( "select count(*) from sec_attornmentapplyform s");
		sqlBuf.append(" where s.statusid in (");
		for(int i=0;i<checkStatusId.length;i++){
			sqlBuf.append(checkStatusId[i]);
			if(i!=(checkStatusId.length-1)){
		...
		sqlBuf.append(")");
		sqlBuf.append(" and s.repurchaseapplyid = " + applyId);
		try
		{
			initDAO();
			prepareStatement(sqlBuf.toString());
	...
	/**
	 * Added by zwsun, 2007/9/19
	 * 检查交割单是否已被使用
	 * @param applyId 申请单id
	 * @return
	 */
	public boolean checkUsed(long applyId) throws SecuritiesDAOException{
		boolean flag=false;
		long[] checkStatusId={
				...
				SECConstant.DeliveryOrderStatus.APPROVALED,
				SECConstant.DeliveryOrderStatus.APPROVALING,
	 ...
	 * @throws SecuritiesDAOException
	 */
	public String checkCapitalLandingCreditExtension(long counterpartID, long transactionTypeID, double newPledgeSecuritiesAmount, long lApplyID) throws SecuritiesDAOException
	{
		StringBuffer strSQL = new StringBuffer();
		Connection con = null;
		PreparedStatement ps = null;
		...
		catch (Exception e)
		{
			...
			try {
				...
				throw new SecuritiesDAOException("数据库关闭异常发生",e);			
			}			
		}
	 ...
	 * @throws IException
	 */
	public long updateStatusID(ApplyInfo aInfo) throws Exception
	{
		long lReturn = -1;
		Connection con = null;
		PreparedStatement ps = null;
		...
		try
		{
			...
			strSQLBuffer.append(" SET StatusID = "+aInfo.getStatusId()+", updatedate = sysdate, nextcheckuserid = "+aInfo.getNextCheckUserId()+" ");
			strSQLBuffer.append(" WHERE ID = "+aInfo.getId()+" ");
	 ...
	 * @throws IException
	 */
	public long updateStatusID2(SecuritiesContractInfo ContractInfo) throws Exception
	{
		long lReturn = -1;
		Connection con = null;
		PreparedStatement ps = null;
		...
		try
		{
			con = Database.getConnection();
			StringBuffer strSQLBuffer = new StringBuffer();
			strSQLBuffer.append("UPDATE SEC_ApplyContract ");
			strSQLBuffer.append(" SET StatusID = "+ContractInfo.getStatusId()+", updatedate = sysdate, nextcheckuserid = "+ContractInfo.getNextCheckUserId()+" ");
			strSQLBuffer.append(" WHERE ID = "+ContractInfo.getId()+" ");
	 ...
	 * @throws IException
	 */
	public long updateStatusID3(long lID, long StatusID) throws Exception
	{
		long lReturn = -1;
		Connection con = null;
		PreparedStatement ps = null;
	...
	throws Exception {
      ...
      try {
	conn = Database.getConnection();
	strSQL = " update SEC_ApplyForm set StatusID=? where ID=?";
	...
	if (lResult < 0) {
		log.info("update SEC_ApplyForm property info error:" + lResult);
		return -1;
	} else {
		return loanID;
	}
     ...
     * @throws IRollbackException
     */
	public long updateAttornByID(long ApplyID)
	throws Exception {
       PreparedStatement ps = null;
       Connection conn = null;
       String strSQL = null;
      ...
      try {
	conn = Database.getConnection();
	strSQL = "update sec_attornmentapplyform m  set  m.statusid= " + LOANConstant.AttornmentStatus.CANCEL
			+"where m.repurchaseapplyid = "+ApplyID;
