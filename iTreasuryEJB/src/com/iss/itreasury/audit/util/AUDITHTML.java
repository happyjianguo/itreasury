...
public class AUDITHTML
{
       ...
       if (sessionMng.isLogin())
        {
            try
            {
                if (sessionMng.m_lModuleID == Constant.ModuleType.AUDIT){
    ...
    public static boolean validateRequest(JspWriter out, HttpServletRequest request, HttpServletResponse response) throws Exception
    {
        ...
        try
        {
            ...
            {
                AUDITHTML.showMessage(out, sessionMng, request, response, Env.getClientName(), Constant.RecordStatus.INVALID, "Gen_E002");
                AUDITHTML.showHomeEnd(out);
                out.flush();
                bResult = false;
            }
            //判断是否有权限
            ...
            {
            	AUDITHTML.showMessage(out, sessionMng, request, response, Env.getClientName(), Constant.RecordStatus.INVALID, "Gen_E003");
            	AUDITHTML.showHomeEnd(out);
                out.flush();
               bResult = false;
            }
            ...
            if (lActionID.longValue() != 0)
            {
                if (lTemp != null && lActionID.compareTo(lTemp) <= 0)
                {
                    ...
                    AUDITHTML.showMessage(out, sessionMng, request, response, Env.getClientName(), Constant.RecordStatus.INVALID, "Gen_E008");
                    AUDITHTML.showHomeEnd(out);
                    out.flush();
                    bResult = false;
                }
        ...
        {
        	AUDITHTML.showException(out, sessionMng, ie, request, response, Env.getClientName(), Constant.RecordStatus.INVALID, "");
        	AUDITHTML.showHomeEnd(out);
            out.flush();
            ie.printStackTrace();
            bResult = false;
