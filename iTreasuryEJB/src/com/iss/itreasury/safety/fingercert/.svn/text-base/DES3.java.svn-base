...
public class DES3 {
	...
	// static String key = "&*@#$H%S";

	// 密匙num
	private static int keyNum = 0;

	// 密匙数组
	static String key[] = new String[32];
	...
	private static byte[] hex2byte(String hStr) {
		String hs = "";
		String stmp = "";
		byte[] tmp_byte = new byte[hStr.length() / 2];
		byte nByte = 0;

		for (int n = 0; n < hStr.length(); n += 2) {
			stmp = hStr.substring(n, n + 2);
			nByte = java.lang.Integer.valueOf(stmp, 16).byteValue();
			tmp_byte[n / 2] = nByte;
		}
		return tmp_byte;
	}

	/** 如果key为固定的值，那么iu只需要带一个参数data即可 */
	public static String DesEncrypt(String data, String[] keys) {
		...
		byte[] bytekey1 = keys[0].getBytes();
		byte[] bytekey2 = keys[1].getBytes();
		byte[] bytekey3 = keys[2].getBytes();

		DES3 des = new DES3();

		result = des.DesEncrypt(bytekey1, bytedata, 1);
		result = des.DesEncrypt(bytekey2, result, 0);
		result = des.DesEncrypt(bytekey3, result, 1);
		return byte2hex(result);
	}

	/** 如果key为固定的值，那么iu只需要带一个参数data即可 */
	public static String UnDesEncrypt(String data, String[] keys) {
		byte[] bytekey1 = keys[0].getBytes();
		byte[] bytekey2 = keys[1].getBytes();
		byte[] bytekey3 = keys[2].getBytes();
		...
		int bytelen = data.getBytes().length;
		byte[] result = null;
		byte[] bytedata = hex2byte(data);

		DES3 des = new DES3();
		result = des.DesEncrypt(bytekey3, bytedata, 0);
		result = des.DesEncrypt(bytekey2, result, 1);
		result = des.DesEncrypt(bytekey1, result, 0);
	...
	// 3DES加密
	public static String DES3Encrypt(String data,String randomNum) {
		keyNum = getKeyNum(randomNum);
		return DesEncrypt(data, getTempKey());
	}

	// 3DES解密
	public static String DES3Decrypt(String data, String randomNum) {
		keyNum = getKeyNum(randomNum);
		return UnDesEncrypt(data, getTempKey());
	}

	private static String[] getTempKey() {
		String[] temKey = new String[3];

		if (keyNum != key.length - 2 && keyNum != key.length - 1) {
			temKey[0] = key[keyNum];
			temKey[1] = key[keyNum + 1];
			temKey[2] = key[keyNum + 2];
		}
		if (keyNum == key.length - 2) {
			temKey[0] = key[keyNum];
			temKey[1] = key[keyNum + 1];
			temKey[2] = key[0];
		}
	 ...
	 * @return
	 */
	private static int getKeyNum(String randomNum) {
		int tStringNum = Integer.parseInt(randomNum);
		// 根据key的size 返回模大小
		return tStringNum%32;
	}

	/** 以下代码为测试用 */
	public static void main(String[] args) {
		String bcd = "CF170C8C5312FD293414EF9D5B61ADF6A9A633D8C35ECAC1E9A7788BC1A545AB875D7BFFE6693A74971FD0B7C32F5142E3BBAE663D7EFD64";
		System.out.println("Result=[" + DES3.DES3Decrypt(bcd, "10").trim() + "]");
	}
}