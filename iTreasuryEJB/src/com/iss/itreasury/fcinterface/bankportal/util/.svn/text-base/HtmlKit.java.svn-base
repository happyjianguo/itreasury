...
public class HtmlKit {
	 ...
	 */
	public static void convertDataEntityToHiddenTag(JspWriter out, BaseDataEntity paramInfo,String[] notSetToHidden)	
	{
		//取得查询条件
		if(paramInfo ==null)
		{
				...
				for(int i=0; i<properties.length;i++)
				{
					...
					if(notSetToHidden!=null)
					{
					 ...
					 if(oValue!=null)
					 {
						...
						{
							strTemp = DataFormat.formatDate((Date)oValue);
	 ...
	 * @return	格式化后的字符串。
	 */
	public static String getStringFromInfo(BaseDataEntity paramInfo, String propertyName, int flag)
	{
		String strResult = "";
	...
	private static String convertDateToString(Object oValue, int flag) {
		...
		strResult = DataFormat.formatDate(dateTmp);
	...
	*/
	 ...
	 * @return
	 */
	public static long 	getLongFromInfo(BaseDataEntity paramInfo, String propertyName)
	{
		if(paramInfo==null)
		{
			log.debug("pramInfo 为空");
	...
	private static String getStringFromCodeNameRef(String propertyName, long refValue)
	{
		...
		try {
			Class nameRef  = Class.forName("com.iss.itreasury.fcinterface.bankportal.util.CodeNameRef");
			Method m = null;
			//首字大写
