 ...
 */
public class BaseDAO_oracle extends AbstractBaseDAO implements BaseDAO
{
	/**
	 * @param tableName
	 * @param isNeedPrefix
	 ...
	 */
	protected BaseDAO_oracle(String tableName, boolean isNeedPrefix, Connection conn)
		throws SystemException
	{
	...
	public long getMaxID() throws SystemException
	{
		...
		sb.append("select nvl(max(n_id)+1,1) n_id from " + strTableName);
		try
		{
			//内部维护RS和PS，否则将会产生冲突,但Connection使用同一个
			if (transConn == null)
			...
			case DAO_OPERATION_FIND:
				{
					...
					else if (Date.class.getName().equals(fieldType))
					{
						...
						{
							fieldExp = "to_char("+fieldName+",'yyyyMMdd') " + getFieldOpSymbol(fieldOp) + " to_char(?,'yyyyMMdd')";
						}
						else
						{
							fieldExp = "";
