...
import com.iss.itreasury.integratedCredit.dao.LoanCreditgradeInfoQueryDao;
...
public long updateLoanStatus(long lID) throws SettlementException, SQLException {	
	...
	public String[] getQuerySQL(LoanCreditgradeInfo info,String laction){
		...
		sql[0] = /*SELECT*/
			...
			"NVL(G.CREDITLEVEL,'') AS CREDITLEVEL,G.GRADEDATE,U.SNAME AS INPUTUSER,G.STATUS,G.NDEPARTMENTID,G.CREDITLEVELVALUE ";
		...
		{
			sql[2] = /*WHERE*/
				"C.ID = G.CLIENTID AND U.ID=G.INPUTUSERID AND G.STATUS IN (1,2,3,4,5,7)";
		}
		//限定条件
		...
		{
			if(info.getStatus()==2){
			 sqlBuf.append(" AND (G.STATUS =1 or G.STATUS =2)"  );	
			}else if(info.getStatus()==7){
			 sqlBuf.append(" AND (G.STATUS =7 or G.STATUS =5)"  );	
			}
			else{
			sqlBuf.append(" AND G.STATUS =" + info.getStatus());
			}
		}
		if(info.getNdepartmentid() > 0)
		{
			sqlBuf.append(" AND G.NDEPARTMENTID =" + info.getNdepartmentid());
		}
	...
	public PageLoader getCreditGradeInfoList(LoanCreditgradeInfo info,String laction)throws Exception{
		...
		pageLoader.initPageLoader(
			...
			"com.iss.itreasury.integratedCredit.dataentity.CreditGradeQueryInfo",
			null);
