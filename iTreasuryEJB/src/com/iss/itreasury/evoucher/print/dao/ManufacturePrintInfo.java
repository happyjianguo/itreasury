...
{
	protected Log4j Log = new Log4j(Constant.ModuleType.EVOUCHER, this);
	/**
	...
	public PrintInfo getFixDepositContinuePrintInfo(long lID) throws Exception
	{
		...
		try
		{
			...
			if (resultInfo != null)
			{
				...
				//定期帐户ID,可取帐户户名,账号(在定期续存中收方和付方都是定期帐户号)
				if (resultInfo.getAccountID() > 0)
				{
					printInfo.setReceiveAccountID(resultInfo.getAccountID());
				}
				...
				//付款方帐户ID ，可得付款帐户号(在定期续存中收方和付方都是定期帐户号)
				if (resultInfo.getAccountID() > 0)
				{
					printInfo.setPayAccountID(resultInfo.getAccountID());
				}
	...
	public PrintInfo getLoanGrantPrintInfo(long lID) throws Exception
	{
		...
		try
		{
			...
			if (resultInfo != null)
			{
				...
				Log.print("==========委托贷款帐户号：" + NameRef.getAccountNoByID(printInfo.getLoanAccountID()));
				//收款方
...
//				}
		...
		try
		{
			...
			if (resultInfo != null)
			{
				...
				if (resultInfo.getTransNo() != null && resultInfo.getTransNo().length() > 0)
				{
					...
					resultCollection = dao.findByConditionsForSquareUp(findCondition, -1, false);
						...
						if (resultInfo.getTypeID() == SETTConstant.ReceiveOrPay.PAY) //如果当前记录是付方
						{
							...
							if (bFlagPay == false)
							{
								...
								if (lIndexReceive == 1)
								{
									...
									if (resultInfo.getIsBank() > 0 || resultInfo.getBankID() > 0) //if银行转账(为应用模版,此处进行特殊处理,银行信息均设为外部帐户号)
									{
										...
										printInfo.setPayExtRemitInBank(NameRef.getBankNameByID(resultInfo.getBankID())); //银行转帐不写开户行信息
									...
									if (tempTransOnePayMultiReceiveInfo.getIsBank() > 0 || tempTransOnePayMultiReceiveInfo.getBankID() > 0) //if银行转账(为应用模版,此处进行特殊处理,银行信息均设为外部帐户号)
									{
										printInfo.setReceiveExtClientName(resultInfo.getExtClientName());
										printInfo.setReceiveExtAccountNo(resultInfo.getExtAccountNo());
										printInfo.setReceiveExtRemitInBank(resultInfo.getRemitInBank()); //银行转帐不写开户行信息
							...
							if (bFlagReceive == false)
							{
								...
								if (lIndexPay == 1)
								{
									...
									if (tempTransOnePayMultiReceiveInfo.getIsBank() > 0 || tempTransOnePayMultiReceiveInfo.getBankID() > 0) //if银行转账(为应用模版,此处进行特殊处理,银行信息均设为外部帐户号)
									{
										...
										printInfo.setPayExtRemitInBank(NameRef.getBankNameByID(tempTransOnePayMultiReceiveInfo.getBankID())); //银行转帐不写开户行信息
	...
	public PrintInfo getInterestFeePaymentPrintInfo(long lID) throws Exception
	{
		...
		try
		{
			...
			{
				ArrayList list = null;
				Sett_SyndicationLoanInterestDAO iDao = new Sett_SyndicationLoanInterestDAO();
				list = (ArrayList)iDao.findBySyndicationLoanReceiveID(lID);
			...
			if (resultInfo != null)
			{
				...
				//在利息费用支付中付方帐户,也是付息帐户号,付手续费，担保费账户号
				if (resultInfo.getPayInterestAccountID() > 0)
				{
	...
	public PrintInfo getPrintInfo(long lID,long lTransactionTypeID) throws Exception{
		...
		try
		{
				...
				lTransactionTypeID == SETTConstant.TransactionType.CAUTIONMONEYSAVE)		//保证金存款
			{
				...
				System.out.println("调用补打凭证信息----------（活期帐户组)-----------(开始)");
				printInfo = this.getCurrentPrintInfo(lID);			//	1.银行收款 银行付款 内部转账 委托存款 保证金存款
				System.out.println("调用补打凭证信息----------（活期帐户组)-----------(结束)");
			}
			else if(
			...
			)
			{
				printInfo=this.getDiscountRepaymentPrintInfo(lID);	//	8.贴现收回
					...
					lTransactionTypeID==SETTConstant.TransactionType.GENERALLEDGER 	//总帐
			)
			{
				printInfo=this.getGLPrintInfo(lID);	//	9.总帐
			}
			else if(
