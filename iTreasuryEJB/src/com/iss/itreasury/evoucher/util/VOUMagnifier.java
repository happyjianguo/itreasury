	...
	*/
		...
		throws Exception
	{
		...
		try
		{
			...
			String sqlparam = URLEncoder.encode(strMainNames[0] + ".value");
			if (!isSQL(strSQL))
			{
				strParam += "&strSQL= select * from ( '+" + strSQL + "+' ) where 1=1 '+get" + strMainNames[0] + "(" +sqlparam+")+'";
			}
			else
			{
				strParam += "&strSQL= select * from ( " + strSQL + " ) where 1=1 '+get" + strMainNames[0] + "(" + sqlparam+")+'";
