...
public class VOUHTML
{
       ...
       if (sessionMng.isLogin())
        {
            try
            {
                if (sessionMng.m_lModuleID == Constant.ModuleType.EVOUCHER){
    ...
    public static boolean validateRequest(JspWriter out, HttpServletRequest request, HttpServletResponse response) throws Exception
    {
        ...
        try
        {
            ...
            {
                VOUHTML.showMessage(out, sessionMng, request, response, Env.getClientName(), Constant.RecordStatus.INVALID, "Gen_E002");
                VOUHTML.showHomeEnd(out);
                out.flush();
                bResult = false;
            }
            //判断是否有权限
...
//            }
            ...
            if (lActionID.longValue() != 0)
            {
                if (lTemp != null && lActionID.compareTo(lTemp) <= 0)
                {
                    ...
                    VOUHTML.showMessage(out, sessionMng, request, response, Env.getClientName(), Constant.RecordStatus.INVALID, "Gen_E008");
                    VOUHTML.showHomeEnd(out);
                    out.flush();
                    bResult = false;
                }
        ...
        {
        	VOUHTML.showException(out, sessionMng, ie, request, response, Env.getClientName(), Constant.RecordStatus.INVALID, "");
        	VOUHTML.showHomeEnd(out);
            out.flush();
            ie.printStackTrace();
            bResult = false;
     ...
     * @throws IOException
     */
    public static void displyShowSendingHtml(JspWriter out,long topHeight) throws IException, IOException{
    	out.println("<script language=javascript>\n");
    	out.println("function showSending() \n");
    	out.println("{\n");
    	out.println(" gnIsShowSending=1;\n");
    	out.println("	sending.style.display=\"block\";\n");
    	out.println("	cover.style.display=\"block\";\n");
       	out.println("	sending.style.top=document.body.scrollTop+"+topHeight+";\n");
    	out.println("}\n");
    	out.println("\n");
    	out.println("</script>\n");
    	out.println("<div id=\"sending\" style=\"position:absolute; top:0; left:20; z-index:10; display:none\"><TABLE WIDTH=100% BORDER=0 CELLSPACING=0 CELLPADDING=0><TR><td width=30%></td><TD bgcolor=#ff9900><TABLE WIDTH=100% height=70 BORDER=0 CELLSPACING=2 CELLPADDING=0><TR><td bgcolor=#eeeeee align=center>正在执行中, 请稍候...</td></tr></table></td><td width=30%></td></tr></table></div>\n");
    	out.println("<div id=\"cover\" style=\"position:absolute; top:0; left:0; z-index:9; display:none\"><TABLE WIDTH=100% height=900 BORDER=0 CELLSPACING=0 CELLPADDING=0><TR><TD align=center><br></td></tr></table></div>\n");
    	out.println("\n");
    }
