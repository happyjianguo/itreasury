...
import com.iss.itreasury.settlement.interest.dao.Sett_CommissionDAO;
...
public class TransformOperationDataEntity implements Serializable
{
	protected HashMap usedFields = new HashMap();
	
	public HashMap getOperationHashMap(long transTypeID, long lID, String transNo) throws Exception
	{
     ...
     * @param isNetBank 是否是网银用户
     * @param  billName 模板文件名称
     * @return String
     */
	public String isSignature(long clientID,long transTypeID,long nOfficeID,long nCurrencyID,int isNetBank,String billName)throws ITreasuryDAOException
	{
		String isSignature = "0";
		SetDataEntityGD dataEntityGD = new SetDataEntityGD();
		isSignature = dataEntityGD.getIsSignature(clientID,transTypeID,nOfficeID,nCurrencyID,billName,isNetBank);
		return isSignature;
	}
		...
		try
		{
			...
			else if( lTransactionTypeID == SETTConstant.TransactionType.MULTILOANRECEIVE )
			{
				System.out.println("多笔贷款收回这个没有做在settlement-print-control-co17.jsp中有定义");
				  ...
				  || lTransactionTypeID == SETTConstant.TransactionType.DISCOUNTACCRUAL						//贴现贷款计提应收利息（含冲销）
			...
			else if(lTransactionTypeID==SETTConstant.TransactionType.CONSIGN_LOAN_COMMISION_FEE)
			{
				printInfo = this.getCommissionPrintInfo(lID);
			}
			//暂时无此交易类型的打印
			else
			{
	...
	public PrintInfo getCommissionPrintInfo(long lID) throws Exception
	{
		PrintInfo printInfo = new PrintInfo();
		try
		{
			CommissionInfo resultInfo = new CommissionInfo();
			Sett_CommissionDAO dao = new Sett_CommissionDAO();
			...
			if(resultInfo != null)
			{
				...
				{
					printInfo.setCommission(resultInfo.getCommissionAmount());
				}				
				if(resultInfo.getExecuteDate()!=null)
				{
					printInfo.setExecuteDate(resultInfo.getExecuteDate());
				}
				if(!resultInfo.getTransNo().trim().equals(""))
				{
					printInfo.setTransNo(resultInfo.getTransNo().trim());
				}
				if(!resultInfo.getAbstract().trim().equals(""))
				{
					printInfo.setAbstract(resultInfo.getAbstract().trim());
				}
				...
				{
					printInfo.setPayCommissionAccountID(resultInfo.getAccountId());
				}
				if(resultInfo.getBorrowClientId()>0)
				{
					printInfo.setBorrowClientID(resultInfo.getBorrowClientId());
				}
				printInfo.setTransactionTypeID(SETTConstant.TransactionType.CONSIGN_LOAN_COMMISION_FEE);
	...
	public PrintInfo getFixDepositOpenPrintInfo(long lID) throws Exception
	{
		...
		try
		{
			...
			if (resultInfo != null)
			{
				...
				if(resultInfo.getIsAutoContinue()>0){
					printInfo.setIsAutoContinue(resultInfo.getIsAutoContinue());
				}
			}
		}
		catch (Exception exp)
	...
	public PrintInfo getFixDepositWithDrawPrintInfo(long lID) throws Exception
	{
		...
		try
		{
			...
			if (resultInfo != null)
			{
				...
				//利息转至活期账户号(南航项目添加) add by 2012-06-07
	...
	public PrintInfo getFixDepositContinuePrintInfo(long lID) throws Exception
	{
		...
		try
		{
			...
			if (resultInfo != null)
			{
				...
				//利息支付
				//2011-04-12 by jiangqi 
				//BUG #4186::78上对于定期计提利息后的到期续存，复核后的打印单据中“存款利息计付通知单”利息为空，此为错编辑 解决 关闭 激活 复制 返回 				
				if (resultInfo.getWithDrawInterest() !=0)
				{
				...
				if(resultInfo.getIsCapitalAndInterestTransfer() > 0)
				{
					...
					//BUG #4186::78上对于定期计提利息后的到期续存，复核后的打印单据中“存款利息计付通知单”利息为空
					//jiangqi 2011-02-25
	...
	public PrintInfo getLoanRepaymentPrintInfo(long lID) throws Exception
	{
		...
		try
		{
			...
			if (resultInfo != null)
			{
				...
				//收款方
				if(resultInfo.getTransactionTypeID()==SETTConstant.TransactionType.CONSIGNLOANRECEIVE)
				{
				...
				//add by dwj 20110815 添加委托贷款收回子账户信息
				if(resultInfo.getSubAccountID()>0)
				{
					printInfo.setSubAccountID(resultInfo.getSubAccountID());
				}
				//end add by dwj 20110815
	...
	public PrintInfo getGLPrintInfo(long lID) throws Exception
	{
		...
		try
		{
			...
			if (resultInfo != null)
			{
				 ...
				 *  添加账户类型和对应科目
				 */
				if (resultInfo.getSSubject() != null && resultInfo.getSSubject().length() > 0)
				{
					printInfo.setSSubject(resultInfo.getSSubject());
				}
				if (resultInfo.getNAccountTypeID() > 0)
				{
					printInfo.setNAccountTypeID(resultInfo.getNAccountTypeID());
				}
				//end
				if (resultInfo.getOfficeID() > 0)
				{
				...
				if( resultInfo.getAmountOne()!=0.0)
				{
				...
				if( resultInfo.getAmountTwo()!=0.0 )
				{
				...
				if( resultInfo.getAmountThree()!=0.0 )
				{
...
//				}
              ...
              //其它
                ...
                BranchInfo branchInfo = new BranchInfo();
 				...
 				payBankID = resultInfo.getNpaybankid();
				receiveBankID = resultInfo.getNreceivebankid();						
				if( payBankID > 0 )
				{
				...
				{
					receiveAccountNo = resultInfo.getSextaccountno();
				}
				...
				//end add by dwj 20110813
				
				/*update by dwj 20110813
	...
	public PrintInfo getFixDepositChangePrintInfo(long lID) throws Exception
	{
		...
		try
		{
			...
			if (resultInfo != null)
			{
				...
				//换开定期存单号
				if(resultInfo.getDepositBillNO()!=null&&!resultInfo.getDepositBillNO().trim().equals("-1"))
				{
	...
	public PrintInfo getTransOpenMarginDepositPrintInfo(long lID) throws Exception
	{
        ...
        if (resultInfo != null)
		{
			...
			//BUG #3490::担保收款的交易打印提示“找不到活期子账户信息，导致生成会计分录失败”此为不正确
			//jiangqi 2010-02-25
            //账号，收款方账户号
            if (resultInfo.getAccountID() > 0 && resultInfo.getDepositNo() != null && resultInfo.getDepositNo().length() > 0)
            {
                    printInfo.setSubAccountID(NameRef.getSubAccountIDByAccountIDAndDEpositNo(resultInfo.getAccountID(),resultInfo.getDepositNo()));
		...
		try
		{
			...
			/*
					if (vector != null && vector.size()> 0){
						...
						int i = 0;
						if(vector.size() == 1){
							dailAccInfo1 = (DailyAccountBalanceInfo)vector.get(i);
							sumAmount = dailAccInfo1.getDailyInterest();
							list.add(Double.toString(sumAmount));
						}
						else {
							while(i <= vector.size()){
								dailAccInfo1 = (DailyAccountBalanceInfo)vector.get(i);
								...
								for(int j=i+1; j<vector.size(); j++){
									dailAccInfo2 = (DailyAccountBalanceInfo)vector.get(j);
									i = j;
									if(dailAccInfo1.getInterestRate() == dailAccInfo2.getInterestRate()){
										sumAmount = sumAmount + dailAccInfo2.getDailyInterest();
										dailAccInfo1 = (DailyAccountBalanceInfo)vector.get(j);
									}
									else{
										list.add(Double.toString(sumAmount));
										sumAmount = 0.0;
					...
					/*if ( conn != null && conn.size()> 0) 
					{
						 ...
						 double sumAmount3 = 0.0;
						 double rate = 0.0;
						 dailAccInfo1 = (DailyAccountBalanceInfo)iter.next();
						// sumAmount1 = dailAccInfo1.getDailyInterest();
						 while (iter != null && iter.hasNext()) 
						 {
							 dailAccInfo2 = (DailyAccountBalanceInfo)iter.next();
	
							if(dailAccInfo2.getInterestRate() == dailAccInfo1.getInterestRate())
							{							
								sumAmount1 = dailAccInfo1.getDailyInterest() + dailAccInfo2.getDailyInterest();
								//sumAmount1 = sumAmount1+dailAccInfo2.getDailyInterest();
								System.out.println(sumAmount1);
								dailAccInfo1 = dailAccInfo2;
							 }
							else
							{   
								list.add(Double.toString(sumAmount1));
								sumAmount1=0.0;
								dailAccInfo1 = dailAccInfo2;
							}							
	...
	public void TransformOperationData(PrintInfo printOperationInfo) throws Exception
	{
				...
				||printOperationInfo.getTransactionTypeID() == SETTConstant.TransactionType.TRANSFERPAY )
		{
			...
			usedFields.put("ChineseCommissionFee", PrintDataFormat.getChineseFormatAmount(printOperationInfo.getCommission(), 1));
	...
	else
		{
			...
			if(printOperationInfo.getTransactionTypeID() == SETTConstant.TransactionType.CONSIGN_LOAN_COMMISION_FEE)
			{
				usedFields.put("RepaymentUnitName", NameRef.getClientNameByID( printOperationInfo.getBorrowClientID() ) );
				usedFields.put("CommissionFee", printOperationInfo.getCommission()+"");
				usedFields.put("ChineseCommissionFee", PrintDataFormat.getChineseFormatAmount(printOperationInfo.getCommission(), 1));	
			...
			else if( printOperationInfo.getPayBankID() > 0 && printOperationInfo.getTransTypeID() != SETTConstant.TransactionType.BANKRECEIVE )
			{
				...
				//付款人全称
				//修改显示为"开户行打印名称"
				//usedFields.put("PayAccountName", DataFormat.formatString( branchInfo.getEnterpriseName() ));
				
				if( printOperationInfo.getTransTypeID() == SETTConstant.TransactionType.SPECIALOPERATION )
				{
					//usedFields.put("PayAccountName", Config.getProperty(ConfigConstant.SYSTEM_PROJECTNAME)==null?"":Config.getProperty(ConfigConstant.SYSTEM_PROJECTNAME));
					//2011-04-12 by jiangqi
			...
			else
			{
				...
				else
				{
					...
					//{
						//付款方账号
					//	usedFields.put("PayAccountNo", "");
					//}
				}
			}
			
			//委托贷款结息
			if(printOperationInfo.getTransTypeID()==SETTConstant.TransactionType.CONSIGN_LOAN_INTEREST
			...
			if (printOperationInfo.getReceiveAccountID() > 0)
			{
				...
				if(printOperationInfo.getTransTypeID()==SETTConstant.TransactionType.CONSIGNLOANRECEIVE)
				{ 
					usedFields.put("ConsignClientNo", NameRef.getAccountNoByID( printOperationInfo.getReceiveAccountID() ));
				}
				   ...
				   || (printOperationInfo.getReceiveBankID() > 0 && printOperationInfo.getTransTypeID() >1000) )
			{
				...
				if( printOperationInfo.getTransTypeID() == SETTConstant.TransactionType.SPECIALOPERATION )
				{
					//usedFields.put("ReceiveAccountName", Config.getProperty(ConfigConstant.SYSTEM_PROJECTNAME)==null?"":Config.getProperty(ConfigConstant.SYSTEM_PROJECTNAME));
			...
			else if(printOperationInfo.getNerceivedirection() ==SETTConstant.DebitOrCredit.DEBIT  && printOperationInfo.getTransTypeID() == SETTConstant.TransactionType.SPECIALOPERATION && printOperationInfo.getOperationType()==1003)
			{
				//modify by xwhe 2008- 12 -16
				if (printOperationInfo.getReceiveBankID()> 0&& printOperationInfo.getPayGL()>0 && printOperationInfo.getNerceivedirection() ==SETTConstant.DebitOrCredit.DEBIT  && printOperationInfo.getTransTypeID() == SETTConstant.TransactionType.SPECIALOPERATION && printOperationInfo.getOperationType()==1003)
				{
					Sett_BranchDAO sett_BranchDAO = new Sett_BranchDAO();
					BranchInfo branchInfo = new BranchInfo();
					branchInfo = sett_BranchDAO.findByID(printOperationInfo.getReceiveBankID());  
			    ...
			    //modify by xwhe 2008- 12 -16
				else if (printOperationInfo.getReceiveAccountID()>0&& printOperationInfo.getPayGL()>0 && printOperationInfo.getNerceivedirection() ==SETTConstant.DebitOrCredit.DEBIT  && printOperationInfo.getTransTypeID() == SETTConstant.TransactionType.SPECIALOPERATION && printOperationInfo.getOperationType()==1003)
				{
					//借方信息
					usedFields.put("Account", "账号");
					usedFields.put("ReceiveAccountNo", NameRef.getAccountNoByID(printOperationInfo.getReceiveAccountID()));	
				...
				//modify by xwhe 2008- 12 -16 
				else if (printOperationInfo.getPayAccountID()>0 && printOperationInfo.getReceiveGL()>0 && printOperationInfo.getNerceivedirection() ==SETTConstant.DebitOrCredit.DEBIT  && printOperationInfo.getTransTypeID() == SETTConstant.TransactionType.SPECIALOPERATION && printOperationInfo.getOperationType()==1003)
				{
					//借方信息
				...
				//modify by xwhe 2008- 12 -16
				else if (printOperationInfo.getReceiveAccountID()>0&& printOperationInfo.getPayBankID()> 0 && printOperationInfo.getNerceivedirection() ==SETTConstant.DebitOrCredit.DEBIT  && printOperationInfo.getTransTypeID() == SETTConstant.TransactionType.SPECIALOPERATION && printOperationInfo.getOperationType()==1003)
				{
					Sett_BranchDAO sett_BranchDAO = new Sett_BranchDAO();
					BranchInfo branchInfo = new BranchInfo();
					branchInfo = sett_BranchDAO.findByID(printOperationInfo.getPayBankID()); 
				...
				//modify by xwhe 2008- 12 -16
				else if (printOperationInfo.getReceiveBankID()> 0&& printOperationInfo.getPayAccountID()>0 && printOperationInfo.getNerceivedirection() ==SETTConstant.DebitOrCredit.DEBIT  && printOperationInfo.getTransTypeID() == SETTConstant.TransactionType.SPECIALOPERATION && printOperationInfo.getOperationType()==1003)
				{
					Sett_BranchDAO sett_BranchDAO = new Sett_BranchDAO();
					BranchInfo branchInfo = new BranchInfo();
					branchInfo = sett_BranchDAO.findByID(printOperationInfo.getReceiveBankID());  
				...
				//modify by xwhe 2008- 12 -16 
				else if (printOperationInfo.getPayGL()>0 && printOperationInfo.getReceiveGL()>0 && printOperationInfo.getNerceivedirection() ==SETTConstant.DebitOrCredit.DEBIT  && printOperationInfo.getTransTypeID() == SETTConstant.TransactionType.SPECIALOPERATION && printOperationInfo.getOperationType()==1003)
				{
					//借方信息
					usedFields.put("Account", "科目号");
					usedFields.put("ReceiveAccountNo",DataFormat.formatString(NameRef.getGLKNoByID(printOperationInfo.getReceiveGL())));	
				...
				//modify by xwhe 2008- 12 -16 
				else if (printOperationInfo.getPayBankID()>0 && printOperationInfo.getReceiveGL()>0 && printOperationInfo.getNerceivedirection() ==SETTConstant.DebitOrCredit.DEBIT  && printOperationInfo.getTransTypeID() == SETTConstant.TransactionType.SPECIALOPERATION && printOperationInfo.getOperationType()==1003)
				{
					Sett_BranchDAO sett_BranchDAO = new Sett_BranchDAO();
					BranchInfo branchInfo = new BranchInfo();
					branchInfo = sett_BranchDAO.findByID(printOperationInfo.getPayBankID());  
			...
			else if(printOperationInfo.getNpaydirection() ==SETTConstant.DebitOrCredit.DEBIT  && printOperationInfo.getTransTypeID() == SETTConstant.TransactionType.SPECIALOPERATION && printOperationInfo.getOperationType()==1003)
			{
		        //modify by xwhe 2008- 12 -16
				if (printOperationInfo.getReceiveBankID()> 0&& printOperationInfo.getPayGL()>0 && printOperationInfo.getNpaydirection() ==SETTConstant.DebitOrCredit.DEBIT  && printOperationInfo.getTransTypeID() == SETTConstant.TransactionType.SPECIALOPERATION && printOperationInfo.getOperationType()==1003)
				{
					Sett_BranchDAO sett_BranchDAO = new Sett_BranchDAO();
					BranchInfo branchInfo = new BranchInfo();
					branchInfo = sett_BranchDAO.findByID(printOperationInfo.getReceiveBankID());  
			    ...
			    //modify by xwhe 2008- 12 -16
				else if (printOperationInfo.getReceiveAccountID()>0&& printOperationInfo.getPayGL()>0 && printOperationInfo.getNpaydirection() ==SETTConstant.DebitOrCredit.DEBIT  && printOperationInfo.getTransTypeID() == SETTConstant.TransactionType.SPECIALOPERATION && printOperationInfo.getOperationType()==1003)
				{
					//借方信息
					usedFields.put("Account", "科目号");
					usedFields.put("ReceiveAccountNo",DataFormat.formatString(NameRef.getGLKNoByID(printOperationInfo.getPayGL())));	
				...
				//modify by xwhe 2008- 12 -16 
				else if (printOperationInfo.getPayGL()>0 && printOperationInfo.getReceiveGL()>0 && printOperationInfo.getNpaydirection() ==SETTConstant.DebitOrCredit.DEBIT  && printOperationInfo.getTransTypeID() == SETTConstant.TransactionType.SPECIALOPERATION && printOperationInfo.getOperationType()==1003)
				{
					//借方信息
					usedFields.put("Account", "科目号");
					usedFields.put("ReceiveAccountNo",DataFormat.formatString(NameRef.getGLKNoByID(printOperationInfo.getPayGL())));	
				...
				//modify by xwhe 2008- 12 -16 
				else if (printOperationInfo.getPayAccountID()>0 && printOperationInfo.getReceiveGL()>0 && printOperationInfo.getNpaydirection() ==SETTConstant.DebitOrCredit.DEBIT  && printOperationInfo.getTransTypeID() == SETTConstant.TransactionType.SPECIALOPERATION && printOperationInfo.getOperationType()==1003)
				{
					//借方信息
					usedFields.put("Account", "账号");			
				...
				//modify by xwhe 2008- 12 -16 
				else if (printOperationInfo.getPayAccountID()>0 && printOperationInfo.getReceiveBankID()>0 && printOperationInfo.getNpaydirection() ==SETTConstant.DebitOrCredit.DEBIT  && printOperationInfo.getTransTypeID() == SETTConstant.TransactionType.SPECIALOPERATION && printOperationInfo.getOperationType()==1003)
				{
					Sett_BranchDAO sett_BranchDAO = new Sett_BranchDAO();
					BranchInfo branchInfo = new BranchInfo();
					branchInfo = sett_BranchDAO.findByID(printOperationInfo.getReceiveBankID()); 				
				...
				//modify by xwhe 2008- 12 -16 
				else if (printOperationInfo.getPayBankID()>0 && printOperationInfo.getReceiveAccountID()>0 && printOperationInfo.getNpaydirection() ==SETTConstant.DebitOrCredit.DEBIT  && printOperationInfo.getTransTypeID() == SETTConstant.TransactionType.SPECIALOPERATION && printOperationInfo.getOperationType()==1003)
				{
					Sett_BranchDAO sett_BranchDAO = new Sett_BranchDAO();
					BranchInfo branchInfo = new BranchInfo();
					branchInfo = sett_BranchDAO.findByID(printOperationInfo.getPayBankID());  
				...
				//modify by xwhe 2008- 12 -16 
				else if (printOperationInfo.getPayBankID()>0 && printOperationInfo.getReceiveGL()>0 && printOperationInfo.getNpaydirection() ==SETTConstant.DebitOrCredit.DEBIT  && printOperationInfo.getTransTypeID() == SETTConstant.TransactionType.SPECIALOPERATION && printOperationInfo.getOperationType()==1003)
				{
					Sett_BranchDAO sett_BranchDAO = new Sett_BranchDAO();
					BranchInfo branchInfo = new BranchInfo();
					branchInfo = sett_BranchDAO.findByID(printOperationInfo.getPayBankID());  
			...
			//2011-04-02 by jiangqi 特殊业务 总账类 收款方客户 取数逻辑不对。
			...
			else
			{
				...
				else if( printOperationInfo.getFeeTypeID() > 0 && printOperationInfo.getTransTypeID() == SETTConstant.TransactionType.TRANSFEE )
				{
					...
					{
						//usedFields.put("ReceiveSubjectCode", transFeeTypeSetInfo.getSubjectCode());
			...
			//摘要
			//2011-09-15 根据航天科工项目的要求，摘要中去掉‘起息日’
			/*if(printOperationInfo.getInterestStartDate() != null)
			{
				...
				if(printOperationInfo.getTransTypeID() == SETTConstant.TransactionType.FIXEDCONTINUETRANSFER)
				{
					...
					//strAbstract+= "新存单起息日：" + DataFormat.getChineseDateString( printOperationInfo.getInterestStartDate() );
				}
			 ...
			 || printOperationInfo.getTransTypeID() == SETTConstant.TransactionType.NOTIFYDEPOSITDRAW )
			{
				...
				{
					//出现问题，在打印显示本金的时候显示的是这里的利息金额，修改成本金金额
					/*if(printOperationInfo.getPayableInterest() != 0.0)
					{
						String strAmount = DataFormat.formatAmount(printOperationInfo.getPayableInterest());
						
						//本金金额小写
						usedFields.put("Amount", strAmount);
						
						String strChineseAmount = ChineseCurrency.showChinese(strAmount, printOperationInfo.getCurrencyID());
					...
					//收款人全称
					//usedFields.put("ReceiveAccountName", NameRef.getAccountNameByID( printOperationInfo.getReceiveInterestAccountID() ));
				...
				if(printOperationInfo.getAccountAmount() > printOperationInfo.getDrawAmount() && printOperationInfo.getInterestStartDate().compareTo(printOperationInfo.getEndDate()) < 0)
		        {
	                ...
	                if(transFixedOpenInfo != null)
	                {
	                	usedFields.put("HNewDepositNo", DataFormat.formatString(transFixedOpenInfo.getDepositNo()));
	                
	                
	                
	                usedFields.put("HNewAmount",  PrintDataFormat.getFormatAmount( transFixedOpenInfo.getAmount() ));
	                
	                usedFields.put("HNewChineseAmount", PrintDataFormat.getChineseFormatAmount( transFixedOpenInfo.getAmount(), printOperationInfo.getCurrencyID() ));
	               ...
	               /* if(transFixedOpenInfo.getInterestStartDate() != null)
	                {
	                   ...
	                   // String strAbstract = "起息日：" + DataFormat.getChineseDateString(transFixedOpenInfo.getInterestStartDate());
	                ...
	                usedFields.put("zhiqu", "√");//(南航项目添加)
	                //本金金额小写(南航项目添加)
					usedFields.put("fixedAmount", PrintDataFormat.getFormatAmount( printOperationInfo.getAmount() ));
					//本金金额大写(南航项目添加)
					usedFields.put("fixedChineseAmount", PrintDataFormat.getChineseFormatAmount( printOperationInfo.getAmount(), printOperationInfo.getCurrencyID() ));
					//定期支取本金存入账号(南航项目添加)
					usedFields.put("principalAccount", NameRef.getAccountNoByID( printOperationInfo.getReceiveAccountID() ));
					//定期支取利息存入账号(南航项目添加)
					usedFields.put("interestAccount", NameRef.getAccountNoByID( printOperationInfo.getReceiveInterestAccountID() ));
				...
				if(printOperationInfo.getAccountAmount() > printOperationInfo.getDrawAmount() && printOperationInfo.getTransTypeID() == SETTConstant.TransactionType.NOTIFYDEPOSITDRAW)
				{
	                ...
	                usedFields.put("HNewAmount",  PrintDataFormat.getFormatAmount( printOperationInfo.getDepositBalance() ));
	                
	                usedFields.put("HNewChineseAmount", PrintDataFormat.getChineseFormatAmount( printOperationInfo.getDepositBalance(), printOperationInfo.getCurrencyID() ));
			...
			else
			{
				...
				if (printOperationInfo.getTransTypeID() == SETTConstant.TransactionType.FIXEDCONTINUETRANSFER)
				{
					...
					{
						//(南航项目添加)
						usedFields.put("benxixucun", "√");
						if(printOperationInfo.getRealInterest() != 0.0 || printOperationInfo.getAmount() != 0.0)
						{
							//本金金额小写
							usedFields.put("Amount", PrintDataFormat.getFormatAmount( printOperationInfo.getRealInterest() + printOperationInfo.getAmount() ));
							
							//本金金额大写
							usedFields.put("ChineseAmount", PrintDataFormat.getChineseFormatAmount( printOperationInfo.getRealInterest() + printOperationInfo.getAmount(), printOperationInfo.getCurrencyID() ));
							
							//本金金额小写(南航项目添加)
							usedFields.put("renewAmount", PrintDataFormat.getFormatAmount( printOperationInfo.getRealInterest() + printOperationInfo.getAmount() ));
							//本金金额大写(南航项目添加)
							usedFields.put("renewChineseAmount", PrintDataFormat.getChineseFormatAmount( printOperationInfo.getRealInterest() + printOperationInfo.getAmount(), printOperationInfo.getCurrencyID() ));
						...
						else
						{
							...
							//本金金额小写(南航项目添加)
							usedFields.put("renewAmount","0.00");
							//本金金额大写(南航项目添加)
							usedFields.put("renewChineseAmount", strChineseAmount);
					...
					else
					{
						...
						if(printOperationInfo.getAmount() != 0.0)
						{
							...
							//本金金额小写(南航项目添加)
							usedFields.put("renewAmount", PrintDataFormat.getFormatAmount( printOperationInfo.getAmount() ));
							//本金金额大写(南航项目添加)
							usedFields.put("renewChineseAmount", PrintDataFormat.getChineseFormatAmount( printOperationInfo.getAmount(), printOperationInfo.getCurrencyID() ));
						}
						else
						{
							//本金金额小写
						...
						//(南航项目添加)
						usedFields.put("renewInterestAccount", NameRef.getAccountNoByID( printOperationInfo.getReceiveInterestAccountID() ));
					}
				}
				//其他业务
				else
				{
					...
					if(printOperationInfo.getTransTypeID() == SETTConstant.TransactionType.CONSIGNLOANRECEIVE)
					{
						...
						/***** 委托贷款本金回收凭证,显示的金额加了利息,现在去掉,只显示本金(如果需要使用重新PUT) *****/
						//printOperationInfo.setAmount( DataFormat.formatDouble (
						//			  DataFormat.formatDouble(printOperationInfo.getAmount())
						...
						//			- DataFormat.formatDouble(printOperationInfo.getRealInterestTax()) ) );
					}
			 ...
			 || printOperationInfo.getTransTypeID() == SETTConstant.TransactionType.CONSIGNLOANRECEIVE )
			{
				...
				{
					//本金由活期账户支付
					dAmount = DataFormat.formatDouble( printOperationInfo.getAmount() );
					
					//存在付息活期账户号
					if(printOperationInfo.getPayInterestAccountID() > 0)
					{
						//dAmount = DataFormat.formatDouble( printOperationInfo.getAmount() );
					}
					...
					{
						//自营贷款收回，存在付担保费活期账户号
						if (printOperationInfo.getPaySuretyFeeAccountID() > 0)
						{
							//dAmount = DataFormat.formatDouble( dAmount + printOperationInfo.getRealSuretyFee() );
						}
					}
					else
					{
						...
						{
							//dAmount = DataFormat.formatDouble(dAmount + DataFormat.formatDouble( printOperationInfo.getRealCommission() ));
						}
					}
				}
				else
				{
					...
					{
						//付款人全称
						usedFields.put("PayAccountName", NameRef.getAccountNameByID( printOperationInfo.getPayInterestAccountID() ));
						
						//付款方账号
						usedFields.put("PayAccountNo", NameRef.getAccountNoByID( printOperationInfo.getPayInterestAccountID() ));
						
						//付款方开户行名称
						usedFields.put("PayBankName", DataFormat.formatString( NameRef.getOfficeNameByID( printOperationInfo.getOfficeID() ) ));
					...
					{
						//付担保费账户号（贷款方） 付手续费账户号
						if(printOperationInfo.getPaySuretyFeeAccountID() > 0 || printOperationInfo.getPayCommissionAccountID() > 0)
						{
							//自营贷款收回
							if (printOperationInfo.getTransTypeID() == SETTConstant.TransactionType.TRUSTLOANRECEIVE)
							{
								//实际支付担保费
								dAmount = DataFormat.formatDouble(printOperationInfo.getRealSuretyFee());
								
								//付款人全称
								usedFields.put("PayAccountName", NameRef.getAccountNameByID( printOperationInfo.getPaySuretyFeeAccountID() ));
								
								//付款方账号
								usedFields.put("PayAccountNo", NameRef.getAccountNoByID( printOperationInfo.getPaySuretyFeeAccountID() ));
							...
							{
								//实际支付手续费
								dAmount = DataFormat.formatDouble(printOperationInfo.getRealCommission());
	
								//付款人全称
								usedFields.put("PayAccountName", NameRef.getAccountNameByID( printOperationInfo.getPayCommissionAccountID() ));
								
								//付款方账号
								usedFields.put("PayAccountNo", NameRef.getAccountNoByID( printOperationInfo.getPayCommissionAccountID() ));
				...
				{
					//本金金额小写
					usedFields.put("Amount", this.getFormatAmount( dAmount ));
					
					//本金金额大写
					usedFields.put("ChineseAmount", this.getChineseFormatAmount( dAmount, printOperationInfo.getCurrencyID() ));
				}
				else
				{
					//本金金额小写
			...
			//委托收回（手续费） add by qianggao 2008-07-28
			if(printOperationInfo.getTransTypeID() == SETTConstant.TransactionType.CONSIGNLOANRECEIVE)
	        {
				if(printOperationInfo.getPayCommissionAccountID() > 0) 
	            {
	            ...
	            if(printOperationInfo.getReceiveAccountID() > 0)
	            {
	            	 usedFields.put("CommissionReceiveAccountName", NameRef.getAccountNameByID(printOperationInfo.getReceiveAccountID()));
		             usedFields.put("CommissionReceiveAccountNo", NameRef.getAccountNoByID(printOperationInfo.getReceiveAccountID()));
		             /*
		             AccountBean accountbean = new AccountBean();
		             GLSubjectDefinitionInfo subjectDefinitionInfo = new GLSubjectDefinitionInfo();
		             ...
		             if(subjectDefinitionInfo != null)
		             {
		             	  sedFields.put("CommissionReceiveAccountName", DataFormat.formatString(subjectDefinitionInfo.getSsubjectName()));
			...
			if(printOperationInfo.getTransTypeID() == SETTConstant.TransactionType.DISCOUNTRECEIVE)
			{
				...
				{
					//本金金额小写
					usedFields.put("Amount", PrintDataFormat.getFormatAmount( printOperationInfo.getAmount() + printOperationInfo.getOverDueAmount() ));
					
					//本金金额大写
					usedFields.put("ChineseAmount", PrintDataFormat.getChineseFormatAmount( printOperationInfo.getAmount() + printOperationInfo.getOverDueAmount(), printOperationInfo.getCurrencyID() ));
				}
				else
				{
					//本金金额小写
					...
					|| printOperationInfo.getTransTypeID() == SETTConstant.TransactionType.INTERESTFEEPAYMENT )
			{
				...
				if(printOperationInfo.getLoanNoteID() > 0)
				{
					...
					{
						//正常利息开始日期
						//usedFields.put("NormalInterestDateStart", DataFormat.getChineseDateString( lpfdinfo.getStart() ) );
						usedFields.put("NormalInterestDateStart", DataFormat.getChineseDateString( printOperationInfo.getLatestInterestClearDate() ) );
						
						//正常利息结束日期
						//usedFields.put("NormalInterestDateEnd", DataFormat.getChineseDateString( printOperationInfo.getExecuteDate() ) );
						...
						//正常利息天数
						//usedFields.put("NormalInterestDay", DataFormat.formatString( (PrintVoucher.getIntervalDays(lpfdinfo.getStart(), printOperationInfo.getExecuteDate(), 1)) + "") );
					...
					if (printOperationInfo.getRealCommission() > 0)
					{
						...
						//手续费利率 modify by xwhe 2008-07-28 取放款通知单的手续费率
					    //usedFields.put("CommissionFeeRate", DataFormat.formatRate( printOperationInfo.getCommissionRate(), 6 ));
	                    //手续费利率
					 ...
					 )
					{
						...
						if(printOperationInfo.getInterest1()>0)
						{
							if(printOperationInfo.getRate1()!=0)
							{
								...
								usedFields.put("LoanAmass1", Constant.CurrencyType.getSymbol(printOperationInfo.getCurrencyID())+DataFormat.formatListAmount( Double.parseDouble(LoanAmass1), 2 ));  //贷款积数1
							}
						...
						if(printOperationInfo.getInterest2()>0)
						{
							if(printOperationInfo.getRate2()!=0)
							{
								...
								usedFields.put("LoanAmass2", Constant.CurrencyType.getSymbol(printOperationInfo.getCurrencyID())+DataFormat.formatListAmount( Double.parseDouble(LoanAmass2), 2 ));  //贷款积数2
							}						
						...
						if(printOperationInfo.getInterest3()>0)
						{
							if(printOperationInfo.getRate3()!=0)
							{
								...
								usedFields.put("LoanAmass3", Constant.CurrencyType.getSymbol(printOperationInfo.getCurrencyID())+DataFormat.formatListAmount( Double.parseDouble(LoanAmass3), 2 ));  //贷款积数3
							}
						...
						//modify by kevin(刘连凯) 新增sectionalTotalInterest 专用于分段的利息合计
						usedFields.put("chineseSectionalTotalInterest", PrintDataFormat.getChineseFormatAmount( printOperationInfo.getInterest1()+printOperationInfo.getInterest2()+printOperationInfo.getInterest3(), printOperationInfo.getCurrencyID() ));		
						
			            //利息合计小写 
						usedFields.put("sectionalTotalInterest", Constant.CurrencyType.getSymbol(printOperationInfo.getCurrencyID())+PrintDataFormat.getFormatAmount( printOperationInfo.getInterest1()+printOperationInfo.getInterest2()+printOperationInfo.getInterest3() ));
			...
			if ( printOperationInfo.getTransTypeID() == SETTConstant.TransactionType.INTERESTFEEPAYMENT )
			{
				...
				if (printOperationInfo.getRealInterest() != 0)
				{
					...
					//Boxu 2008年12月18日 为了京能打印修改
					usedFields.put("NormalInterestDay", DataFormat.formatString( (PrintVoucher.getIntervalDays(printOperationInfo.getInterestStartDate(), printOperationInfo.getEndDate(), 1)) + "") );
					
	                //利息合计大写 modify by xwhe 2008-07-15 modify by xwhe 2008-08-04
					usedFields.put("chineseOpenTotalInterest", PrintDataFormat.getChineseFormatAmount( printOperationInfo.getRealInterest(), printOperationInfo.getCurrencyID() ));		
					
		            //利息合计小写 
					usedFields.put("NoticeOpenTotalInterest", Constant.CurrencyType.getSymbol(printOperationInfo.getCurrencyID())+PrintDataFormat.getFormatAmount( printOperationInfo.getRealInterest() ));
			...
			if( printOperationInfo.getTransTypeID() == SETTConstant.TransactionType.CONSIGN_LOAN_PREDRAW_INTEREST
			  ...
			  || printOperationInfo.getTransTypeID() == SETTConstant.TransactionType.DISCOUNTACCRUAL	//贴现贷款计提应收利息（含冲销） 
			  || printOperationInfo.getTransTypeID() == SETTConstant.TransactionType.YT_LOAN_PREDRAW_INTEREST)
			{
				//合同编号
				usedFields.put("ContractNo", DataFormat.formatString( NameRef.getCommonContractcodeBySubAccountID(printOperationInfo.getSubAccountID()) ) );
...
//					modify by xiangzhou 2013 分段利息显示临时方案
					Interest1 = DataFormat.formatAmount(printOperationInfo.getCurrentInterest());
			...
			{
				//计提利息
				if (printOperationInfo.getRealInterestReceivable() > 0)
				{
					Amount1 = DataFormat.formatAmount(printOperationInfo.getAmount());
					
					Interest1 = (DataFormat.formatAmount(printOperationInfo.getRealInterestReceivable()));
					...
					//取上次计提日期，作为计提日期的结束日期
					OverDate1 = DataFormat.getChineseDateString(tsLastPreDraw);
					
					DayNumber1 = ((PrintVoucher.getIntervalDays(printOperationInfo.getStartDate(), tsLastPreDraw, 2)) + "");
						
					if (printOperationInfo.getPayableInterest() > 0)
					{
						Amount2 = DataFormat.formatAmount(printOperationInfo.getAmount());
						
						Interest2 = (DataFormat.formatAmount(printOperationInfo.getPayableInterest()));
						
						Rate2 = DataFormat.formatRate( printOperationInfo.getRate(), 6 );
						...
						if (printOperationInfo.getTransTypeID() == SETTConstant.TransactionType.FIXEDCONTINUETRANSFER)
						{
							if (printOperationInfo.getRealInterestReceivable() > 0)
							{
								BeginDate2 = DataFormat.getChineseDateString(tsLastPreDraw);
							}
							else
							{
								BeginDate2 = DataFormat.getChineseDateString(printOperationInfo.getStartDate());
						...
						if (printOperationInfo.getRealInterestReceivable() > 0)
						{
							DayNumber2 = ((PrintVoucher.getIntervalDays(tsLastPreDraw, printOperationInfo.getEndDate(), 2)) + "");
						}
						else
						{
							DayNumber2 = ((PrintVoucher.getIntervalDays(printOperationInfo.getStartDate(), printOperationInfo.getEndDate(), 2)) + "");
						}
					}
				}
				else
				{
					if (printOperationInfo.getPayableInterest() > 0)
					{
						...
						Interest1 = DataFormat.formatAmount(printOperationInfo.getPayableInterest());
				...
				totalInterest = (
					DataFormat.formatAmount(
							DataFormat.formatDouble(
									DataFormat.formatDouble(printOperationInfo.getRealInterestReceivable()) + DataFormat.formatDouble(printOperationInfo.getPayableInterest()))));
			}*/
	       ...
	       //add by xwhe 2008-08-26
			...
			if (printOperationInfo.getTransTypeID() == SETTConstant.TransactionType.TRUST_LOAN_INTEREST)
			{
				...
				usedFields.put("BusinessTypeName", "自营贷款结息");
			}
			else if (printOperationInfo.getTransTypeID() == SETTConstant.TransactionType.CONSIGN_LOAN_INTEREST)
			{
				...
				usedFields.put("BusinessTypeName", "委托贷款结息");
			}
			...
			else if (printOperationInfo.getTransTypeID() == SETTConstant.TransactionType.CONSIGN_LOAN_PREDRAW_INTEREST)
			{
				...
				usedFields.put("BusinessTypeName", "委托贷款计提利息");
			}
			else if (printOperationInfo.getTransTypeID() == SETTConstant.TransactionType.TRUST_LOAN_PREDRAW_INTEREST)
			{
				...
				usedFields.put("BusinessTypeName", "自营贷款计提利息");
			}
			...
			else if ( printOperationInfo.getTransTypeID() == SETTConstant.TransactionType.INTERESTFEEPAYMENT
				   || printOperationInfo.getTransTypeID() == SETTConstant.TransactionType.TRUSTLOANRECEIVE
				   || printOperationInfo.getTransTypeID() == SETTConstant.TransactionType.CONSIGNLOANRECEIVE )
			{
				...
				{
					//usedFields.put("BusinessTypeName", SETTConstant.AccountType.getName( printOperationInfo.getAccountTypeID() )+"结息" );
				...
				{
					//usedFields.put("BusinessTypeName", SETTConstant.AccountType.getName( printOperationInfo.getAccountTypeID() )+"结手续费" );
					usedFields.put("BusinessTypeName", SETTConstant.AccountType.getName( printOperationInfo.getAccountTypeID() ) );
			...
			if ( printOperationInfo.getTransTypeID() == SETTConstant.TransactionType.TRUSTLOANRECEIVE 
			  || printOperationInfo.getTransTypeID() == SETTConstant.TransactionType.CONSIGNLOANRECEIVE )
			{
				if (printOperationInfo.getLoanAccountID()> 0)
				{
					AccountName = NameRef.getAccountNameByID(printOperationInfo.getLoanAccountID());
					AccountNo = NameRef.getAccountNoByID(printOperationInfo.getLoanAccountID());
				}				
			}
			else
			{
			...
			contractInfo = contractDao.findByID( printOperationInfo.getContractID() );
			if (contractInfo != null)
			{
				if (contractInfo.getInputDate() != null)
				{
					...
					//合同签订日
					usedFields.put("DateSign", DataFormat.formatDate( contractInfo.getLoanStart() ) );
				}
			}
			
			//支付日期
			if (printOperationInfo.getClearInterestDate() != null)
			{
				//strYear = DataFormat.getYearString(printOperationInfo.getClearInterestDate());
				//strMonth = Long.valueOf(DataFormat.getMonthString(printOperationInfo.getClearInterestDate())).longValue() + 1 + "";
				//strDay = DataFormat.getDayString(printOperationInfo.getClearInterestDate());
				//strTemp = "<u>&nbsp;" + strYear + "&nbsp;</u>年<u>&nbsp;" + strMonth + "&nbsp;</u>月<u>&nbsp;" + strDay + "&nbsp;</u>日";
				//info.setDateDuePay(strTemp);
				
				usedFields.put("DateSign", DataFormat.formatDate( printOperationInfo.getClearInterestDate() ) );
			}
			
			//贷款起始日期
			if (contractInfo != null)
			{
				printOperationInfo.setStartDate(contractInfo.getLoanStart());
			}
			if (printOperationInfo.getStartDate() != null)
			{
				usedFields.put("StartDate", DataFormat.formatDate( printOperationInfo.getStartDate() ) );
			}
			
			//贷款到期日期
			if (contractInfo != null)
			{
				printOperationInfo.setEndDate(contractInfo.getLoanEnd());
			}
			if (printOperationInfo.getEndDate() != null)
			{
				usedFields.put("EndDate", DataFormat.formatDate( printOperationInfo.getEndDate() ) );
			}
			
			//贷款期限
			if( printOperationInfo.getStartDate() != null && printOperationInfo.getEndDate() != null )
			{
				usedFields.put("IntervalDay", ""+ DataFormat.getIntervalDays( printOperationInfo.getStartDate(), printOperationInfo.getEndDate() ) );
			}
			*/
			...
			if( printOperationInfo.getTransTypeID() == SETTConstant.TransactionType.GENERALLEDGER )
			{
				...
				if(printOperationInfo.getPayAccountID() > 0 || printOperationInfo.getReceiveAccountID() > 0)
				{
					if(printOperationInfo.getPayAccountID() > 0)
					{
						...
						usedFields.put("GenerPayAccountNo1", printOperationInfo.getSSubject() != null? printOperationInfo.getSSubject():NameRef.getSubjectByAccountTypeID( printOperationInfo.getNAccountTypeID() ));
					}
					else
					{
						usedFields.put("GenerReceiveAccount", NameRef.getAccountNoByID( printOperationInfo.getReceiveAccountID() ));
						usedFields.put("GenerReceiveAccountNo1", printOperationInfo.getSSubject() != null? printOperationInfo.getSSubject():NameRef.getSubjectByAccountTypeID( printOperationInfo.getNAccountTypeID() ));
					}
					
					usedFields.put("GenerAmount1", DataFormat.formatListAmount( printOperationInfo.getAccountAmount(), 2 ));
					//2011-03-04 by jiangqi  
					//BUG #5013::电子单据柜-设置-单据格式设置：特转贷方传票中，[总帐交易]中，增加“金额1大写”字段显示。
					usedFields.put("ChineseGenerAmount1", PrintDataFormat.getChineseFormatAmount( printOperationInfo.getAccountAmount(), printOperationInfo.getCurrencyID()));
				...
				else
				{
					if(printOperationInfo.getPayGL1() > 0 || printOperationInfo.getReceiveGL1() > 0)
					{
						...
						//2011-03-04 by jiangqi  
						//BUG #5013::电子单据柜-设置-单据格式设置：特转贷方传票中，[总帐交易]中，增加“金额1大写”字段显示。
						usedFields.put("ChineseGenerAmount1", PrintDataFormat.getChineseFormatAmount( printOperationInfo.getAmount1(), printOperationInfo.getCurrencyID()));
					...
					else
					{
						if(printOperationInfo.getPayGL2() > 0 || printOperationInfo.getReceiveGL2() > 0)
						{
							...
							//2011-03-04 by jiangqi  
							//BUG #5013::电子单据柜-设置-单据格式设置：特转贷方传票中，[总帐交易]中，增加“金额1大写”字段显示。
							usedFields.put("ChineseGenerAmount1", PrintDataFormat.getChineseFormatAmount( printOperationInfo.getAmount2(), printOperationInfo.getCurrencyID()));
						...
						else
						{
							if(printOperationInfo.getPayGL3() > 0 || printOperationInfo.getReceiveGL3() > 0)
							{
								...
								//2011-03-04 by jiangqi  
								//BUG #5013::电子单据柜-设置-单据格式设置：特转贷方传票中，[总帐交易]中，增加“金额1大写”字段显示。
								usedFields.put("ChineseGenerAmount1", PrintDataFormat.getChineseFormatAmount( printOperationInfo.getAmount3(), printOperationInfo.getCurrencyID()));
							...
							else
							{
								if(printOperationInfo.getPayGL4() > 0 || printOperationInfo.getReceiveGL4() > 0)
								{
									...
									//2011-03-04 by jiangqi  
									//BUG #5013::电子单据柜-设置-单据格式设置：特转贷方传票中，[总帐交易]中，增加“金额1大写”字段显示。
									usedFields.put("ChineseGenerAmount1", PrintDataFormat.getChineseFormatAmount( printOperationInfo.getAmount4(), printOperationInfo.getCurrencyID()));
		...
		//add by xiangzhou 签章中日期取执行日
		boolean isShow = Config.getBoolean(ConfigConstant.GLOBAL_SIGNATURE_SHOWDATE, false);
		if(isShow){
			usedFields.put("SignatureDate", DataFormat.getDateString(printOperationInfo.getExecuteDate()));
		}
		usedFields.put("IsAutoContinue", printOperationInfo.getIsAutoContinue()==1?"是":"否");
		//网银指令信息
		if(printOperationInfo.getInstructionNo()!=null && !printOperationInfo.getInstructionNo().equals("")){
			...
			try
			{
				...
				financeInfo = obFinanceInstrDao.findByID(Long.valueOf(printOperationInfo.getInstructionNo()).longValue());
			}
			catch(NumberFormatException ne)
			{
				Log.print("=======获取网银指令提交时间时出错，该指令非网银指令==  指令号为："+printOperationInfo.getInstructionNo());
			}
			
			if(financeInfo!=null)
			{
				//提交时间
				usedFields.put("DtOBsubmit", financeInfo.getSignDate() != null ? DataFormat.formatDate(financeInfo.getSignDate()) : DataFormat.formatDate(financeInfo.getCheckDate()));
			}
		}
